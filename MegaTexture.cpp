...
====================
*/
void idMegaTexture::SetViewOrigin( const anVec3 viewOrigin ) {
...
==============
*/
float arcHeightMap::GetInterpolatedHeight( const anVec3 &pos, const arcHeightMapScaleData &scale ) const {
...
==============
*/
void arcHeightMap::GetHeight( const anBounds &in, anVec2 &out, const arcHeightMapScaleData &scale ) const {
	if ( data.Num() == 0 ) {
...
==============
*/
float arcHeightMap::GetHeight( const anVec3 &start, const anVec3 &end, const arcHeightMapScaleData &scale ) const {
	if ( data.Num() == 0 ) {
...
==============
*/
float arcHeightMap::TracePoint( const anVec3& start, const anVec3& end, anVec3& result, float heightOffset, const arcHeightMapScaleData& scale ) const {
	result = end;
	if ( data.Num() == 0 ) {
...
================
*/
void arcHeightMapInstance::Init( const char *declName, const anBounds &bounds ) {
...
void arcHeightMapInstance::Init( const sdHeightMap *map, const anBounds& bounds ) {
	heightMap = map;
	heightMapData.Init( bounds );
}