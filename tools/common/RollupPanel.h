...
ARC_INLINE int rvRollupPanel::GetItemCount ( void )
{
	return mItems.Num();
}

ARC_INLINE bool rvRollupPanel::IsItemExpanded ( int index )
{
...
ARC_INLINE bool rvRollupPanel::IsItemEnabled( int index )
{
	if ( index >= mItems.Num() || index < 0 )
	{
		return false;
...
ARC_INLINE HWND rvRollupPanel::GetWindow ( void )
{
	return mWindow;
}
