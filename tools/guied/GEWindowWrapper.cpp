...
rvGEWindowWrapper::rvGEWindowWrapper( idWindow *window,EWindowType type ) {
	...
	if ( dynamic_cast< idEditWindow*>(window) ) {
		mType = WT_EDIT;
	} else if ( dynamic_cast< arcListWindow*>(window) ) {
		mType = WT_LIST;
...
void rvGEWindowWrapper::SetState( const anDict &dict ) {
	...
	UpdateWindowState(); // Push the window state to the window itself
	UpdateRect();	// Update the internal rectangle since it may have changed in the state
}
