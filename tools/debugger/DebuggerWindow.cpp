	...
	switch { msg ) {
		...
		case WM_CLOSE:
			if ( window->mClient->IsConnected() ) {
				if ( IDNO == MessageBox( wnd, "The debugger is currently connected to a running version of the game.  Are you sure you want to close now?", "Q4 Script Debugger", MB_YESNO|MB_ICONQUESTION ) ) {
					return 0;
				}
			}
			PostQuitMessage( 0 );
