		...
		if ( dict ) {
			...
			if ( DoNewColor( &r, &g, &b, &ob ) ) {
				for ( int i = 0; i < count; i++ ) {
					dict = engineEdit->EntityGetSpawnArgs( list[i] );
					...
					if ( ent ) {
						...
						engineEdit->EntityChangeSpawnArgs( ent, &dict2 );
						engineEdit->MapSetEntityKeyVal( name, "_color",  str );
					}
				}
			}
		}
...
void CDialogParticleEditor::SetSelectedModel( const char *val ) {
	...
	if ( count ) {
		for ( int i = 0; i < count; i++ ) {
			...
			const char *name = dict->GetString( "name" );
			engineEdit->EntitySetModel( list[i], val );
			engineEdit->EntityUpdateVisuals( list[i] );
			engineEdit->EntityGetAxis( list[i], axis );
			vectorControl.SetidAxis( axis );
			engineEdit->MapSetEntityKeyVal( name, "model", val );
		}
		CWnd *wnd = GetDlgItem( IDC_BUTTON_SAVE_PARTICLEENTITIES );
		if ( wnd ) {
			wnd->EnableWindow( TRUE );
	...
	if ( particleMode ) {
		...
		if ( count ) {
			for ( int i = 0; i < count; i++ ) {
				...
				const char *name = dict->GetString( "name" );
				engineEdit->EntitySetAxis( list[i], rotation.ToMat3() );
				engineEdit->EntityUpdateVisuals( list[i] );
				engineEdit->MapSetEntityKeyVal( name, "rotation", rotation.ToMat3().ToString() );
			}
			CWnd *wnd = GetDlgItem( IDC_BUTTON_SAVE_PARTICLEENTITIES );
			if ( wnd ) {
				wnd->EnableWindow( TRUE );
