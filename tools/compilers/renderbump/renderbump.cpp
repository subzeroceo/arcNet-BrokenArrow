...
static void SaveWindow( void ) {
	oldWidth = qglConfig.vidWidth;
	oldHeight = qglConfig.vidHeight;
}
		...
		for ( j = 0; j < height; j++ ) {
			...
			for ( k = -1; k < 2; k++ ) {
				for ( l = -1; l < 2; l++ ) {
					...
					in = orig + ( ( ( j + 1 )&( height-1 ) )*width + (( i+k)&( width-1 ) ) ) * 4;
...
static void RenderBumpTriangles( srfTriangles_t *lowMesh, renderBump_t *rb ) {
	...
	for ( int j = 0; j < lowMesh->numIndexes; j += 3 ) {
		...
		qglClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );
		qglRasterPos2f( 0, 1 );
		qglPixelZoom( qglConfig.vidWidth / ( float )rb->width, qglConfig.vidHeight / ( float )rb->height );
