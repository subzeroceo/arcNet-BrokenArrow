...
void WXY_Print( void ) {
	...
	if ( !PrintDlg(&pd) || !pd.hDC) {
		g_pParentWnd->MessageBox( "Could not PrintDlg()", "Radiant Print Error", MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (StartDoc(pd.hDC, &di) <= 0 ) {
		g_pParentWnd->MessageBox( "Could not StartDoc()", "Radiant Print Error", MB_OK | MB_ICONERROR);
		return;
	}

	// StartPage
	if (StartPage(pd.hDC) <= 0 ) {
		g_pParentWnd->MessageBox( "Could not StartPage()", "Radiant Print Error", MB_OK | MB_ICONERROR);
	return;
	...
	// EndPage and EndDoc
	if (EndPage(pd.hDC) <= 0 ) {
		g_pParentWnd->MessageBox( "RadiantPrint Error", "Could not EndPage()", MB_OK | MB_ICONERROR);
		return;
	}

	if (EndDoc(pd.hDC) <= 0 ) {
		g_pParentWnd->MessageBox( "Radiant Print Error", "Could not EndDoc()", MB_OK | MB_ICONERROR);
		return;
	}
}
		...
		if (PointMode() ) {
			if (g_pMovingPoint && GetCapture() == this) {
				...
				for ( int n = 0; n < g_nPointCount; n++ ) {
					if ( fDiff(g_PointPoints[n].m_ptClip[nDim1], tdp[nDim1] ) < 3 && fDiff(g_PointPoints[n].m_ptClip[nDim2], tdp[nDim2] ) < 3 ) {
						bCrossHair = true;
			...
			if (g_pMovingClip && GetCapture() == this) {
				...
				int nDim2 = (m_nViewType == XY) ? 1 : 2;
				if (g_Clip1.Set() ) {
					if ( fDiff(g_Clip1.m_ptClip[nDim1], tdp[nDim1] ) < 3 && fDiff(g_Clip1.m_ptClip[nDim2], tdp[nDim2] ) < 3 ) {
						bCrossHair = true;
				...
				if (g_Clip2.Set() ) {
					if ( fDiff(g_Clip2.m_ptClip[nDim1], tdp[nDim1] ) < 3 && fDiff(g_Clip2.m_ptClip[nDim2], tdp[nDim2] ) < 3 ) {
						bCrossHair = true;
				...
				if (g_Clip3.Set() ) {
					if (fDiff(g_Clip3.m_ptClip[nDim1], tdp[nDim1] ) < 3 && fDiff(g_Clip3.m_ptClip[nDim2], tdp[nDim2] ) < 3 ) {
						bCrossHair = true;
			...
			if (g_pMovingPath && GetCapture() == this) {
				...
				for ( int n = 0; n < g_nPathCount; n++ ) {
					if (fDiff(g_PathPoints[n].m_ptClip[nDim1], tdp[nDim1] ) < 3 && fDiff(g_PathPoints[n].m_ptClip[nDim2], tdp[nDim2] ) < 3) {
						bCrossHair = true;
...
void FinishSmartCreation() {
	...
	if (g_strSmartEntity.Find( "Smart_Train" ) >= 0 ) {
		...
		for (n = 0; n < g_nPathCount; n++ ) {
			Select_Deselect();
			CreateRightClickEntity(g_pParentWnd->ActiveXY(), g_PathPoints[n].m_ptScreen.x, g_PathPoints[n].m_ptScreen.y,
				"path_corner"
			);
			array.Add(reinterpret_cast < void * > (selected_brushes.next) );
		}
...
void DrawPathLines( void ) {
	...
	int num_entities = 0;
	for ( entity_t *te = entities.next; te != &entities && num_entities != MAX_MAP_ENTITIES; te = te->next) {
	...
	for ( entity_t *se = entities.next; se != &entities; se = se->next) {
		...
		for ( int k = 0; k < num_entities; k++ ) {
			...
			entity_t *te = ent_entity[k];
			brush_t *tb = te->brushes.onext;
			if (tb == &te->brushes) {
				continue;
			}
