...
void CDlgCamera::OnBtnAddevent() {
	...
	if (dlg.DoModal() == IDOK) {
		...
		g_splineList->addEvent(static_cast<aRCCameraEvent::eventType>(dlg.m_event+1 ), dlg.m_strParm, n);
		setupFromCamera();
	}
}
...
void CDlgCamera::OnBtnAddtarget() {
	...
	if (dlg.DoModal() == IDOK) {
		g_splineList->addTarget(dlg.m_strName, static_cast<ARCCameraPos::positionType>(dlg.m_nType) );
		setupFromCamera();
