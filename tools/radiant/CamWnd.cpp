...
void CCamWnd::BuildEntityRenderState( entity_t *ent, bool update) {
	...
	if ( v && *v ) {
		...
		refent.sndRef = ent->soundEmitter;
		...
		if ( !stricmp( name, v ) ) {
			...
			if (md5) {
				...
				refent.joints = ( arcJointMat * )Mem_Alloc16( refent.numJoints * sizeof( *refent.joints ) );
				const idMD5Anim *anim = engineEdit->ANIM_GetAnimFromEntityDef(spawnArgs.GetString( "classname" ), str);
	...
	engineEdit->ParseSpawnArgsToRenderLight( &spawnArgs, &lightParms );
	lightParms.sndRef = ent->soundEmitter;
