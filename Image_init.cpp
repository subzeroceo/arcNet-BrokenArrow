...
void R_HorizontalFlip( GLbyte *data, int width, int height ) {
	...
	for ( int i = 0; i < height; i++ ) {
		for ( int j = 0; j < width / 2; j++ ) {
    	    int temp = intData[i * width + j];
            intData[i * width + j] = intData[i * width + width - 1 - j];
            intData[i * width + width - 1 - j] = temp;
		}
	}
}
...
void R_VerticalFlip( GLbyte *data, int width, int height ) {
	...
	for ( int i = 0; i < width; i++ ) {
		for ( int j = 0; j < height / 2; j++ ) {
            int temp = intData[j * width + i];
            intData[j * width + i] = intData[( height - 1 - j ) * width + i];
            intData[( height - 1 - j ) * width + i] = temp;
        }
	}
}
...
This provices a Lanczos Interpolation to scale an image, its simplified and assumes
the image has RGBA channels But we will improve this later
...
anImage *anImageManager::AllocStandaloneImage( const char *name ) {
	...
	anImage * image = new (TAG_IMAGE) anImage( name );
...
anImage *anImageManager::ImageFromFunction( const char *_name, void (*generatorFunction)( anImage *image ) ) {
	...
	if ( !name ) {
		common->FatalError( "anImageManager::ImageFromFunction: NULL name" );
	}
...
void anImageManager::ReloadAllImages() {
	...
	SetNormalPalette();
	anCommandArgs args.TokenizeString( "reloadImages reload", false );
...
====================
*/
void anImageManager::Preload( const idPreloadManifest &manifest, const bool & mapPreload ) {
...
void anImageManager::EndLevelLoad() {
	...
	int	end = Sys_Milliseconds();
	common->Printf( "----- anImageManager::EndLevelLoad -----\n" );
	...
	for ( int i = 0; i < images.Num(); i++ ) {
		...
		} else if ( image->texnum != anImage::TEXTURE_NOT_LOADED ) {
	...
	for ( int i = 0; i < images.Num(); i++ ) {
		...
		if ( image->levelLoadReferenced && image->texnum == anImage::TEXTURE_NOT_LOADED && !image->partialImage ) {
...
void anImageManager::FinishBuild( bool removeDups ) {
	...
	anFile *batchFile = fileSystem->OpenFileWrite( ( removeDups ) ? "makedds2.bat" : "makedds.bat" );
	if ( batchFile ) {
...
void anImageManager::PrintMemInfo( MemInfo_t *mi ) {
	...
	for ( i = 0; i < images.Num(); i++ ) {
		...
		f->Printf( "%s %3i %s\n", anString::FormatNumber( size ).c_str(), im->refCount, im->imgName.c_str() );
	}
...
/*void anImage::GenerateShadowArray( int width, int height, textureFilter_t filterParm, textureRepeat_t repeatParm, textureUsage_t usageParm ) {
	PurgeImage();
...
static void GetGeneratedName( anString &_name, const textureUsage_t &_usage, const cubeFiles_t &_cube ) {
	static char sName[MAX_IMAGE_NAME];
	int		i = 0;
	char	letter;

	while ( name[i] != '\0' && i < MAX_IMAGE_NAME - 1 ) {
