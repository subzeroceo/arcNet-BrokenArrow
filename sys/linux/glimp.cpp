...
void GLimp_WakeBackEnd( void *a ) {
	BE_RenderDPrintf( "GLimp_WakeBackEnd stub\n" );
}
...
void GLimp_EnableLogging( bool log ) {
	static bool logging;
	if (log != logging) {
		BE_RenderDPrintf( "GLimp_EnableLogging - disabled at compile time (ID_GL_HARDLINK)\n" );
		logging = log;
	}
}
#endif
...
void GLimp_FrontEndSleep() {
	BE_RenderDPrintf( "GLimp_FrontEndSleep stub\n" );
}

void *GLimp_BackEndSleep() {
	BE_RenderDPrintf( "GLimp_BackEndSleep stub\n" );
	return 0;
}

bool GLimp_SpawnRenderThread( void(*a)() ) {
	BE_RenderDPrintf( "GLimp_SpawnRenderThread stub\n" );
	return false;
}
...
void GLimp_SetGamma( unsigned short red[256], unsigned short green[256], unsigned short blue[256] ) {
	if ( dpy ) {
		...
		BE_RenderDPrintf( "GammaRampSize: %d\n", size);
...
// GLX_TestDGA
// Check for DGA - update in_dgamouse if neede
void GLX_TestDGA() {
	...
	if ( !XF86DGAQueryVersion( dpy, &dga_MajorVersion, &dga_MinorVersion ) ) {
		...
		BE_RenderPrintf( "Failed to detect DGA DirectVideo Mouse\n" );
		// in_dgamouse = false;
		dga_found = false;
	} else {
		BE_RenderPrintf( "DGA DirectVideo Mouse (Version %d.%d) initialized\n",
...
int idXErrorHandler(Display * l_dpy, XErrorEvent * ev) {
	char buf[1024];
	BE_RenderPrintf( "Fatal X Error:\n" );
	BE_RenderPrintf( "  Major opcode of failed request: %d\n", ev->request_code );
	BE_RenderPrintf( "  Minor opcode of failed request: %d\n", ev->minor_code );
	BE_RenderPrintf( "  Serial number of failed request: %lu\n", ev->serial );
...
bool GLimp_OpenDisplay( void ) {
	...
	//if ( cvarSystem->GetCVarInteger( "net_serverDedicated" ) == 1 ) {
		BE_RenderDPrintf( "not opening the display: dedicated server\n" );
		return false;
	//}

	BE_RenderPrintf( "Setup X Session Display Connection\n" );

	// that should be the first call into X
	if ( !XInitThreads() ) {
		BE_RenderPrintf( "XInitThreads Failed\n" );
		return false;
	}
...
#define ATTR_ALPHA_IDX 13
	...
	BE_RenderPrintf( "Initializing OpenGL display\n" );

	root = RootWindow( dpy, scrnum );

	actualWidth = qglConfig.vidWidth;
	actualHeight = qglConfig.vidHeight;

	// Get video mode list
	if ( !XF86VidModeQueryVersion( dpy, &vidmode_MajorVersion, &vidmode_MinorVersion ) ) {
		vidmode_ext = false;
		BE_RenderPrintf( "XFree86 Video Mode Extension not available\n" );
	} else {
		vidmode_ext = true;
		BE_RenderPrintf( "Using XFree86 Version %d.%d\n", vidmode_MajorVersion, vidmode_MinorVersion);
	}
	...
	if ( vidmode_ext ) {
		...
		if ( a.fullScreen ) {
			...
			if (best_fit != -1 ) {
				...
				BE_RenderPrintf( "XFree86 Video Mode Extension Activated at %dx%d\n", actualWidth, actualHeight );
			} else {
				a.fullScreen = false;
				BE_RenderPrintf( "XFree86 Video Mode Extension: No acceptable modes found\n" );
			}
		} else {
			BE_RenderPrintf( "XFree86 Video Mode Extension: not fullscreen, ignored\n" );
		}
	}
	...
	for ( i = 0; i < 16; i++ ) {
		...
		BE_RenderPrintf( "Using %d/%d/%d Color bits, %d Alpha bits, %d depth, %d stencil display.\n",
			 attrib[ATTR_RED_IDX], attrib[ATTR_GREEN_IDX],
			 attrib[ATTR_BLUE_IDX], attrib[ATTR_ALPHA_IDX],
			 attrib[ATTR_DEPTH_IDX], attrib[ATTR_STENCIL_IDX] );

		qglConfig.colorBits = tcolorbits;
		qglConfig.depthBits = tdepthbits;
		qglConfig.stencilBits = tstencilbits;
		break;
	}

	if ( !visinfo) {
		BE_RenderPrintf( "Cannot get a visual\n" );
		return false;
	}
	// window attributes
	...
	qglConfig.isFullscreen = a.fullScreen;
...
int Sys_GetVideoRam( void ) {
	...
	BE_RenderPrintf( "guessing video ram ( use +set sys_videoRam to force ) ..\n" );
	if ( !GLimp_OpenDisplay() ) {
		run_once = 64;
		return run_once;
	}
	...
	if ( XNVCTRLQueryVersion( l_dpy, &major, &minor ) ) {
		BE_RenderPrintf( "found XNVCtrl extension %d.%d\n", major, minor );
		if ( XNVCTRLIsNvScreen( l_dpy, l_scrnum ) ) {
			if ( XNVCTRLQueryAttribute( l_dpy, l_scrnum, 0, NV_CTRL_VIDEO_RAM, &value ) ) {
				run_once = value / 1024;
				return run_once;
			} else {
				BE_RenderPrintf( "XNVCtrlQueryAttribute NV_CTRL_VIDEO_RAM failed\n" );
			}
		} else {
			BE_RenderPrintf( "default screen %d is not controlled by NVIDIA driver\n", l_scrnum );
		}
	}
	...
	if ( ( fd = open( "/proc/dri/0/umm", O_RDONLY ) ) != -1 ) {
		...
		if ( ( len = read( fd, umm_buf, 1024 ) ) != -1 ) {
			...
			BE_RenderPrintf( "read /proc/dri/0/umm Failed: %s\n", strerror( errno ) );
		}
	}
	BE_RenderPrintf( "Estimate Failed, return default low-end VRAM setting ( 64MB VRAM )\n" );
	run_once = 64;
	return run_once;
}
