...
const char *Sys_EXEPath( void ) {
	...
	if ( len == -1 ) {
		Sys_Printf( "couldn't start exe path link %s\n", linkpath.c_str() );
		buf[ len ] = '\0';
	}
	return buf;
}
...
const char *Sys_DefaultBasePath( void ) {
	...
	if ( basepath.Length() ) {
		...
		if ( stat( testbase.c_str(), &st ) != -1 && S_ISDIR( st.st_mode ) ) {
			...
			Printf( "no '%s' directory in exe path %s, skipping\n", BASE_GAMEDIR, basepath.c_str() );
		}
	}
	if ( basepath != Posix_Cwd() ) {
		...
		if ( stat( testbase.c_str(), &st ) != -1 && S_ISDIR( st.st_mode ) ) {
			...
			Printf( "no '%s' directory in cwd path %s, skipping\n", BASE_GAMEDIR, basepath.c_str() );
		}
	}
	Printf( "WARNING: using hardcoded default base path\n" );
...
double Sys_ClockTicksPerSecond( void ) {
	...
	if ( fd == -1 ) {
		...
		init = true;
		Printf( "Measured CPU frequency: %g MHz\n", ret / 1000000.0 );
		return ret;
	}
	...
	while ( pos < len ) {
		if ( !anString::Cmpn( buf + pos, "CPU MHz", 7 ) ) {
			...
			if ( pos < len && end < len ) {
				...
				Printf( "Failed Parsing CPU-Info\n" );
				ret = MeasureClockTicks();
				init = true;
				Printf( "Measured CPU frequency: %g MHz\n", ret / 1000000.0 );
				return ret;
			}
			Printf( "/proc/cpuinfo CPU frequency: %g MHz\n", ret );
			ret *= 1000000;
			init = true;
			return ret;
		}
...
int Sys_GetSystemRam( void ) {
	...
	if ( count == -1 ) {
		Printf( "GetSystemRam: sysconf _SC_PHYS_PAGES failed\n" );
		return 512;
	}
	page_size = sysconf( _SC_PAGE_SIZE );
	if ( page_size == -1 ) {
		Printf( "GetSystemRam: sysconf _SC_PAGE_SIZE failed\n" );
		return 512;
	}
...
void anSystemLocal::OpenURL( const char *url, bool quit ) {
	...
	if ( !script_file ) {
		Printf( "Cannot find URL script 'openurl.sh' in either savepath or basepath\n" );
