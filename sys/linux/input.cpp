...
static void Sys_XInstallGrabs( void ) {
	...
	XWarpPointer( dpy, None, win,
				 0, 0, 0, 0,
				 qglConfig.vidWidth / 2, qglConfig.vidHeight / 2 );
...
void Sys_XUninstallGrabs( void ) {
	...
	XWarpPointer( dpy, None, win, 0, 0, 0, 0, qglConfig.vidWidth / 2, qglConfig.vidHeight / 2);
		...
		switch (event.type) {
			...
			case MotionNotify:
				...
				if (in_dgamouse.GetBool() ) {
					...
					if (event.xmotion.x == qglConfig.vidWidth / 2 &&
						event.xmotion.y == qglConfig.vidHeight / 2) {
						mwx = qglConfig.vidWidth / 2;
						mwy = qglConfig.vidHeight / 2;
