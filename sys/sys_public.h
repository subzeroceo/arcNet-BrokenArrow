...
typedef enum {
	...
	CPUID_AVX = 0x100000 		// AVX instructions and processor support
} cpuid_t;
...
typedef enum {
  ...
  SE_MAX      // for event overflow debugging
} sysEventType_t;
...
public:
  ...
  // bool		SendPacketVDP( const struct sockaddr *to, const void *data, int
  // size );
  // void		GetTrafficStats(  int &bytesSent, int &packetsSent, int
  // &bytesReceived, int &packetsReceived ) const;
...
void Sys_CreateThread(xthread_t function, void *parms, xthreadPriority priority,
                      xthreadInfo &info, const char *name,
                      xthreadInfo *threads[MAX_THREADS], int *thread_count);
