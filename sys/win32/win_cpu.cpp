...
==============================================================
	...
	Clock ticks
						...
						NOTE OF THE AVX FUNC.

		BIT 25 is SSE, so we believe 28 for the AVX CPUID bit under [_REG_ECX].
		Further investigation is required. we need to locate the AVX CPUID bit
			number and insure its correct, if not correct it immediately.
==============================================================
*/
...
BIT 25 is SSE, so we believe 28 for the AVX CPUID bit under [_REG_ECX].
Further investigation is required. we need to locate the AVX CPUID bit number and insure its
correct, if not correct it immediately.
================
*/
static bool HasAVX( void ) {
    ...
    int numExtendedFunctions = regs[_REG_EAX];

	// get CPU feature bits
	CPUID( 1, regs );

	// check if AVX feature is supported
	// bit 25 of EDX denotes SSE existence
	if ( regs[_REG_EDX] & ( 1 << 5 ) ) {
		return true;
	}
