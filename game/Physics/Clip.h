...
public:
							...
							explicit idClipModel( const idTraceModel &trm, const arcMaterial *material = NULL );
...
// RAVEN END
	...
	void					LoadModel( const idTraceModel &trm, const arcMaterial *material, bool notHashed = false );
...
// ddynerman: multiple clip worlds
	...
	void					Link( idEntity *ent, int newId, const arcVec3 &newOrigin, const arcMat3 &newAxis, int renderModelHandle = -1 );
// RAVEN END
	void					Unlink( void );						// unlink from sectors
	void					SetPosition( const arcVec3 &newOrigin, const arcMat3 &newAxis );	// unlinks the clip model
	void					Translate( const arcVec3 &translation );							// unlinks the clip model
	...
	const arcBounds &		GetBounds( void ) const;
	const arcBounds &		GetAbsBounds( void ) const;
	const arcVec3 &			GetOrigin( void ) const;
	const arcMat3 &			GetAxis( void ) const;
...
// RAVEN END
	...
	arcVec3					origin;					// origin of clip model
	arcMat3					axis;					// orientation of clip model
	arcBounds				bounds;					// bounds
	arcBounds				absBounds;				// absolute bounds
	...
	static int				AllocTraceModel( const idTraceModel &trm, const arcMaterial *material, bool notHashed = false );
	static void				ReplaceTraceModel( int index, const idTraceModel &trm, const arcMaterial *material, bool notHashed = false );
...
ARC_INLINE void idClipModel::Translate( const arcVec3 &translation ) {
	Unlink();
	origin += translation;
}

ARC_INLINE void idClipModel::Rotate( const idRotation &rotation ) {
	Unlink();
	origin *= rotation;
	axis *= rotation.ToMat3();
}

ARC_INLINE void idClipModel::Enable( void ) {
	enabled = true;
}

ARC_INLINE void idClipModel::Disable( void ) {
	enabled = false;
}

ARC_INLINE void idClipModel::SetContents( int newContents ) {
...
ARC_INLINE int idClipModel::GetContents( void ) const {
	return contents;
}

ARC_INLINE void idClipModel::SetEntity( idEntity *newEntity ) {
	entity = newEntity;
}

ARC_INLINE idEntity *idClipModel::GetEntity( void ) const {
	return entity;
}

ARC_INLINE void idClipModel::SetId( int newId ) {
	id = newId;
}

ARC_INLINE int idClipModel::GetId( void ) const {
	return id;
}

ARC_INLINE void idClipModel::SetOwner( idEntity *newOwner ) {
	owner = newOwner;
}

ARC_INLINE idEntity *idClipModel::GetOwner( void ) const {
	return owner;
}

ARC_INLINE const arcBounds &idClipModel::GetBounds( void ) const {
	return bounds;
}

ARC_INLINE const arcBounds &idClipModel::GetAbsBounds( void ) const {
	return absBounds;
}

ARC_INLINE const arcVec3 &idClipModel::GetOrigin( void ) const {
	return origin;
}

ARC_INLINE const arcMat3 &idClipModel::GetAxis( void ) const {
	return axis;
}

ARC_INLINE bool idClipModel::IsRenderModel( void ) const {
	return ( renderModelHandle != -1 );
}

ARC_INLINE bool idClipModel::IsTraceModel( void ) const {
	return ( traceModelIndex != -1 );
}

ARC_INLINE bool idClipModel::IsLinked( void ) const {
	return ( clipLinks != NULL );
}

ARC_INLINE bool idClipModel::IsEnabled( void ) const {
	return enabled;
}

ARC_INLINE bool idClipModel::IsEqual( const idTraceModel &trm ) const {
	return ( traceModelIndex != -1 && *GetCachedTraceModel( traceModelIndex ) == trm );
}

ARC_INLINE const idTraceModel *idClipModel::GetTraceModel( void ) const {
	if ( !IsTraceModel() ) {
		return NULL;
	}
...
// nmckenzie: we have cases where both a guy and his target need to be ignored by a translation
	bool					Translation( trace_t &results, const arcVec3 &start, const arcVec3 &end,
								const idClipModel *mdl, const arcMat3 &trmAxis, int contentMask, const idEntity *passEntity, const idEntity *passEntity2 = 0 );
// RAVEN END
	bool					Rotation( trace_t &results, const arcVec3 &start, const idRotation &rotation,
								const idClipModel *mdl, const arcMat3 &trmAxis, int contentMask, const idEntity *passEntity );
	bool					Motion( trace_t &results, const arcVec3 &start, const arcVec3 &end, const idRotation &rotation,
								const idClipModel *mdl, const arcMat3 &trmAxis, int contentMask, const idEntity *passEntity );
...
// AReis: Added ability to get the entity that was touched as well.
	int						Contents( const arcVec3 &start,
								const idClipModel *mdl, const arcMat3 &trmAxis, int contentMask, const idEntity *passEntity, idEntity **touchedEntity = NULL );
// RAVEN END

	// special case translations versus the rest of the world
	bool					TracePoint( trace_t &results, const arcVec3 &start, const arcVec3 &end,
								int contentMask, const idEntity *passEntity );
	bool					TraceBounds( trace_t &results, const arcVec3 &start, const arcVec3 &end, const arcBounds &bounds,
								int contentMask, const idEntity *passEntity );

	// clip versus a specific model
	void					TranslationModel( trace_t &results, const arcVec3 &start, const arcVec3 &end,
	...
	void					RotationModel( trace_t &results, const arcVec3 &start, const idRotation &rotation,
								const idClipModel *mdl, const arcMat3 &trmAxis, int contentMask,
								idCollisionModel *model, const arcVec3 &modelOrigin, const arcMat3 &modelAxis );
	int						ContactsModel( contactInfo_t *contacts, const int maxContacts, const arcVec3 &start, const arcVec6 &dir, const float depth,
								const idClipModel *mdl, const arcMat3 &trmAxis, int contentMask,
								idCollisionModel *model, const arcVec3 &modelOrigin, const arcMat3 &modelAxis );
	int						ContentsModel( const arcVec3 &start,
								const idClipModel *mdl, const arcMat3 &trmAxis, int contentMask,
								idCollisionModel *model, const arcVec3 &modelOrigin, const arcMat3 &modelAxis );
...
// nmckenzie: had to add a second pass entity so we can safely ignore both a guy and his target in some cases
	void					TranslationEntities( trace_t &results, const arcVec3 &start, const arcVec3 &end,
								const idClipModel *mdl, const arcMat3 &trmAxis, int contentMask, const idEntity *passEntity, const idEntity *passEntity2 = 0 );
// RAVEN END
	...
	int						EntitiesTouchingBounds( const arcBounds &bounds, int contentMask, idEntity **entityList, int maxCount ) const;
	int						ClipModelsTouchingBounds( const arcBounds &bounds, int contentMask, idClipModel **clipModelList, int maxCount ) const;

// RAVEN BEGIN
// ddynerman: another helper function, useful in MP
	int						PlayersTouchingBounds( const arcBounds &bounds, int contentMask, idPlayer **entityList, int maxCount ) const;
// RAVEN END

	const arcBounds &		GetWorldBounds( void ) const;
...
// RAVEN END
	...
	void					DrawClipModels( const arcVec3 &eye, const float radius, const idEntity *passEntity, const idTypeInfo* type = NULL );
...
// RAVEN BEGIN
// ddynerman: SD's clip sector code
	void					CoordsForBounds( int* coords, arcBounds& bounds ) const;
...
private:
	struct clipSector_s *	CreateClipSectors_r( const int depth, const arcBounds &bounds, arcVec3 &maxSector );
...
// RAVEN BEGIN
// nmckenzie: had to add a second pass entity so we can safely ignore both a guy and his target in some cases
	int						GetTraceClipModels( const arcBounds &bounds, int contentMask, const idEntity *passEntity, idClipModel **clipModelList, const idEntity *passEntity2 = 0 ) const;
// RAVEN END
	void					TraceRenderModel( trace_t &trace, const arcVec3 &start, const arcVec3 &end, const float radius, const arcMat3 &axis, idClipModel *touch ) const;
// RAVEN BEGIN
// ddynerman: SD's clip sector code
...
ARC_INLINE bool idClip::TracePoint( trace_t &results, const arcVec3 &start, const arcVec3 &end, int contentMask, const idEntity *passEntity ) {
	Translation( results, start, end, NULL, mat3_identity, contentMask, passEntity );
	return ( results.fraction < 1.0f );
}

ARC_INLINE bool idClip::TraceBounds( trace_t &results, const arcVec3 &start, const arcVec3 &end, const arcBounds &bounds, int contentMask, const idEntity *passEntity ) {
...
ARC_INLINE const arcBounds & idClip::GetWorldBounds( void ) const {
	return worldBounds;
}

// RAVEN BEGIN
// ddynerman: SD's clip sector code
ARC_INLINE void idClip::CoordsForBounds( int* coords, arcBounds& bounds ) const {
	float fCoords[ 4 ];
	...
	for( i = 0; i < 4; i++ ) {
		...
		coords[ i ] = arcMath::FtoiFast( fCoords[ i ] );
