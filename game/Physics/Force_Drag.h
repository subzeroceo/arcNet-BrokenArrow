...
public:
	...
	void				SetPhysics( idPhysics *physics, int id, const arcVec3 &p );
						// set position to drag towards
	void				SetDragPosition( const arcVec3 &pos );
						// get the position dragged towards
	const arcVec3 &		GetDragPosition( void ) const;
						// get the position on the dragged physics object
	const arcVec3		GetDraggedPosition( void ) const;
...
private:
	...
	arcVec3				p;				// position on clip model
	arcVec3				dragPosition;	// drag towards this position
};
