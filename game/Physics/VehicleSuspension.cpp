...
================
*/
bool sdVehicleSuspension_Pivot::Init( sdVehicleSuspensionInterface* object, const anDict& info ) {
	anVec3 wheelOrg, suspensionOrg;

	_object = object;

	anTransport* transport = _object->GetParent();
	...
	anVec3 dir = wheelOrg - suspensionOrg;
	dir[ 1 ] = 0.f;
...
bool sdVehicleSuspension_Pivot::UpdateIKJoints( idAnimator* animator ) {
	...
	if ( _lastOffset != -anMath::INFINITY ) {
...
================
*/
bool sdVehicleSuspension_DoubleWishbone::Init( sdVehicleSuspensionInterface* object, const anDict& info ) {
	anVec3 wheelOrg, suspensionOrg;
	anMat3 suspensionAxes;
...
================
*/
bool sdVehicleSuspension_Vertical::Init( sdVehicleSuspensionInterface* object, const anDict& info ) {
...
bool sdVehicleSuspension_Vertical::UpdateIKJoints( idAnimator* animator ) {
	float offset = _object->GetOffset();
	if ( _offset != -anMath::INFINITY && offset < _offset ) { 
		_offset = Lerp( _offset, offset, 0.3f );
	} else {
		_offset = offset;
	}

	if ( anMath::Fabs( _oldOffset - _offset ) < SUSPENSION_VERTICAL_OFFSET_THRESHOLD ) {
		return false;
	}

	animator->SetJointPos( _suspensionJoint, JOINTMOD_WORLD, anVec3( 0.f, 0.f, _offset ) );
	_oldOffset = _offset;
	return true;
}
...
================
*/
bool sdVehicleSuspension_2JointLeg::Init( sdVehicleSuspensionInterface* object, const anDict& info ) {
	...
	anVec3 dir = info.GetVector( "dir_hip", "0 0 1" );
	dir.Normalize();
...
bool sdVehicleSuspension_2JointLeg::UpdateIKJoints( idAnimator* animator ) {
	...
	if ( anMath::Fabs( _lastOffset - offset ) < 0.1f ) {
		return false;
	}
	...
	const anVec3 &modelOrigin = _object->GetParent()->GetRenderEntity()->origin;
	const anMat3 &modelAxis = _object->GetParent()->GetRenderEntity()->axis;

	anVec3 newOffset = _offset;
	if ( !_reverse ) {
	...
	anVec3 target = modelOrigin + ( newOffset * modelAxis );
	...
	anMat3 hipAxis = _upperLegToHipJoint * ( axis * modelAxis.Transpose() );

	// get the axis for the knee joint
	idIK::GetBoneAxis( kneeOrigin, target, kneeDir, axis );
	anMat3 kneeAxis = _lowerLegToKneeJoint * ( axis * modelAxis.Transpose() );
