...
ARC_INLINE const anTraceModel *anTraceModelCache::GetTraceModel( const int traceModelIndex ) const {
	return &cache[ traceModelIndex ]->trm;
}

ARC_INLINE arcCollisionModel *anTraceModelCache::GetCollisionModel( const int traceModelIndex ) const {
	return cache[ traceModelIndex ]->collisionModel;
}

ARC_INLINE const traceModelWater_t* anTraceModelCache::GetWaterPoints( const int traceModelIndex ) const {
	return cache[ traceModelIndex ]->hasWater ? cache[ traceModelIndex ]->waterPoints : nullptr;
}

ARC_INLINE float anTraceModelCache::GetVolume( const int traceModelIndex ) const {
