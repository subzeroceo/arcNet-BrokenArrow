...
//
//=======================================================
class anPhysics_Spline : public idPhysics_Base {

public:

	CLASS_PROTOTYPE( anPhysics_Spline );

							anPhysics_Spline( void );
	virtual					~anPhysics_Spline( void );
...
protected:
	...
	CLASS_STATES_PROTOTYPE( anPhysics_Spline );
};

extern const anEventDef EV_DoneMoving;


class anPhysics_SimpleSpline : public idPhysics_Base {
public:
	typedef idCurve_CubicBezier<anVec3>	splineType_t;

public:
	CLASS_PROTOTYPE( anPhysics_SimpleSpline );

										anPhysics_SimpleSpline( void );
										~anPhysics_SimpleSpline( void );

	void								SetSpline( int _startTime, const anList< splineType_t >& _spline, const anList< int >& _splineTimes );
	...
	const anVec3 &						GetLinearAcceleration( void ) const { return current.acceleration; }

	virtual const anVec3 &				GetOrigin( int id ) const { return current.worldOrigin; }
	virtual const anMat3 &				GetAxis( int id ) const { return current.worldAxes; }
	...
	virtual void						SetBounds( const anVec3 &mins, const anVec3 &maxs );
...
private:
	...
	int									totalTime;
	int									startTime;
	anList<splineType_t>				spline;
	anList<int>							splineTimes;
};
...
protected:
	virtual anStr			GetTrackInfo( const anSplinePath* track ) const;
...
protected:
	...
	anSplinePath*		FindSplineToTrack( anSplinePath* spline, const anStr& track ) const;
	...
	int					ConvertToTrackNumber( const anStr& trackInfo ) const { return ConvertToTrackNumber(anStr::ToUpper(trackInfo.Right( 1 )[0])); }
