...
typedef struct impactInfo_s {
	...
	arcMat3						invInertiaTensor;	// inverse inertia tensor
	arcVec3						position;			// impact position relative to center of mass
	arcVec3						velocity;			// velocity at the impact position
} impactInfo_t;
...
public:	// common physics interface
	...
	virtual void				SetClipBox( const arcBounds &bounds, float density );
...
// bdube: added center mass
	...
	virtual arcVec3				GetCenterMass ( int id = -1 ) const = 0;
// RAVEN END
	...
	virtual const arcBounds &	GetBounds( int id = -1 ) const = 0;
	virtual const arcBounds &	GetAbsBounds( int id = -1 ) const = 0;
	...
	virtual void				GetImpactInfo( const int id, const arcVec3 &point, impactInfo_t *info ) const = 0;
	virtual void				ApplyImpulse( const int id, const arcVec3 &point, const arcVec3 &impulse ) = 0;
	virtual void				AddForce( const int id, const arcVec3 &point, const arcVec3 &force ) = 0;
...
// RAVEN END
	...
	virtual void				SetOrigin( const arcVec3 &newOrigin, int id = -1 ) = 0;
	virtual void				SetAxis( const arcMat3 &newAxis, int id = -1 ) = 0;
								// translate or rotate the physics object in world space
	virtual void				Translate( const arcVec3 &translation, int id = -1 ) = 0;
	virtual void				Rotate( const idRotation &rotation, int id = -1 ) = 0;
								// get the position and orientation in world space
	virtual const arcVec3 &		GetOrigin( int id = 0 ) const = 0;
	virtual const arcMat3 &		GetAxis( int id = 0 ) const = 0;
								// set linear and angular velocity
	virtual void				SetLinearVelocity( const arcVec3 &newLinearVelocity, int id = 0 ) = 0;
	virtual void				SetAngularVelocity( const arcVec3 &newAngularVelocity, int id = 0 ) = 0;
								// get linear and angular velocity
	virtual const arcVec3 &		GetLinearVelocity( int id = 0 ) const = 0;
	virtual const arcVec3 &		GetAngularVelocity( int id = 0 ) const = 0;
								// gravity
	virtual void				SetGravity( const arcVec3 &newGravity ) = 0;
	virtual const arcVec3 &		GetGravity( void ) const = 0;
	virtual const arcVec3 &		GetGravityNormal( void ) const = 0;
								// get first collision when translating or rotating this physics object
	virtual void				ClipTranslation( trace_t &results, const arcVec3 &translation, const idClipModel *model ) const = 0;
...
// RAVEN BEGIN
// abahr
	virtual const arcVec3		GetContactNormal() const { return vec3_zero; }
	virtual const arcVec3		GetGroundContactNormal() const { return vec3_zero; }
...
// RAVEN END
	...
	virtual const arcVec3 &		GetPushedLinearVelocity( const int id = 0 ) const = 0;
	virtual const arcVec3 &		GetPushedAngularVelocity( const int id = 0 ) const = 0;
