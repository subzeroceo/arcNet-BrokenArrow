...
=====================
*/
void idAI::Event_TestMoveToPosition( const arcVec3 &position ) {
	predictedPath_t path;
...
void idAI::Event_GetAngles( void ) {
	idThread::ReturnVector( arcVec3( 0.0f, move.current_yaw, 0.0f ) );
}
...
=====================
*/
void idAI::Event_FindActorsInBounds( const arcVec3 &mins, const arcVec3 &maxs ) {
	idEntity *	ent;
...
// ddynerman: multiple clip worlds
	numListedEntities = gameLocal.EntitiesTouchingBounds( this, arcBounds( mins, maxs ), CONTENTS_BODY, entityList, MAX_GENTITIES );
// RAVEN END
...
// RAVEN END
	...
	bestDistSquared = arcMath::INFINITY;
	bestEnt = NULL;
	for( ent = actor->enemyList.Next(); ent != NULL; ent = ent->enemyNode.Next() ) {
		...
		if ( distSquared < bestDistSquared ) {
			const arcVec3 &enemyPos = ent->GetPhysics()->GetOrigin();
...
void idAI::Event_EntityInAttackCone( idEntity *ent ) {
	...
	relYaw = arcMath::AngleNormalize180( move.ideal_yaw - yaw );
	if ( arcMath::Fabs( relYaw ) < ( attack_cone * 0.5f ) ) {
...
================
*/
void idAI::Event_CanReachPosition( const arcVec3 &pos ) {
	aasPath_t	path;
	int			toAreaNum;
	int			areaNum;
