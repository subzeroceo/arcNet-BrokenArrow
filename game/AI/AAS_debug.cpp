...
============
*/
void idAASLocal::DrawCone( const arcVec3 &origin, const arcVec3 &dir, float radius, const arcVec4 &color ) const {
	int i;
	arcMat3 axis;
	arcVec3 center, top, p, lastp;
	...
	for ( i = 20; i <= 360; i += 20 ) {
		p = center + arcMath::Sin( DEG2RAD(i) ) * radius * axis[0] + arcMath::Cos( DEG2RAD(i) ) * radius * axis[1];
...
void idAASLocal::DrawEdge( int edgeNum, bool arrow ) const {
	...
	if ( gameLocal.GetLocalPlayer() ) {
		gameRenderWorld->DrawText( va( "%d", edgeNum ), ( file->GetVertex( edge->vertexNum[0] ) + file->GetVertex( edge->vertexNum[1] ) ) * 0.5f + arcVec3(0,0,4), 0.1f, colorRed, gameLocal.GetLocalPlayer()->viewAxis );
	}
}
...
============
*/
const arcBounds &idAASLocal::DefaultSearchBounds( void ) const {
...
============
*/
void idAASLocal::ShowArea( const arcVec3 &origin ) const {
	...
	if ( org != origin ) {
		arcBounds bnds = file->GetSettings().boundingBoxes[ 0 ];
		bnds[ 1 ].z = bnds[ 0 ].z;
...
// RAVEN END
	...
	const			aasFace_t *face;
	arcVec3			sides[MAX_AAS_WALL_EDGES];
...
============
*/
void idAASLocal::ShowAreas( const arcVec3 &origin, bool ShowProblemAreas ) const {
	int				i, areaNum;
	idPlayer		*player;
	...
	const arcBounds	bounds = arcBounds( origin ).Expand( 256.0f );
...
void idAASLocal::ShowProblemEdge( int edgeNum ) const {
	...
	arcVec3			sides[4];
	trace_t			results;
	arcVec3			forward, forwardNormal, left, down, start, end;
	...
	start = (arcVec3)file->GetVertex( edge->vertexNum[0] );
	end = (arcVec3)file->GetVertex( edge->vertexNum[1] );
	forward =  end - start ;
	forward.Normalize();
	...
	sides[0] = -left + arcVec3( 0.0f, 0.0f, file->GetSettings().boundingBoxes[0][0][2] + 1.0f );
	sides[1] = left + arcVec3( 0.0f, 0.0f, file->GetSettings().boundingBoxes[0][0][2] + 1.0f );
	sides[2] = left + arcVec3( 0.0f, 0.0f, file->GetSettings().boundingBoxes[0][1][2] - 1.0f );
	sides[3] = -left + arcVec3( 0.0f, 0.0f, file->GetSettings().boundingBoxes[0][1][2] - 1.0f );
...
============
*/
void idAASLocal::ShowProblemArea( const arcVec3 &origin ) const {
	static int	lastAreaNum;
	int			areaNum;
	arcVec3		org;
...
============
*/
void idAASLocal::ShowWalkPath( const arcVec3 &origin, int goalAreaNum, const arcVec3 &goalOrigin ) const {
	int i, areaNum, curAreaNum, travelTime;
	idReachability *reach;
	arcVec3 org, areaCenter;
	aasPath_t path;
...
============
*/
void idAASLocal::ShowFlyPath( const arcVec3 &origin, int goalAreaNum, const arcVec3 &goalOrigin ) const {
	int i, areaNum, curAreaNum, travelTime;
	idReachability *reach;
	arcVec3 org, areaCenter;
	aasPath_t path;
...
============
*/
void idAASLocal::ShowWallEdges( const arcVec3 &origin ) const {
	...
	areaNum = PointReachableAreaNum( origin, DefaultSearchBounds(), (AREA_REACHABLE_WALK|AREA_REACHABLE_FLY) );
	numEdges = GetWallEdges( areaNum, arcBounds( origin ).Expand( 256.0f ), TFL_WALK, edges, 1024 );
	for ( i = 0; i < numEdges; i++ ) {
...
============
*/
void idAASLocal::ShowHideArea( const arcVec3 &origin, int targetAreaNum ) const {
	...
	obstacles[0].absBounds = arcBounds( target ).Expand( 16 );
	numObstacles = 1;

	DrawCone( target, arcVec3(0,0,1), 16.0f, colorYellow );
	...
	if ( FindNearestGoal( goal, areaNum, origin, target, TFL_WALK|TFL_AIR, 0.0f, 0.0f, obstacles, numObstacles, findHide ) ) {
		...
		DrawCone( goal.origin, arcVec3(0,0,1), 16.0f, colorWhite );
	}
}
...
============
*/
bool idAASLocal::PullPlayer( const arcVec3 &origin, int toAreaNum ) const {
	int areaNum;
	arcVec3 areaCenter, dir, vel;
	idAngles delta;
	aasPath_t path;
	idPlayer *player;
...
============
*/
void idAASLocal::RandomPullPlayer( const arcVec3 &origin ) const {
	int rnd, i, n;
...
============
*/
void idAASLocal::ShowPushIntoArea( const arcVec3 &origin ) const {
	int areaNum;
	arcVec3 target;
...
============
*/
void idAASLocal::Test( const arcVec3 &origin ) {
