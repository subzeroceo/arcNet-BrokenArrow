...
//	savefile->ReadInt( chatterRateIdle );		// Don't save
	...
	arcVec3 gravity = spawnArgs.GetVector( "gravityDir", "0 0 -1" );
...
void idAI::Spawn( void ) {
	...
	physicsObj.SetOrigin( GetPhysics()->GetOrigin() + arcVec3( 0, 0, CM_CLIP_EPSILON ) );
...
void idAI::Think( void ) {
	...
	if ( thinkFlags & TH_THINK ) {
		...
		move.ideal_yaw = arcMath::AngleNormalize180( move.ideal_yaw + deltaViewAngles.yaw );
...
============
*/
void idAI::UpdateFocus ( const arcMat3& orientationAxis ) {
	...
	if ( !allowJointMod || !allowEyeFocus ) {
		...
		if ( leader ) {
			arcVec3 dir2Leader = leader->GetPhysics()->GetOrigin()-GetPhysics()->GetOrigin();
...
=====================
*/
bool idAI::Pain( idEntity *inflictor, idEntity *attacker, int damage, const arcVec3 &dir, int location ) {
...
=====================
*/
void idAI::Killed( idEntity *inflictor, idEntity *attacker, int damage, const arcVec3 &dir, int location ) {
	idAngles			ang;
		...
		if( kv->GetValue() != "" ){
			...
			if (gameLocal.SpawnEntityDef( args, &tEnt )) {
				if ( tEnt && tEnt->GetPhysics()) { //tEnt *should* be valid, but hey...
					...
					tEnt->GetPhysics()->SetAngularVelocity( arcVec3( (gameLocal.random.RandomFloat() * 10.0f) + 20.0f,
...
*/
		...
		if ( oldVisible != IsEnemyVisible ( ) ) {
			...
			if ( !oldVisible && combat.attackSightDelay ) {
				...
				if ( enemy.lastVisibleChangeTime ) {
					...
					delay = arcMath::ClampFloat ( 0.0f, 1.0f, (gameLocal.time - enemy.lastVisibleChangeTime) / AI_SIGHTDELAYSCALE );
				} else {
	...
	if ( combat.aggressiveRange > 0.0f ) {
		...
		combat.aggressiveScale *= 1.0f - arcMath::ClampFloat ( 0.0f, 1.0f, enemy.range / combat.aggressiveRange );
		combat.aggressiveScale += 1.0f;
	}
}
...
=====================
*/
const arcVec3& idAI::LastKnownPosition ( const idEntity *ent ) {
...
void idAI::CreateProjectileClipModel( void ) const {
	if ( projectileClipModel == NULL ) {
		arcBounds projectileBounds( vec3_origin );
...
=====================
*/
void idAI::GetPredictedAimDirOffset ( const arcVec3& source, const arcVec3& target, float projectileSpeed, const arcVec3& targetVelocity, arcVec3& offset ) const {
	float  a;
	float  b;
	float  c;
	float  d;
	...
	) const
{
	arcVec3		targetPos1;
	arcVec3		targetPos2;
	arcVec3		targetLinearVel;
	arcVec3		targetPushedVel;
	arcVec3		delta;
	float		max_height;
	bool		result;
	...
	arcVec3	projectileGravity = idProjectile::GetGravity ( projectileDef );
...
=====================
*/
idProjectile *idAI::CreateProjectile ( const idDict* projectileDict, const arcVec3 &pos, const arcVec3 &dir ) {
	idEntity*	ent;
	const char*	clsname;
...
=====================
*/
bool idAI::Attack ( const char* attackName, jointHandle_t joint, idEntity* target, const arcVec3& pushVelocity ) {
	...
	)
{
	...
	if ( GetEnemy() && GetEnemy() == target && GetEnemy() == gameLocal.GetLocalPlayer() && spawnArgs.GetBool( va("attack_%s_missFirstShot",attackName) ) ) {
		...
		if ( gameLocal.random.RandomFloat() < 0.5f ) {
			...
			arcVec3 targetPos = enemy.lastVisibleEyePosition;
			arcVec3 eFacing;
			arcVec3 left, up;
	...
	if ( attack_cone ) {
		...
		diff = arcMath::AngleDelta( ang.yaw, move.current_yaw );
		if ( diff > attack_cone ) {
	...
	for( i = 0; i < attack_count; i++ ) {
		...
		angle = arcMath::Sin( attack_spread * gameLocal.random.RandomFloat() );
		spin = (float)DEG2RAD( 360.0f ) * gameLocal.random.RandomFloat();
		dir = axis[ 0 ] + axis[ 2 ] * ( angle * arcMath::Sin( spin ) ) - axis[ 1 ] * ( angle * arcMath::Cos( spin ) );
		dir.Normalize();
...
bool idAI::TestMelee( void ) const {
	...
	arcVec3 enemyOrg = enemyEnt->GetPhysics()->GetOrigin();
	arcBounds enemyBounds = enemyEnt->GetPhysics()->GetBounds();
	...
	arcVec3 start = GetEyePosition();
	arcVec3 end = enemyEnt->GetEyePosition();
...
bool idAI::AttackMelee ( const char *attackName, const idDict* meleeDict ) {
	...
	if ( meleeDict->GetString( "fx_impact", NULL ) ) {
		if ( enemyEnt == gameLocal.GetLocalPlayer() ) {
			...
			if ( ePlayer ) {
				arcVec3 dir = ePlayer->firstPersonViewOrigin-GetEyePosition();
				dir.Normalize();
				arcVec3 org = ePlayer->firstPersonViewOrigin + (dir * -((gameLocal.random.RandomFloat()*8.0f)+8.0f) );
				idAngles ang = ePlayer->firstPersonViewAxis.ToAngles() * -1;
				arcMat3 axis = ang.ToMat3();
...
================
*/
void idAI::GetMuzzle( jointHandle_t joint, arcVec3 &muzzle, arcMat3 &axis ) {
	if ( joint == INVALID_JOINT ) {
		...
		{//just pull it back a little anyway?
			arcVec3 xyOfs = muzzle-physicsObj.GetOrigin();
			xyOfs.z = 0;
...
idEntity *idAI::HeardSound( int ignore_team ){
	...
	if ( actor && ( !ignore_team || ( ReactionTo( actor ) & ATTACK_ON_SIGHT ) ) && gameLocal.InPlayerPVS( this ) ) 	{
		arcVec3 pos = actor->GetPhysics()->GetOrigin();
		arcVec3 org = physicsObj.GetOrigin();
...
bool idAI::UpdateAnimationControllers( void ) {
	arcVec3		left;
	arcVec3 		dir;
	arcVec3 		orientationJointPos;
	arcVec3 		localDir;
	idAngles 	newLookAng;
	arcMat3		mat;
	arcMat3		axis;
	arcMat3		orientationJointAxis;
	arcVec3		pos;
	int			i;
	idAngles	jointAng;
	...
	newLookAng.pitch = -arcMath::AngleNormalize180( localDir.ToPitch() );
	...
	if ( lookMin.yaw <= -360.0f && lookMax.yaw >= 360.0f )
	{
		...
		float diff;
		diff = ( arcMath::AngleNormalize360(newLookAng.yaw) - arcMath::AngleNormalize360(f) );
		diff = arcMath::AngleNormalize180( diff );
		lookAng.yaw    += diff * currentHeadFocusRate;
		//lookAng.yaw   += ( newLookAng.yaw - f ) * currentHeadFocusRate;
	}
	else
	{
		lookAng.yaw    = arcMath::AngleNormalize180 ( f - orientationJointYaw );
		lookAng.yaw   += (arcMath::AngleNormalize180 ( newLookAng.yaw - f ) * currentHeadFocusRate);
	}
	lookAng.pitch += ( arcMath::AngleNormalize180( newLookAng.pitch - lookAng.pitch ) * currentHeadFocusRate );
	...
	f = eyeAng.yaw;
	eyeAng.yaw    = arcMath::AngleNormalize180( f - orientationJointYaw );
	eyeAng.yaw   += (arcMath::AngleNormalize180( newLookAng.yaw - f ) * eyeFocusRate);
	eyeAng.pitch += (arcMath::AngleNormalize180( newLookAng.pitch - eyeAng.pitch ) * eyeFocusRate);
	...
	lookAng.yaw = arcMath::AngleNormalize180( lookAng.yaw + orientationJointYaw );
	eyeAng.yaw = arcMath::AngleNormalize180( eyeAng.yaw + orientationJointYaw );
	...
	if ( leftEyeJoint != INVALID_JOINT && rightEyeJoint != INVALID_JOINT ) {
		if ( head ) {
			...
			if ( focusType != AIFOCUS_NONE && allowEyeFocus && canLook ) {
				...
				eyeAng.yaw -= 0.5f;
				eyeAng.pitch += 3.25f;
				arcMat3 eyeAxis = ( eyeAng ).ToMat3() * head->GetPhysics()->GetAxis().Transpose ( );
			...
			if ( allowEyeFocus && focusType != AIFOCUS_NONE && !torsoAnim.GetAnimFlags().ai_no_look && !aifl.disableLook ) {
				arcMat3 eyeAxis = ( eyeAng ).ToMat3() * GetPhysics()->GetAxis().Transpose ( );
...
idProjectile* idAI::AttackProjectile ( const idDict* projectileDict, const arcVec3 &org, const idAngles &ang ) {
	arcVec3				start;
	trace_t				tr;
	arcBounds			projBounds;
	...
	const arcBounds &ownerBounds = physicsObj.GetAbsBounds();
...
bool idAI::CanHitEnemy ( void ) {
	...
	arcVec3 toPos = enemyEnt->GetEyePosition();
	arcVec3 eye = GetEyePosition();
	arcVec3 dir;
...
float HeightForTrajectory( const arcVec3 &start, float zVel, float gravity );

int idAI::TestTrajectory( const arcVec3 &firePos, const arcVec3 &target, const char *projectileName ){
	arcVec3	projVelocity;
	arcVec3	projGravity;
	float	testTime;
	...
	arcMath::SinCos( pitch, s, c );

	delta_x			= arcMath::Sqrt( ( target.x - firePos.x ) * ( target.x - firePos.x ) + ( target.y - firePos.y ) * ( target.y - firePos.y ) );
	delta_z			= target.z - firePos.z;
	newVel			= ( delta_x / arcMath::Cos( pitch ) ) * arcMath::Sqrt( projGravity.z / ( 2.0f * ( delta_x * arcMath::Tan( pitch ) - delta_z ) ) );
	testTime		= delta_x / ( newVel * c );
	zVel			= newVel * s;

	float a = arcMath::ASin ( delta_x * GetPhysics()->GetGravity().Length() / (projVelocity.x * projVelocity.x) );
	a = a / 2;

	float r = (projVelocity.x * projVelocity.x) * arcMath::Sin ( 2 * a ) / GetPhysics()->GetGravity().Length();
	if ( r < delta_x - (delta_x * 0.1) ) {
		...
		mVar.lobDir.z = 0;
		mVar.lobDir.z = arcMath::Tan ( a ) * mVar.lobDir.LengthFast();
...
float idAI::GetTurnDelta( void ){
	...
	if ( move.turnRate ) {
		amount = arcMath::AngleNormalize180( move.ideal_yaw - move.current_yaw );
		return amount;
	} else {
		return 0.0f;
	}
...
============
*/
bool idAI::CanHitEnemyFromAnim( int animNum, arcVec3 offset ) {
	arcVec3		dir;
	arcVec3		local_dir;
	arcVec3		fromPos;
	arcMat3		axis;
	...
	// calculate the world transform of the launch position
  	arcVec3 org = physicsObj.GetOrigin()+offset;
	arcVec3 from;
...
============
*/
void idAI::ReactToShotAt ( idEntity* attacker, const arcVec3 &origOrigin, const arcVec3 &origDir ) {
	...
	float angleDelta = arcMath::AngleDelta ( angles[YAW], move.current_yaw );
