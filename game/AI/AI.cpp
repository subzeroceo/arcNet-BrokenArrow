...
=====================
*/
void anSAAI::Save( anSaveGame *savefile ) const {
	int i;

// cnicholson: These 3 vars are intentionally not saved, as noted in the restore
	// NOSAVE: anLinkList<anSAAI>		simpleThinkNode;
	// NOSAVE: anSEAS*				aas;
	// NOSAVE: anSEASCallback*		aasFind;
	...
	savefile->WriteBool( GetPhysics() == static_cast<const anPhysics *>(&physicsObj) );
	...
	// TOSAVE: mutable anClipModel*	projectileClipModel;
...
=====================
*/
void anSAAI::Restore( anRestoreGame *savefile ) {
	bool		restorePhysics;
	int			i;
	int			num;
	anBounds	bounds;
	...
	// INTENTIONALLY NOT SAVED: anLinkList<anSAAI>	simpleThinkNode;
	// INTENTIONALLY NOT SAVED: anSEAS*				aas;
	// INTENTIONALLY NOT SAVED: anSEASCallback*		aasFind;
	// NOTE That some AAS stuff is done at end of ::Restore
	...
	// TORESTORE: mutable anClipModel*	projectileClipModel;
...
//	savefile->ReadInt( chatterRateIdle );		// Don't save
	...
	pusher.Restore( savefile );
	scriptedActionEnt.Restore ( savefile );
	...
	anVec3 gravity = spawnArgs.GetVector( "gravityDir", "0 0 -1" );
...
/*
=====================
anSAAI::InitNonPersistentSpawnArgs
=====================
*/
void anSAAI::InitNonPersistentSpawnArgs ( void ) {
	aasSensor = anSEASTacticalSensor::CREATE_SENSOR(this);
	aasFind   = nullptr;
...
void anSAAI::Spawn( void ) {
	...
	if ( spawnArgs.GetString( "objectivetitle_failed", nullptr ) && spawnArgs.GetBool( "meleeSuperhero", "1" )   )  {
	...
	if ( spawnArgs.GetBool( "big_monster" ) ) {
		...
		physicsObj.SetClipMask( MASK_MONSTERSOLID & ~CONTENTS_ACTORBODY );
	} else {
		if ( use_combat_bbox ) {
			physicsObj.SetContents( CONTENTS_ACTORBODY|CONTENTS_SOLID );
		} else {
			physicsObj.SetContents( CONTENTS_ACTORBODY );
		}
		physicsObj.SetClipMask( MASK_MONSTERSOLID );
	}

	// move up to make sure the monster is at least an epsilon above the floor
	physicsObj.SetOrigin( GetPhysics()->GetOrigin() + anVec3( 0, 0, CM_CLIP_EPSILON ) );
	...
	// Initialize actions
	actionEvadeLeft.Init		( spawnArgs, "action_evadeLeft",		nullptr,					0 );
	actionEvadeRight.Init		( spawnArgs, "action_evadeRight",		nullptr, 					0 );
	actionRangedAttack.Init		( spawnArgs, "action_rangedAttack",		nullptr, 					AIACTIONF_ATTACK );
	actionMeleeAttack.Init		( spawnArgs, "action_meleeAttack",		nullptr, 					AIACTIONF_ATTACK|AIACTIONF_MELEE );
	actionLeapAttack.Init		( spawnArgs, "action_leapAttack",		nullptr, 					AIACTIONF_ATTACK );
	actionJumpBack.Init			( spawnArgs, "action_jumpBack",			nullptr, 					0 );
...
===================
*/
void anSAAI::List_f( const anCommandArgs &args ) {
	int			e;
	int			i;
	anEntity*	check;
	int			count;
	anDict		countsFixed;
	anDict		countsSpawned;
	...
	for ( e = 0; e < MAX_GENTITIES; e++ ) {
		...
		if ( check->IsType ( anSAAI::Type ) ) {
			anSAAI* checkAI = static_cast<anSAAI*>(check);
			...
			for ( kv = check->spawnArgs.MatchPrefix( "def_spawn", nullptr ); kv; kv = check->spawnArgs.MatchPrefix( "def_spawn", kv ) ) {
	...
	if ( countsFixed.GetNumKeyVals() ) {
		...
		for ( e = 0; e < countsFixed.GetNumKeyVals(); e ++ ) {
			const anKeyValue* kv = countsFixed.GetKeyVal ( e );
...
================
*/
void anSAAI::DormantEnd( void ) {
	if ( enemy.ent && !enemyNode.InList() ) {
		// let our enemy know we're back on the trail
		anActor *enemyEnt = dynamic_cast< anActor *>( enemy.ent.GetEntity() );
		if ( enemyEnt ){
...
/*
=====================
anSAAI::DoDormantTests
=====================
*/
bool anSAAI::DoDormantTests ( void ) {
	...
	if ( leader && leader->IsType ( anBasePlayer::GetClassType() ) ) {
		return false;
	}
...
void anSAAI::Think( void ) {
	...
	if ( thinkFlags & TH_THINK ) {
		...
		move.ideal_yaw = anMath::AngleNormalize180( move.ideal_yaw + deltaViewAngles.yaw );
...
void anSAAI::UpdateFocus ( const anMat3 &orientationAxis ) {
	...
	if ( !allowJointMod || !allowEyeFocus ) {
		...
		anEntity*	lookat   = nullptr;
		aiFocus_t	newfocus = AIFOCUS_NONE;

		if ( leader ) {
			anVec3 dir2Leader = leader->GetPhysics()->GetOrigin()-GetPhysics()->GetOrigin();
	...
	if ( focusType == AIFOCUS_NONE ) {
		...
		if ( focusEnt ) {
			if ( focusEnt->IsType ( anActor::GetClassType() ) ) {
				currentFocusPos = static_cast<anActor*>(focusEnt)->GetEyePosition() - eyeVerticalOffset * focusEnt->GetPhysics()->GetGravityNormal();
			} else {
...
/*
=====================
anSAAI::OnStateChange
=====================
*/
void anSAAI::OnStateChange ( int channel ) {
}
...
=====================
*/
void anSAAI::OverrideFlag ( aiFlagOverride_t flag, bool value ) {
	bool oldValue;
...
=====================
*/
void anSAAI::RestoreFlag ( aiFlagOverride_t flag ) {
	int flagOverride		= 1 << (flag * 2);
	int flagOverrideValue	= 1 << ((flag * 2) + 1);
	bool value;
...
=====================
*/
int anSAAI::ReactionTo( const anEntity *ent ) {
	...
	const anActor *actor = static_cast<const anActor *>( ent );
	if ( actor->IsType( anBasePlayer::GetClassType() ) && static_cast<const anBasePlayer *>(actor)->noclip ) {
...
/*
=====================
anSAAI::AdjustHealthByDamage
=====================
*/
void anSAAI::AdjustHealthByDamage	( int damage ) {
	if ( aifl.undying ) {
		return;
	}
	anActor::AdjustHealthByDamage ( damage );
...
=====================
*/
bool anSAAI::Pain( anEntity *inflictor, anEntity *attacker, int damage, const anVec3 &dir, int location ) {
	aifl.pain   = anActor::Pain( inflictor, attacker, damage, dir, location );
	aifl.damage = true;
...
=====================
*/
void anSAAI::Killed( anEntity *inflictor, anEntity *attacker, int damage, const anVec3 &dir, int location ) {
	anAngles			ang;
	...
	for ( kv = spawnArgs.MatchPrefix ( "deathhidesurface", nullptr );
		  kv;
	...
	if ( attacker && attacker->IsType( anActor::GetClassType() ) ) {
		gameLocal.AlertAI( ( anActor * )attacker );
	...
	kv = spawnArgs.MatchPrefix( "def_drops", nullptr );
	while( kv ) {
		anDict args;
		anEntity *tEnt;
		if ( kv->GetValue() != "" ){
			...
			if (gameLocal.SpawnEntityDef( args, &tEnt ) ) {
				if ( tEnt && tEnt->GetPhysics()) { //tEnt *should* be valid, but hey...
					...
					tEnt->GetPhysics()->SetAngularVelocity( anVec3( (gameLocal.random.RandomFloat() * 10.0f) + 20.0f,
...
=====================
*/
void anSAAI::Activate( anEntity *activator ) {
	anBasePlayer *player;

	// Set our tether?
	if ( activator && activator->IsType ( anSAAITether::GetClassType() ) ) {
		SetTether ( static_cast<anSAAITether*>(activator) );
	}
	...
	aifl.activated = true;
	if ( !activator || !activator->IsType( anBasePlayer::GetClassType() ) ) {
		player = gameLocal.GetLocalPlayer();
	} else {
		player = static_cast<anBasePlayer *>( activator );
	}
...
=====================
*/
void anSAAI::TalkTo( anActor *actor ) {
...
=====================
*/
talkState_t anSAAI::GetTalkState( void ) const {
...
/*
=====================
anSAAI::TouchedByFlashlight
=====================
*/
void anSAAI::TouchedByFlashlight( anActor *flashlight_owner ) {
...
=====================
*/
void anSAAI::ClearEnemy( bool dead ) {
...
/*
=====================
anSAAI::UpdateEnemyPosition
=====================
*/
void anSAAI::UpdateEnemyPosition ( bool forceUpdate ) {
...
*/
		...
		if ( oldVisible != IsEnemyVisible() ) {
			...
			if ( !oldVisible && combat.attackSightDelay ) {
				...
				if ( enemy.lastVisibleChangeTime ) {
					...
					delay = anMath::ClampFloat ( 0.0f, 1.0f, (gameLocal.time - enemy.lastVisibleChangeTime) / AI_SIGHTDELAYSCALE );
				} else {
	...
	if ( combat.aggressiveRange > 0.0f ) {
		...
		combat.aggressiveScale *= 1.0f - anMath::ClampFloat ( 0.0f, 1.0f, enemy.range / combat.aggressiveRange );
		combat.aggressiveScale += 1.0f;
	}
}
...
=====================
*/
void anSAAI::SetSpawner ( rvSpawner* _spawner ) {
	spawner = _spawner;
}
...
=====================
*/
rvSpawner* anSAAI::GetSpawner ( void ) {
	return spawner;
}
...
=====================
*/
bool anSAAI::SetEnemy( anEntity *newEnemy ) {
	anEntity*	oldEnemy;
	...
	if ( !newEnemy ) {
		...
		if ( newEnemy->IsType( anActor::GetClassType() ) ){
			anActor* enemyActor = static_cast<anActor*>( newEnemy );
...
/*
===================
anSAAI::CalculateAttackOffsets

calculate joint positions on attack frames so we can do proper "can hit" tests
===================
*/
void anSAAI::CalculateAttackOffsets ( void ) {
...
/*
=====================
anSAAI::CreateProjectileClipModel
=====================
*/
void anSAAI::CreateProjectileClipModel( void ) const {
	if ( projectileClipModel == nullptr ) {
		anBounds projectileBounds( vec3_origin );
		projectileBounds.ExpandSelf( 2.0f ); // projectileRadius );
		projectileClipModel	= new anClipModel( anTraceModel( projectileBounds ) );
	}
}
...
=====================
*/
bool anSAAI::GetAimDir(
	const anVec3&	firePos,
	const anEntity*	target,
	const anDict*	projectileDef,
	anEntity*		ignore,
	anVec3&			aimDir,
	float			aimOffset,
	float			predict
	) const
{
	anVec3		targetPos1;
	anVec3		targetPos2;
	anVec3		targetLinearVel;
	anVec3		targetPushedVel;
	anVec3		delta;
	float		max_height;
	bool		result;
	anEntity*	targetEnt = const_cast<anEntity*>(target);
	...
	anVec3	projectileGravity = idProjectile::GetGravity ( projectileDef );
	...
	} else if ( targetEnt->IsType( anActor::GetClassType() ) ) {
		targetPos2  = static_cast<anActor*>(targetEnt)->GetEyePosition();
		targetPos1  = static_cast<anActor*>(targetEnt)->GetChestPosition();
	...
	if ( max_height > 0.0f ) {
		...
		if ( result || !targetEnt->IsType( anActor::GetClassType() ) ) {
			return result;
		}
	}
...
/*
=====================
anSAAI::CreateProjectile
=====================
*/
idProjectile *anSAAI::CreateProjectile ( const anDict* projectileDict, const anVec3 &pos, const anVec3 &dir ) {
	anEntity*	ent;
	const char*	clsname;
...
/*
=====================
anSAAI::RemoveProjectile
=====================
*/
void anSAAI::RemoveProjectile( void ) {
	if ( projectile.GetEntity() ) {
...
bool anSAAI::Attack ( const char *attackName, jointHandle_t joint, anEntity *target, const anVec3 &pushVelocity ) {
	...
	return ( AttackRanged ( attackName, attackDict, joint, target, pushVelocity ) != nullptr );
}
...
=====================
*/
idProjectile* anSAAI::AttackRanged (
	const char*		attackName,
	const anDict*	attackDict,
	jointHandle_t	joint,
	anEntity*		target,
	const anVec3&	pushVelocity
	)
{
	...
	if ( GetEnemy() && GetEnemy() == target && GetEnemy() == gameLocal.GetLocalPlayer() && spawnArgs.GetBool( va( "attack_%s_missFirstShot",attackName) ) ) {
		...
		if ( gameLocal.random.RandomFloat() < 0.5f ) {
			...
			anVec3 targetPos = enemy.lastVisibleEyePosition;
			anVec3 eFacing;
			anVec3 left, up;
			...
			if ( GetEnemy()->IsType( anActor::GetClassType() ) )
			{
				eFacing = ((anActor*)GetEnemy())->viewAxis[0];
			}
			else
			{
			...
			if ( spawnArgs.GetBool ( va( "attack_%s_lockToJoint",attackName), "0" ) )
			{
				if ( spawnArgs.GetBool( va( "attack_%s_traceToNextJoint", attackName ), "0" ) )
				{
					...
					if ( endJoint != INVALID_JOINT && endJoint != joint )
					{
						...
						//const_cast<anDict*>(attackDict)->SetFloat( "range", dir.Normalize() );//NOTE: yes, I intentionally normalize here as well as use the result for length...
						dir.Normalize();
					}
				}
			}
	...
	if ( attack_cone ) {
		...
		diff = anMath::AngleDelta( ang.yaw, move.current_yaw );
		if ( diff > attack_cone ) {
	...
	if ( enemy.ent && enemy.ent->IsType ( anSAAI::GetClassType() ) ) {
		static_cast<anSAAI*>(enemy.ent.GetEntity())->ReactToShotAt ( this, muzzleOrigin, axis[0] );
	}
...
/*s
================
anSAAI::DamageFeedback

callback function for when another entity recieved damage from this entity
================
*/
void anSAAI::DamageFeedback( anEntity *victim, anEntity *inflictor, int &damage ) {
...
=====================
*/
void anSAAI::DirectDamage( const char *meleeDefName, anEntity *ent ) {
	const anDict *meleeDef;
	const char *p;
...
//	ent->Damage( this, this, globalKickDir, meleeDefName, 1.0f, INVALID_JOINT );
	...
	ent->Damage( this, this, globalKickDir, meleeDefName, damageScale, nullptr );
}
...
=====================
*/
bool anSAAI::TestMelee( void ) const {
	trace_t		trace;
	anEntity*	enemyEnt = enemy.ent;
	...
	anVec3 enemyOrg = enemyEnt->GetPhysics()->GetOrigin();
	anBounds enemyBounds = enemyEnt->GetPhysics()->GetBounds();
	...
	anVec3 start = GetEyePosition();
	anVec3 end = enemyEnt->GetEyePosition();
...
=====================
*/
bool anSAAI::AttackMelee ( const char *attackName, const anDict* meleeDict ) {
	anEntity*				enemyEnt = enemy.ent;
	const char*				p;
	const idSoundShader*	shader;
	...
	if ( enemyEnt->IsType( anBasePlayer::GetClassType() ) && g_skill.GetInteger() < 2 ) {
		int	damage, armor;
		anBasePlayer *player = static_cast<anBasePlayer*>( enemyEnt );
	...
	anSAAI* enemyAI = static_cast<anSAAI*>(enemyEnt);
	...
	if ( enemyEnt->IsType ( anSAAI::Type ) ) {
		location = static_cast<anSAAI*>(enemyEnt)->chestOffsetJoint;
	}
	enemyEnt->Damage( this, this, globalKickDir, meleeDict->GetString ( "classname" ), damageScale, location );

	if ( meleeDict->GetString( "fx_impact", nullptr ) ) {
		if ( enemyEnt == gameLocal.GetLocalPlayer() ) {
			anBasePlayer *ePlayer = static_cast<anBasePlayer*>(enemyEnt);
			if ( ePlayer ) {
				anVec3 dir = ePlayer->firstPersonViewOrigin-GetEyePosition();
				dir.Normalize();
				anVec3 org = ePlayer->firstPersonViewOrigin + (dir * -((gameLocal.random.RandomFloat()*8.0f)+8.0f) );
				anAngles ang = ePlayer->firstPersonViewAxis.ToAngles() * -1;
				anMat3 axis = ang.ToMat3();
...
================
*/
void anSAAI::PushWithAF( void ) {
	int i, j;
	afTouch_t touchList[ MAX_GENTITIES ];
	anEntity *pushed_ents[ MAX_GENTITIES ];
	anEntity *ent;
	anVec3 vel;
	int num_pushed;
...
void anSAAI::GetMuzzle( jointHandle_t joint, anVec3 &muzzle, anMat3 &axis ) {
	if ( joint == INVALID_JOINT ) {
		...
		{//just pull it back a little anyway?
			anVec3 xyOfs = muzzle-physicsObj.GetOrigin();
			xyOfs.z = 0;
...
/*
================
anSAAI::CanPlayChatterSounds

Used for playing chatter sounds on monsters.
================
*/
bool anSAAI::CanPlayChatterSounds( void ) const {
	if ( aifl.dead ) {
		return false;
	}
...
=====================
*/
anEntity *anSAAI::HeardSound( int ignore_team ){
	// check if we heard any sounds in the last frame
	anActor	*actor = gameLocal.GetAlertActor();
	if ( actor && ( !ignore_team || ( ReactionTo( actor ) & ATTACK_ON_SIGHT ) ) && gameLocal.InPlayerPVS( this ) ) 	{
		anVec3 pos = actor->GetPhysics()->GetOrigin();
		anVec3 org = physicsObj.GetOrigin();
...
void anSAAI::SetLeader ( anEntity *newLeader ) {
	...
	if ( !newLeader ){
		leader = nullptr;
	} else if ( !newLeader->IsType( anActor::GetClassType() ) ) {
		gameLocal.Error( "'%s' is not an anActor (player or ai controlled character)", newLeader->name.c_str() );
	} else {
		leader = static_cast<anActor *>( newLeader );
	}
...
/*
================
anSAAI::UpdateAnimationControllers
================
*/
bool anSAAI::UpdateAnimationControllers( void ) {
	anVec3		left;
	anVec3 		dir;
	anVec3 		orientationJointPos;
	anVec3 		localDir;
	anAngles 	newLookAng;
	anMat3		mat;
	anMat3		axis;
	anMat3		orientationJointAxis;
	anVec3		pos;
	int			i;
	anAngles	jointAng;
	...
	if ( aifl.dead ) {
		return anActor::UpdateAnimationControllers();
	}
	...
	newLookAng.pitch = -anMath::AngleNormalize180( localDir.ToPitch() );
	...
	if ( lookMin.yaw <= -360.0f && lookMax.yaw >= 360.0f )
	{
		...
		float diff;
		diff = ( anMath::AngleNormalize360(newLookAng.yaw) - anMath::AngleNormalize360(f) );
		diff = anMath::AngleNormalize180( diff );
		lookAng.yaw    += diff * currentHeadFocusRate;
		//lookAng.yaw   += ( newLookAng.yaw - f ) * currentHeadFocusRate;
	}
	else
	{
		lookAng.yaw    = anMath::AngleNormalize180 ( f - orientationJointYaw );
		lookAng.yaw   += (anMath::AngleNormalize180 ( newLookAng.yaw - f ) * currentHeadFocusRate);
	}
	lookAng.pitch += ( anMath::AngleNormalize180( newLookAng.pitch - lookAng.pitch ) * currentHeadFocusRate );
	...
	f = eyeAng.yaw;
	eyeAng.yaw    = anMath::AngleNormalize180( f - orientationJointYaw );
	eyeAng.yaw   += (anMath::AngleNormalize180( newLookAng.yaw - f ) * eyeFocusRate);
	eyeAng.pitch += (anMath::AngleNormalize180( newLookAng.pitch - eyeAng.pitch ) * eyeFocusRate);
	...
	if ( canLook ) {
		...
		if ( animator.GetAnimFlags( animator.CurrentAnim( ANIMCHANNEL_TORSO )->AnimNum()).ai_look_head_only ) {
			...
			if ( newLookAng.Compare( lookAng ) ) {
				...
				anAngles curJointAngleMod;
				for ( i = 0; i < lookJoints.Num(); i++ ) {
					if ( lookJoints[i] != neckJoint
						&& lookJoints[i] != headJoint ) {
						//blend back
						curJointMod = animator.FindExistingJointMod( lookJoints[i], nullptr );
						if ( curJointMod )
						{
	...
	lookAng.yaw = anMath::AngleNormalize180( lookAng.yaw + orientationJointYaw );
	eyeAng.yaw = anMath::AngleNormalize180( eyeAng.yaw + orientationJointYaw );
	...
	if ( leftEyeJoint != INVALID_JOINT && rightEyeJoint != INVALID_JOINT ) {
		if ( head ) {
			...
			if ( focusType != AIFOCUS_NONE && allowEyeFocus && canLook ) {
				...
				eyeAng.yaw -= 0.5f;
				eyeAng.pitch += 3.25f;
				anMat3 eyeAxis = ( eyeAng ).ToMat3() * head->GetPhysics()->GetAxis().Transpose();
			...
			if ( allowEyeFocus && focusType != AIFOCUS_NONE && !torsoAnim.GetAnimFlags().ai_no_look && !aifl.disableLook ) {
				anMat3 eyeAxis = ( eyeAng ).ToMat3() * GetPhysics()->GetAxis().Transpose();
...
void anSAAI::OnTouch( anEntity *other, trace_t *trace ) {
	...
	if ( pusher && pusher->IsType ( anBasePlayer::GetClassType() ) && (combat.tacticalMaskAvailable & AITACTICAL_MOVE_PLAYERPUSH_BIT) ) {
		ForceTacticalUpdate();
	}
}

idProjectile* anSAAI::AttackProjectile ( const anDict* projectileDict, const anVec3 &org, const anAngles &ang ) {
	anVec3				start;
	trace_t				tr;
	anBounds			projBounds;
	const anClipModel*	projClip;
	anMat3				axis;
	float				distance;
	idProjectile*		result;
	...
	const anBounds &ownerBounds = physicsObj.GetAbsBounds();
...
void anSAAI::RadiusDamageFromJoint( const char *jointname, const char *damageDefName ) {
	jointHandle_t joint;
	anVec3 org;
	anMat3 axis;
...
/*
=====================
anSAAI::BecomeNonSolid
=====================
*/
void anSAAI::BecomeNonSolid( void ) {
	fl.takedamage = false;
	physicsObj.SetContents( 0 );
	physicsObj.GetClipModel()->Unlink();
}

const char *anSAAI::ChooseAnim( int channel, const char *animname ) {
	int anim;
			...
			if ( groundEnt->IsType ( anMover::GetClassType() ) ) {
				...
				for ( ent = groundEnt->GetNextTeamEntity(); ent != nullptr; ent = next ) {
					...
					if ( ent->GetBindMaster() == groundEnt && ent->IsType ( anSAAITether::GetClassType() ) ) {
						SetTether ( static_cast<anSAAITether*>(ent) );
...
void anSAAI::OnDeath( void ){
	if ( vehicleController.IsDriving() ){
		...
		vehicleController.SetInput( usercmd, anAngles( 0, 0, 0 ) );
...
============
*/
void anSAAI::OnWakeUp ( void ) {
}

/*
============
anSAAI::OnUpdatePlayback
============
*/
void anSAAI::OnUpdatePlayback ( const rvDeclPlaybackData& pbd ) {
	return;
}
...
/*
============
anSAAI::OnTacticalChange
============
*/
void anSAAI::OnTacticalChange ( aiTactical_t oldTactical ) {
...
/*
============
anSAAI::OnFriendlyFire
============
*/
void anSAAI::OnFriendlyFire ( anActor* attacker ) {
	AnnounceFriendlyFire( static_cast<anActor*>(attacker) );
}
...
============
*/
void anSAAI::OnStartMoving ( void ) {
...
============
*/
void anSAAI::OnStopMoving ( aiMoveCommand_t oldMoveCommand ) {
}

/*
============
anSAAI::OnStartAction
============
*/
void anSAAI::OnStartAction ( void ) {
}
...
============
*/
void anSAAI::OnStopAction	( void ) {
}

/*
============
anSAAI::OnEnemyVisiblityChange
============
*/
void anSAAI::OnEnemyVisiblityChange ( bool oldVisible ) {
}
...
/*
============
anSAAI::OnCoverInvalidated
============
*/
void anSAAI::OnCoverInvalidated ( void ) {
...
/*
============
anSAAI::OnCoverNotFacingEnemy
============
*/
void anSAAI::OnCoverNotFacingEnemy ( void ) {
...
/*
============
anSAAI::SkipCurrentDestination

Is the AI's current destination ok enough to stay at?
============
*/
bool anSAAI::SkipCurrentDestination ( void ) const {
...
============
*/
bool anSAAI::SkipImpulse( anEntity *ent, int id ){
	bool skip = anActor::SkipImpulse( ent, id );
...
bool anSAAI::CanHitEnemy ( void ) {
	...
	anVec3 toPos = enemyEnt->GetEyePosition();
	anVec3 eye = GetEyePosition();
	anVec3 dir;
	...
	if ( tr.fraction >= 1.0f || ( hit == enemyEnt ) ) {
		lastHitCheckResult = true;
	} else if ( ( tr.fraction < 1.0f ) && ( hit->IsType( anSAAI::Type ) ) &&
		( static_cast<anSAAI *>( hit )->team != team ) ) {
		lastHitCheckResult = true;
	} else {
		lastHitCheckResult = false;
	}
...
/*
============
anSAAI::CanHitEnemyFromJoint
============
*/

bool anSAAI::CanHitEnemyFromJoint( const char *jointname ){
	trace_t	tr;
	anVec3	muzzle;
	anMat3	axis;
...
float HeightForTrajectory( const anVec3 &start, float zVel, float gravity );

int anSAAI::TestTrajectory( const anVec3 &firePos, const anVec3 &target, const char *projectileName ){
	anVec3	projVelocity;
	anVec3	projGravity;
	float	testTime;
	...
	anMath::SinCos( pitch, s, c );

	delta_x			= anMath::Sqrt( ( target.x - firePos.x ) * ( target.x - firePos.x ) + ( target.y - firePos.y ) * ( target.y - firePos.y ) );
	delta_z			= target.z - firePos.z;
	newVel			= ( delta_x / anMath::Cos( pitch ) ) * anMath::Sqrt( projGravity.z / ( 2.0f * ( delta_x * anMath::Tan( pitch ) - delta_z ) ) );
	testTime		= delta_x / ( newVel * c );
	zVel			= newVel * s;

	float a = anMath::ASin ( delta_x * GetPhysics()->GetGravity().Length() / (projVelocity.x * projVelocity.x) );
	a = a / 2;

	float r = (projVelocity.x * projVelocity.x) * anMath::Sin ( 2 * a ) / GetPhysics()->GetGravity().Length();
	if ( r < delta_x - (delta_x * 0.1) ) {
		...
		mVar.lobDir.z = 0;
		mVar.lobDir.z = anMath::Tan ( a ) * mVar.lobDir.LengthFast();
	...
	if ( anSAAI::TestTrajectory ( firePos, target, zVel, projGravity[2], testTime, height * 2, nullptr,
...
float anSAAI::GetTurnDelta( void ){
	float amount;

	if ( move.turnRate ) {
		amount = anMath::AngleNormalize180( move.ideal_yaw - move.current_yaw );
		return amount;
	} else {
		return 0.0f;
	}
...
============
*/
anEntity *anSAAI::FindEnemy ( bool inFov, bool forceNearest, float maxDistSqr ){
	anActor*	actor;
	anActor*	bestEnemy;
	anActor*	bestEnemyBackup;
	float		bestThreat;
...
/*
============
anSAAI::CheckForEnemy

Look for a suitable enemy
============
*/
bool anSAAI::CheckForEnemy ( bool useFov, bool force ) {
	anEntity *newEnemy;
...
/*
============
anSAAI::CheckForTeammateEnemy
============
*/
bool anSAAI::CheckForTeammateEnemy( void ) {
	anActor*	teammate;
	anEntity*	teammateEnemy;
	...
	if ( teammate->IsType ( anSAAI::Type ) ) {
		anSAAI* teammateAI = static_cast<anSAAI*>(teammate);
...
/*
============
anSAAI::CheckForCloserEnemy
============
*/
bool anSAAI::CheckForCloserEnemy ( void ) {
	anEntity*	newEnemy = nullptr;
	float		maxDistSqr;
	...
	newEnemy = HeardSound( true );
	if ( newEnemy && newEnemy != enemy.ent && newEnemy->IsType( anActor::GetClassType() ) ) {
		//heard someone else!
...
bool anSAAI::CheckForReplaceEnemy ( anEntity *replacement ) {
	...
	anActor* actor;
	actor = dynamic_cast<anActor*>(replacement);
	...
	if ( replacement->IsType( anActor::GetClassType() ) && ( static_cast<anActor*>(replacement))->team == team ) {
		return false;
	}
...
float anSAAI::CalculateEnemyThreat ( anEntity *enemyEnt ) {
	...
	} else if ( enemyEnt->IsType ( anBasePlayer::GetClassType() ) ) {
		threat = 2.0f;
	} else {
		threat = 1.0f;
	}
...
============
*/
void anSAAI::StopSpeaking( bool stopAnims ){
	speakTime = 0;
...
/*
============
anSAAI::CanHitEnemyFromAnim
============
*/
bool anSAAI::CanHitEnemyFromAnim( int animNum, anVec3 offset ) {
	anVec3		dir;
	anVec3		local_dir;
	anVec3		fromPos;
	anMat3		axis;
	...
	// Enemy actor pointer
	enemyEnt = static_cast<anEntity*>(enemy.ent.GetEntity());
	...
	// calculate the world transform of the launch position
  	anVec3 org = physicsObj.GetOrigin()+offset;
	anVec3 from;
...
/*
================
anSAAI::ScriptedBegin
================
*/
bool anSAAI::ScriptedBegin ( bool endWithIdle, bool allowDormant ) {
	if ( aifl.dead ) {
		return false;
	}
...
void anSAAI::ReactToShotAt ( anEntity *attacker, const anVec3 &origOrigin, const anVec3 &origDir ) {
	...
	anAngles angles = diff.ToAngles();
	float angleDelta = anMath::AngleDelta ( angles[YAW], move.current_yaw );
...
============
*/
void anSAAI::UpdateHelper ( void ) {
	anSAAIHelper* oldhelper;
...
============
*/
void anSAAI::SetTether ( anSAAITether* newTether ) {
	...
	} else if ( newTether->IsType ( anSAAITetherClear::GetClassType() ) ) {
		SetTether ( nullptr );
	} else {
...
============
*/
anSAAITether* anSAAI::GetTether ( void ) {
	return tether;
}
...
============
*/
bool anSAAI::IsTethered ( void ) const {
	// Need a tether entity to be tethered
	if ( !tether ) {
		return false;
	}
	// If we have an enemy and that enemy is within our tether then break it if we can
	if ( enemy.ent && enemy.ent->IsType ( anSAAI::GetClassType() ) && tether->CanBreak() ) {
		if ( tether->ValidateDestination ( static_cast<anSAAI*>(enemy.ent.GetEntity()), enemy.lastKnownPosition ) ) {
			return false;
		}
	}
	return true;
...
/*
============
anSAAI::IsWithinTether
============
*/
bool anSAAI::IsWithinTether ( void ) const {
	if ( !IsTethered() ) {
		return false;
	}
	if ( !tether->ValidateDestination ( (anSAAI*)this, physicsObj.GetOrigin() ) ) {
		return false;
	}
	return true;
}
...
=====================
*/
void anSAAI::SetTalkState ( talkState_t state ) {
...
/*
===================
anSAAI::IsMeleeNeeded
===================
*/
bool anSAAI::IsMeleeNeeded( void )	{

	if ( enemy.ent && enemy.ent->IsType ( anSAAI::Type ) )	{

		anSAAI* enemyAI = static_cast<anSAAI*>(enemy.ent.GetEntity());
...
===================
*/
bool anSAAI::IsCrouching( void ) const {
	return move.fl.crouching;
}

bool anSAAI::CheckDeathCausesMissionFailure( void )
{
