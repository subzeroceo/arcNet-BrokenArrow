...
============
*/
int idAASLocal::PointAreaNum( const arcVec3 &origin ) const {
	if ( !file ) {
		return 0;
	}
...
============
*/
int idAASLocal::PointReachableAreaNum( const arcVec3 &origin, const arcBounds &searchBounds, const int areaFlags ) const {
	if ( !file ) {
		return 0;
	}
...
============
*/
int idAASLocal::BoundsReachableAreaNum( const arcBounds &bounds, const int areaFlags ) const {
	if ( !file ) {
		return 0;
	}
...
============
*/
void idAASLocal::PushPointIntoAreaNum( int areaNum, arcVec3 &origin ) const {
	if ( !file ) {
		return;
	}
...
============
*/
arcVec3 idAASLocal::AreaCenter( int areaNum ) const {
	if ( !file ) {
		return vec3_origin;
	}
...
============
*/
arcBounds & idAASLocal::AreaBounds( int areaNum ) const {
...
============
*/
bool idAASLocal::Trace( aasTrace_t &trace, const arcVec3 &start, const arcVec3 &end ) const {
	if ( !file ) {
		trace.fraction = 0.0f;
		trace.lastAreaNum = 0;
		trace.numAreas = 0;
...
============
*/
void idAASLocal::GetEdge( int edgeNum, arcVec3 &start, arcVec3 &end ) const {
	if ( !file ) {
		start.Zero();
		end.Zero();
		return;
...
============
*/
idAASCallback::testResult_t idAASCallback::Test ( class idAAS *aas, int areaNum, const arcVec3& origin, float minDistance, float maxDistance, const arcVec3* point, aasGoal_t& goal ) {
	// Get AAS file
	...
	if ( ai_debugTactical.GetInteger ( ) > 1 ) {
		gameRenderWorld->DebugLine ( colorYellow, area.center, area.center + arcVec3(0,0,80.0f), 10000 );
	}
	...
	if ( ai_debugTactical.GetInteger ( ) > 1 && point ) {
		gameRenderWorld->DebugLine ( colorMagenta, *point, *point + arcVec3(0,0,64.0f), 10000 );
	}
	...
	if ( ai_debugTactical.GetInteger ( ) > 1 ) {
		gameRenderWorld->DebugLine ( colorCyan, area.center, area.center + arcVec3(0,0,64.0f), 10000 );
	}
	...
	for ( f = 0; f < area.numFaces; f ++ ) {
		...
		for ( e = 0; e < face.numEdges; e ++ ) {
			arcVec3 edgeCenter = file->EdgeCenter ( abs( file->GetEdgeIndex( face.firstEdge + e ) ) );
			arcVec3 dir        = area.center - edgeCenter;
			float  dist;
			for ( dist = dir.Normalize() - 64.0f; dist > 0.0f; dist -= 64.0f ) {
				arcVec3 testPoint = edgeCenter + dir * dist;
				if ( ai_debugTactical.GetInteger ( ) > 1 ) {
					gameRenderWorld->DebugLine ( colorPurple, testPoint, testPoint + arcVec3(0,0,64.0f), 10000 );
				}
...
============
*/
bool idAASCallback::TestPointDistance ( const arcVec3& origin, const arcVec3& point, float minDistance, float maxDistance ) {
...
============
*/
bool idAASCallback::TestPoint ( class idAAS *aas, const arcVec3& pos, const float zAllow ) {
	return true;
}
