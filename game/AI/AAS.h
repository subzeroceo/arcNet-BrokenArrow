...
typedef struct aasPath_s {
	int							type;			// path type
	arcVec3						moveGoal;		// point the AI should move towards
	int							moveAreaNum;	// number of the area the AI should move towards
	arcVec3						secondaryGoal;	// secondary move goal for complex navigation
...
typedef struct aasGoal_s {
	...
	arcVec3						origin;			// position of goal
} aasGoal_t;


typedef struct aasObstacle_s {
	arcBounds					absBounds;		// absolute bounds of obstacle
	arcBounds					expAbsBounds;	// expanded absolute bounds of obstacle
} aasObstacle_t;
...
public:
	...
	testResult_t				Test		( class idAAS *aas, int areaNum, const arcVec3& origin, float minDistance, float maxDistance, const arcVec3* point, aasGoal_t& goal );

protected:

	virtual bool				TestArea	( class idAAS *aas, int areaNum, const aasArea_t& area );
	virtual	bool				TestPoint	( class idAAS *aas, const arcVec3& pos, const float zAllow=0.0f );

private:

	bool		TestPointDistance		( const arcVec3& origin, const arcVec3& point, float minDistance, float maxDistance );
};
...
// RAVEN END
	...
	virtual void				Test( const arcVec3 &origin ) = 0;
	...
	virtual int					PointReachableAreaNum( const arcVec3 &origin, const arcBounds &bounds, const int areaFlags ) const = 0;
								// Returns the number of the first reachable area in or touching the bounds.
	virtual int					BoundsReachableAreaNum( const arcBounds &bounds, const int areaFlags ) const = 0;
								// Push the point into the area.
	virtual void				PushPointIntoAreaNum( int areaNum, arcVec3 &origin ) const = 0;
...
// RAVEN END
	...
	virtual const idPlane &		GetPlane( int planeNum ) const = 0;
								// Get wall edges.
	virtual int					GetWallEdges( int areaNum, const arcBounds &bounds, int travelFlags, int *edges, int maxEdges ) const = 0;
	...
	virtual bool				SetAreaState( const arcBounds &bounds, const int areaContents, bool disabled ) = 0;
								// Add an obstacle to the routing system.
	virtual aasHandle_t			AddObstacle( const arcBounds &bounds ) = 0;
	...
	virtual int					TravelTimeToGoalArea( int areaNum, const arcVec3 &origin, int goalAreaNum, int travelFlags ) const = 0;
								// Get the travel time and first reachability to be used towards the goal, returns true if there is a path.
	virtual bool				RouteToGoalArea( int areaNum, const arcVec3 origin, int goalAreaNum, int travelFlags, int &travelTime, idReachability **reach ) const = 0;
								// Creates a walk path towards the goal.
	virtual bool				WalkPathToGoal( aasPath_t &path, int areaNum, const arcVec3 &origin, int goalAreaNum, const arcVec3 &goalOrigin, int travelFlags ) const = 0;
								// Returns true if one can walk along a straight line from the origin to the goal origin.
	virtual bool				WalkPathValid( int areaNum, const arcVec3 &origin, int goalAreaNum, const arcVec3 &goalOrigin, int travelFlags, arcVec3 &endPos, int &endAreaNum ) const = 0;
								// Creates a fly path towards the goal.
	virtual bool				FlyPathToGoal( aasPath_t &path, int areaNum, const arcVec3 &origin, int goalAreaNum, const arcVec3 &goalOrigin, int travelFlags ) const = 0;
								// Returns true if one can fly along a straight line from the origin to the goal origin.
	virtual bool				FlyPathValid( int areaNum, const arcVec3 &origin, int goalAreaNum, const arcVec3 &goalOrigin, int travelFlags, arcVec3 &endPos, int &endAreaNum ) const = 0;
								// Show the walk path from the origin towards the area.
	virtual void				ShowWalkPath( const arcVec3 &origin, int goalAreaNum, const arcVec3 &goalOrigin ) const = 0;
								// Show the fly path from the origin towards the area.
	virtual void				ShowFlyPath( const arcVec3 &origin, int goalAreaNum, const arcVec3 &goalOrigin ) const = 0;
								// Find the nearest goal which satisfies the callback.
	virtual bool				FindNearestGoal( aasGoal_t &goal, int areaNum, const arcVec3 origin, const arcVec3 &target, int travelFlags, float minDistance, float maxDistance, aasObstacle_t *obstacles, int numObstacles, idAASCallback &callback ) const = 0;
...
// rjohnson: added more debug drawing
	virtual void				ShowAreas( const arcVec3 &origin, bool ShowProblemAreas = false ) const = 0;
