...
=====================
*/
void anSAAI::GetDebugInfo ( debugInfoProc_t proc, void* userData ) {
	// Base class first
	anActor::GetDebugInfo ( proc, userData );

	proc ( "anSAAI", "aifl.damage",			aifl.damage ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.undying",			aifl.undying ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.pain",				aifl.pain ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.dead",				aifl.dead ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.activated",		aifl.activated ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.jump",				aifl.jump ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.hitEnemy",			aifl.hitEnemy ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.pushed",			aifl.pushed ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.lookAtPlayer",		aifl.lookAtPlayer ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.disableAttacks",	aifl.disableAttacks ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.simpleThink",		aifl.simpleThink ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.action",			aifl.action ? "true" : "false", userData );
	proc ( "anSAAI", "aifl.scripted",			aifl.scripted? "true" : "false", userData );

	proc ( "anSAAI", "leader",				leader.GetEntity() ? leader.GetEntity()->GetName() : "<none>", userData );
	proc ( "anSAAI", "move.followRangeMin",	va( "%g",move.followRange[0]), userData );
	proc ( "anSAAI", "move.followRangeMax",	va( "%g",move.followRange[1]), userData );
	proc ( "anSAAI", "combat.fl.aware",			combat.fl.aware ? "true" : "false", userData );
	proc ( "anSAAI", "combat.fl.ignoreEnemies",	combat.fl.ignoreEnemies ? "true" : "false", userData );

	proc ( "anSAAI", "enemy",					enemy.ent ? enemy.ent->GetName() : "<none>", userData );
	proc ( "anSAAI", "enemy.fl.inFov",		enemy.fl.inFov ? "true" : "false", userData );
	proc ( "anSAAI", "enemy.fl.visible",		enemy.fl.visible ? "true" : "false", userData );
	proc ( "anSAAI", "combat.fl.seenEnemyDirectly",combat.fl.seenEnemyDirectly ? "true" : "false", userData );
	proc ( "anSAAI", "enemy.lastVisibleTime",	va( "%d",enemy.lastVisibleTime), userData );
	proc ( "anSAAI", "combat.maxLostVisTime",	va( "%d",combat.maxLostVisTime), userData );
	proc ( "anSAAI", "enemy.range",			va( "%g",enemy.range), userData );
	proc ( "anSAAI", "enemy.ranged2d",		va( "%g",enemy.range2d), userData );
	proc ( "anSAAI", "lastAttackTime",		va( "%d",lastAttackTime), userData );

	proc ( "anSAAI", "move.fl.done",				move.fl.done ? "true" : "false", userData );
	proc ( "anSAAI", "move.fl.disabled",			move.fl.disabled ? "true" : "false", userData );
	proc ( "anSAAI", "move.fl.onGround",			move.fl.onGround ? "true" : "false", userData );
	proc ( "anSAAI", "move.fl.blocked",			move.fl.blocked ? "true" : "false", userData );
	proc ( "anSAAI", "move.fl.obstacleInPath",	move.fl.obstacleInPath ? "true" : "false", userData );
	proc ( "anSAAI", "move.fl.goalUnreachable",	move.fl.goalUnreachable ? "true" : "false", userData );
	proc ( "anSAAI", "move.fl.moving",			move.fl.moving ? "true" : "false", userData );
	proc ( "anSAAI", "move.command",				aiMoveCommandString[move.moveCommand], userData );
	proc ( "anSAAI", "move.status",				aiMoveStatusString[move.moveStatus], userData );
	proc ( "anSAAI", "move.fl.allowDirectional",	move.fl.allowDirectional ? "true" : "false", userData );
	proc ( "anSAAI", "move.direction_ideal",		aiMoveDirectionString[move.idealDirection ], userData );
	proc ( "anSAAI", "move.direction_current",	aiMoveDirectionString[move.currentDirection ], userData );
	proc ( "anSAAI", "move.yaw_ideal",			va( "%d",(int)move.ideal_yaw), userData );
	proc ( "anSAAI", "move.yaw_current",			va( "%d",(int)move.current_yaw), userData );
	proc ( "anSAAI", "move.fly_roll",				va( "%g", move.fly_roll ), userData );
	proc ( "anSAAI", "move.fly_pitch",			va( "%g", move.fly_pitch ), userData );

	proc ( "anSAAI", "tether",					tether!=nullptr?tether->GetName():"<none>", userData );
	proc ( "anSAAI", "IsTethered()",				IsTethered()?"true":"false", userData );

	proc ( "anSAAI", "lookTarget",				lookTarget.GetEntity() ? lookTarget.GetEntity()->GetName() : "<none>", userData );
	proc ( "anSAAI", "talkTarget",				talkTarget.GetEntity() ? talkTarget.GetEntity()->GetName() : "<none>", userData );
	proc ( "anSAAI", "focusType",					aiFocusString[focusType], userData );
	proc ( "anSAAI", "look.yaw",					va( "%g", lookAng.yaw ), userData );
	proc ( "anSAAI", "look.pitch",				va( "%g", lookAng.pitch ), userData );

	proc ( "anSAAI", "combat.attackRangeMin",		va( "%g",combat.attackRange[0]), userData );
	proc ( "anSAAI", "combat.attackRangeMax",		va( "%g",combat.attackRange[1]), userData );
	proc ( "anSAAI", "combat.tacticalCurrent",	aiTacticalString[combat.tacticalCurrent], userData );

	proc ( "anSAAI", "action_rangedAttack",		aiActionStatusString[actionRangedAttack.status], userData );
	proc ( "anSAAI", "action_meleeAttack",		aiActionStatusString[actionMeleeAttack.status], userData );
	proc ( "anSAAI", "action_leapAttack",			aiActionStatusString[actionLeapAttack.status], userData );
	proc ( "anSAAI", "action_jumpBack",			aiActionStatusString[actionJumpBack.status], userData );
	proc ( "anSAAI", "action_evadeLeft",			aiActionStatusString[actionEvadeLeft.status], userData );
	proc ( "anSAAI", "action_evadeRight",			aiActionStatusString[actionEvadeRight.status], userData );

	proc ( "anSAAI", "npc_name",					spawnArgs.FindKey( "npc_name" )?common->GetLocalizedString( spawnArgs.GetString( "npc_name", "<none>" ) ):"<none>", userData );
}
...
=====================
*/
void anSAAI::DrawRoute( void ) const {
...
void anSAAI::DrawTactical ( void ) {
	...
	if ( enemy.ent ) {
		...
		anVec3 enemyEyePosition;
		anVec3 offset;
		if ( enemy.ent->IsType ( anActor::GetClassType() ) ){
			enemyEyePosition = static_cast<anActor*>(enemy.ent.GetEntity())->GetEyePosition();
		} else {
			enemyEyePosition = enemy.ent->GetPhysics()->GetOrigin();
		}

		offset = anVec3(0,0,team*2.0f);
