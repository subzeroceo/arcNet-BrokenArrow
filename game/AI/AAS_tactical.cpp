...
// aasFeature_s::Normal
///////////////////////////////////////////////////////////////////////////////
arcVec3&	aasFeature_s::Normal()
{
	static arcVec3 n(0,0,0);
...
///////////////////////////////////////////////////////////////////////////////
int aasFeature_s::GetLookPos( arcVec3& lookPos,  const arcVec3& aimAtOrigin, const float leanDistance )
{
	static arcVec3		up(0.0f,0.0f,1.0f);
	static arcVec3		direction;
	static arcVec3		right;
...
struct rvTest
{
	...
	void			DrawDebugInfo(const arcVec4 color, const arcVec3& origin, const arcVec3& direction);
	void			DrawDebugInfo(const arcVec4 color, const arcVec3& origin);
};
...
struct rvTestSet
{
	...
	arcVec3					mOrigin;			// Position Of The Test Subject
	arcVec3					mFacing;			// Orientation Of The Test Subject
	...
	arcVec3					mDirection;			// Resulting Direction To Feature
...
//		mDistance.mMin = 0.0f;	// special case (default would be -1.0f because all others are dot products)
	...
	///////////////////////////////////////////////////////////////////////////
	void			SetupOriginAndFacing(const idEntity* ent, const arcVec3* originOverride=0, const arcVec3* facingOverride=0)
	{
		if (!ent)
		{
	...
	void			DrawDebugInfo(const arcVec4& color, const arcVec3& nonProjectedOrigin);
};
...
struct rvAASTacticalSensorLocal : rvAASTacticalSensor
{
	...
	arcVec3					mAssignmentDirection;	// Used By Assignment Test
	...
	arcVec3					mReservedOrigin;		// Origin of feature that is currently reserved
	...
	void			SearchRadius(const arcVec3& origin=vec3_origin, float rangeMin=0.0f, float rangeMax=1.0f);
	...
	const arcVec3&	ReservedOrigin() const	{return mReservedOrigin;}
};
...
void	rvAASTacticalSensorLocal::Update()
{
	...
	arcVec3			velocityFwd			= mOwner->GetPhysics()->GetLinearVelocity();
	const arcVec3&	ownerOrigin			= mOwner->GetPhysics()->GetOrigin();
	...
	arcVec3			velocityLeft;
	arcVec3			velocityDown;
	arcVec3			featureDirection;
	...
	if (velocityFwd.LengthSqr()>1.0f)
	{
		...
		if (mLook)
		{
			const arcVec3& featureOrigin = mLook->Origin();
			const arcVec3& featureNormal = mLook->Normal();
		...
		{
			const arcVec3& featureOrigin = mNear->Origin();
			const arcVec3& featureNormal = mNear->Normal();
...
///////////////////////////////////////////////////////////////////////////////
void rvAASTacticalSensorLocal::SearchRadius(const arcVec3& origin, float rangeMin, float rangeMax)
{
...
{
	static arcVec3 Height;
	static arcVec3 Orig;
	static arcVec3 Norm;
	static arcVec3 Text;
	static arcVec4 color;
	static arcVec3 Left;
	...
	Left		=  Normal().Cross(arcVec3(0,0,-1)) * mDebugRadius;
...
///////////////////////////////////////////////////////////////////////////
void			rvTest::DrawDebugInfo(const arcVec4 color, const arcVec3& origin, const arcVec3& direction)
{
...
// rvTest::DrawDebugInfo
///////////////////////////////////////////////////////////////////////////
void			rvTest::DrawDebugInfo(const arcVec4 color, const arcVec3& origin)
{
	static	arcVec3	up(0.0f,0.0f,-1.0f);
 	if (mMax<1.0f)
	{
...
///////////////////////////////////////////////////////////////////////////
void			rvTestSet::DrawDebugInfo(const arcVec4& color, const arcVec3& nonProjectedOrigin)
{
	static	int		lifetime = 0;
	static	arcVec3	origin;
...
void rvAASTacticalSensorLocal::DrawDebugInfo()
{
	...
	{
		idAASFile* file = aas->GetFile();
		const arcVec3& playerOrigin = gameLocal.GetLocalPlayer()->GetPhysics()->GetOrigin();
	...
	{
		arcVec3 n = mLook->Normal();
		n *= 64.0f;
		gameRenderWorld->DebugArrow(colorYellow, mOwner->GetPhysics()->GetOrigin() + arcVec3(0,0,32), mOwner->GetPhysics()->GetOrigin() + arcVec3(0,0,32) + n, 8);
	}
}
