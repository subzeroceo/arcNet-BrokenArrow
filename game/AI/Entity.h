...
public:
	...
	void					UpdatePVSAreas( const arcVec3 &pos );
...
// abahr:
	...
	virtual void			GetPosition( arcVec3& origin, arcMat3& axis ) const;
...
// RAVEN END
	...
	virtual void			SetColor( const arcVec3 &color );
...
// bdube: added effect functions
	// effects
	rvClientEffect*			PlayEffect		( const char* effectName, jointHandle_t joint, const arcVec3& originOffset, const arcMat3& axisOffset, bool loop = false, const arcVec3& endOrigin = vec3_origin, bool broadcast = false, effectCategory_t category = EC_IGNORE, const arcVec4& effectTint = vec4_one );
	rvClientEffect*			PlayEffect		( const idDecl *effect, jointHandle_t joint, const arcVec3& originOffset, const arcMat3& axisOffset, bool loop = false, const arcVec3& endOrigin = vec3_origin, bool broadcast = false, effectCategory_t category = EC_IGNORE, const arcVec4& effectTint = vec4_one );
	rvClientEffect*			PlayEffect		( const char* effectName, jointHandle_t joint, bool loop = false, const arcVec3& endOrigin = vec3_origin, bool broadcast = false, effectCategory_t category = EC_IGNORE, const arcVec4& effectTint = vec4_one );

	rvClientEffect*			PlayEffect		( const char* effectName, const arcVec3& origin, const arcMat3& axis, bool loop = false, const arcVec3& endOrigin = vec3_origin, bool broadcast = false, effectCategory_t category = EC_IGNORE, const arcVec4& effectTint = vec4_one );
	rvClientEffect*			PlayEffect		( const idDecl *effect, const arcVec3& origin, const arcMat3& axis, bool loop = false, const arcVec3& endOrigin = vec3_origin, bool broadcast = false, effectCategory_t category = EC_IGNORE, const arcVec4& effectTint = vec4_one );
	...
	float					DistanceTo		( const arcVec3& pos ) const;
	float					DistanceTo2d	( idEntity* ent );
	float					DistanceTo2d	( const arcVec3& pos ) const;
...
// abahr: added virtual
	virtual
	void					ConvertLocalToWorldTransform( arcVec3 &offset, arcMat3 &axis );
// RAVEN END
	arcVec3					GetLocalVector( const arcVec3 &vec ) const;
	arcVec3					GetLocalCoordinates( const arcVec3 &vec ) const;
	arcVec3					GetWorldVector( const arcVec3 &vec ) const;
	arcVec3					GetWorldCoordinates( const arcVec3 &vec ) const;

	virtual	bool			GetMasterPosition( arcVec3 &masterOrigin, arcMat3 &masterAxis ) const;
	void					GetWorldVelocities( arcVec3 &linearVelocity, arcVec3 &angularVelocity ) const;
	...
	void					SetOrigin( const arcVec3 &org );
	...
	bool					GetFloorPos( float max_dist, arcVec3 &floorpos ) const;
	...
	virtual bool			Collide( const trace_t &collision, const arcVec3 &velocity, bool &hitTeleporter ) { hitTeleporter = false; return Collide(collision, velocity); }
							// retrieves impact information, 'ent' is the entity retrieving the info
	virtual void			GetImpactInfo( idEntity *ent, int id, const arcVec3 &point, impactInfo_t *info );
...
// rjohnson: moved entity info out of idGameLocal into its own function
	virtual void			DrawDebugEntityInfo( arcBounds *viewBounds = 0, arcBounds *viewTextBounds = 0, arcVec4 *overrideColor = 0 );
// nmckenzie: Adding ability for non-Actors to be enemies for AI characters.  Rename this function at some point, most likely.
	virtual arcVec3			GetEyePosition( void ) const { return GetPhysics()->GetOrigin(); }
// abahr:
	virtual bool			SkipImpulse( idEntity *ent, int id );
	virtual void			ApplyImpulse( idEntity* ent, int id, const arcVec3& point, const arcVec3& dir, const idDict* damageDef );
// RAVEN END
...
// RAVEN BEGIN
	...
	virtual bool			CanDamage( const arcVec3 &origin, arcVec3 &damagePoint, idEntity* ignoreEnt = NULL ) const;
// RAVEN END
...
// RAVEN END
	...
	// misc
	virtual void			Teleport( const arcVec3 &origin, const idAngles &angles, idEntity *destination );
...
// RAVEN END
	...
	void					InitDefaultPhysics( const arcVec3 &origin, const arcMat3 &axis );
...
// RAVEN END
	...
	void					Event_SetWorldOrigin( arcVec3 const &org );
	void					Event_GetOrigin( void );
	void					Event_SetOrigin( const arcVec3 &org );
	void					Event_GetAngles( void );
	void					Event_SetAngles( const idAngles &ang );
	void					Event_SetLinearVelocity( const arcVec3 &velocity );
	void					Event_GetLinearVelocity( void );
	void					Event_SetAngularVelocity( const arcVec3 &velocity );
	void					Event_GetAngularVelocity( void );
	void					Event_SetSize( const arcVec3 &mins, const arcVec3 &maxs );
	...
	void					Event_DistanceToPoint( const arcVec3 &point );
...
// nmckenzie: get bind master
	...
	void					Event_ApplyImpulse				( idEntity *source, const arcVec3 &point, const arcVec3 &impulse );
...
// RAVEN BEGIN
// bdube: added inlines
ARC_INLINE rvClientEffect* idEntity::PlayEffect( const char* effectName, const arcVec3& origin, const arcMat3& axis, bool loop, const arcVec3& endOrigin,
												 bool broadcast, effectCategory_t category, const arcVec4& effectTint ) {
	return PlayEffect( gameLocal.GetEffect( spawnArgs, effectName ), origin, axis, loop, endOrigin, broadcast, category, effectTint );
}

ARC_INLINE rvClientEffect* idEntity::PlayEffect( const char* effectName, jointHandle_t jointHandle, bool loop, const arcVec3& endOrigin,
												bool broadcast, effectCategory_t category, const arcVec4& effectTint ) {
...
ARC_INLINE rvClientEffect* idEntity::PlayEffect( const char* effectName, jointHandle_t jointHandle, const arcVec3& originOffset, const arcMat3& axisOffset, bool loop, const arcVec3& endOrigin,
												bool broadcast, effectCategory_t category, const arcVec4& effectTint ) {
...
ARC_INLINE idPhysics *idEntity::GetPhysics( void ) const {
	return physics;
}

ARC_INLINE renderEntity_t *idEntity::GetRenderEntity( void ) {
	return &renderEntity;
}

ARC_INLINE int idEntity::GetModelDefHandle( void ) {
	return modelDefHandle;
}

// scork: accessors so sound editor can indicate current-highlit ent
ARC_INLINE int idEntity::GetRefSoundShaderFlags( void ) const
{
	return refSound.parms.soundShaderFlags;
}

ARC_INLINE void idEntity::SetRefSoundShaderFlags( int iFlags )
{
...
public:
	...
	bool					GetJointWorldTransform( jointHandle_t jointHandle, int currentTime, arcVec3 &offset, arcMat3 &axis );
	bool					GetJointTransformForAnim( jointHandle_t jointHandle, int animNum, int currentTime, arcVec3 &offset, arcMat3 &axis ) const;

	virtual int				GetDefaultSurfaceType( void ) const;
	virtual void			AddDamageEffect( const trace_t &collision, const arcVec3 &velocity, const char *damageDefName, idEntity* inflictor );
	virtual void			ProjectHeadOverlay( const arcVec3 &point, const arcVec3 &dir, float size, const char *decal ) {}
...
private:
	...
	void 					Event_SetJointPos( jointHandle_t jointnum, jointModTransform_t transform_type, const arcVec3 &pos );
...
ARC_INLINE float idEntity::DistanceTo ( idEntity* ent ) {
	return DistanceTo ( ent->GetPhysics()->GetOrigin() );
}

ARC_INLINE float idEntity::DistanceTo ( const arcVec3& pos ) const {
	return (pos - GetPhysics()->GetOrigin()).LengthFast ( );
}

ARC_INLINE float idEntity::DistanceTo2d ( idEntity* ent ) {
	return DistanceTo2d ( ent->GetPhysics()->GetOrigin() );
}

ARC_INLINE bool idEntity::CanTakeDamage ( void ) const {
	return fl.takedamage;
}

// RAVEN BEGIN
// ddynerman: MP arena stuff
ARC_INLINE int idEntity::GetInstance( void ) const {
	return instance;
}

// ddynerman: multiple collision worlds
ARC_INLINE int idEntity::GetClipWorld( void ) const {
	return clipWorld;
}

ARC_INLINE void idEntity::SetClipWorld( int newCW ) {
	clipWorld = newCW;
	if( GetPhysics() ) {
