...
class idAnimatedEntity : public abEntity {
public:
	...
	bool					GetJointTransformForAnim( jointHandle_t jointHandle, int animNum, int frameTime, anVec3 &offset, anMat3 &axis ) const;
	bool					GetJointTransformForAnim( jointHandle_t jointHandle, int animNum, int frameTime, anVec3 &offset ) const;
	bool					GetJointTransformForAnim( jointHandle_t jointHandle, int animNum, int frameTime, anMat3 &axis ) const;
	...
	virtual const anBounds*	GetSelectionBounds( void ) const;
...
protected:
	anAnimator				animator;
	anClipModel*			combatModel;
	anBounds				selectionBounds;
	int						lastServiceTime;
...
private:
	...
	void					Event_JointToWorldTransform( jointHandle_t jointnum, const anVec3 &vector );
	...
	void					Event_WorldToModelSpace( const anVec3 &vector );
