...
int WakeLayer::AddToBack( WakeNode &node ) {
...
void WakeLayer::PopFront() {
	firstNode = ( firstNode+1 )%MAX_NODES;
	numNodes--;
}

int WakeLayer::RemapIndex( int index ) {
	return ( firstNode+index )%MAX_NODES;
}

WakeNode &WakeLayer::GetNode( int index ) {
	return nodes[ RemapIndex( index ) ];
}

WakeLayer::WakeLayer( void ) {
	numNodes = 0;
	firstNode = 0;
...
void Wake::Init(  const WakeParms &params, int ticket ) {
	minPoint = vec3_origin;
	maxPoint = vec3_origin;
	nextNodeTime = 0.0f;
	numPoints = 0;
	...
	// Allocate surfaces
	if ( !NumSurfaces() ) {
		AddSurfaceDB( params.centerMat, ( WakeLayer::MAX_NODES * 2 + 2), ( WakeLayer::MAX_NODES * 6) );
		GetTriSurf( 0, 0 )->numVerts = GetTriSurf( 0, 0 )->numAllocedVerts;
		GetTriSurf( 1, 0 )->numVerts = GetTriSurf( 1, 0 )->numAllocedVerts;

		AddSurfaceDB( params.edgeMat, ( WakeLayer::MAX_NODES * 2 + 2)*MAX_POINTS, ( WakeLayer::MAX_NODES * 6)*MAX_POINTS );
...
void Wake::ClearPoints( void ) {
	numPoints = 0;
}

bool Wake::HasStopped( void ) {
	return stopped;
}

unsigned int WakeManagerLocal::AllocateWake( const WakeParms &params ) {
...
void WakeManagerLocal::BreakWake( unsigned int handle ) {
	if ( handle == -1 ) return;
	int index = (handle & 0xFFFF)-1;
	wakes[index].Break();
}

void WakeManagerLocal::Init( void ) {
	ticket = 1;
	wakes = new Wake[ MAX_WAKES ];
}

void WakeManagerLocal::Think( void ) {
	for ( int i = 0; i < MAX_WAKES; i++ ) {
		if ( !wakes[i].HasStopped() ) {
			wakes[i].Update();
		}
