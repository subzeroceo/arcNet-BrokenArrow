...
const idEventDef EV_SetAnimFrame( "setAnimFrame", '\0', DOC_TEXT( "Sets the specified channel to a fixed frame in an animation." ), 3, nullptr, "s", "anim", "Name of the animation to play.", "d", "channel", "Channel to set the fixed frame on.", "f", "frame", "Fixed frame to use." );
...
const idEventDef EV_IsAnimating( "isAnimating", 'b', DOC_TEXT( "Returns whether there are any currently active animations." ), 0, nullptr );
...
const idEventDef EV_IsAnimatingOnChannel( "isAnimatingOnChannel", 'b', DOC_TEXT( "Returns whether there are any currently active animations on the given channel." ), 1, nullptr, "d", "channel", "Channel to check." );
...
const idEventDef EV_WorldToModelSpace( "worldToModelSpace", 'v', DOC_TEXT( "Converts a vector from world space to model space." ), 1, nullptr, "v", "value", "Vector to convert." );
...
CLASS_DECLARATION( abEntity, idAnimatedEntity )
...
void idAnimatedEntity::Event_IsAnimating( void ) {
	idProgram::ReturnBoolean( animator.IsAnimating( gameLocal.time ) );
}
...
void idAnimatedEntity::Event_IsAnimatingOnChannel( animChannel_t channel ) {
	idProgram::ReturnBoolean( animator.IsAnimatingOnChannel( channel, gameLocal.time ) );
}
...
void idAnimatedEntity::Event_PlayAnim( animChannel_t channel, const char *animname ) {
	...
	if ( !anim ) {
		...
		animator.Clear( channel, gameLocal.time, 0 );
		idProgram::ReturnFloat( 0 );
	} else {
		animator.PlayAnim( channel, anim, gameLocal.time, 0 );
		idProgram::ReturnFloat( MS2SEC( animator.AnimLength( anim ) ) );
	}
}
...
void idAnimatedEntity::Event_GetAnimatingOnChannel( animChannel_t channel ) {
	...
	idProgram::ReturnString( anim->FullName() );
}
...
================
*/
anAnimator *idAnimatedEntity::GetAnimator( void ) {
	return &animator;
}
...
void idAnimatedEntity::SetModel( const char *modelname ) {
	...
	renderEntity.callback = abEntity::ModelCallback;
...
=====================
*/
bool idAnimatedEntity::GetJointWorldTransform( jointHandle_t jointHandle, int currentTime, anVec3 &offset, anMat3 &axis ) {
...
==============
*/
bool idAnimatedEntity::GetJointTransformForAnim( jointHandle_t jointHandle, int animNum, int frameTime, anMat3 &axis ) const {
	const idAnim	*anim;
	int				numJoints;
	idJointMat		*frame;
...
==============
*/
bool idAnimatedEntity::GetJointTransformForAnim( jointHandle_t jointHandle, int animNum, int frameTime, anVec3 &offset, anMat3 &axis ) const {
	const idAnim	*anim;
	int				numJoints;
	idJointMat		*frame;
...
==============
*/
bool idAnimatedEntity::GetJointTransformForAnim( jointHandle_t jointHandle, int animNum, int frameTime, anVec3 &offset ) const {
	const idAnim	*anim;
	int				numJoints;
	idJointMat		*frame;
...
================
*/
const anBounds*	idAnimatedEntity::GetSelectionBounds( void ) const {
...
================
*/
anClipModel *idAnimatedEntity::GetCombatModel( void ) const {
	return combatModel;
}
...
================
*/
void idAnimatedEntity::Event_SetJointPos( jointHandle_t jointnum, jointModTransform_t transform_type, const anVec3 &pos ) {
...
================
*/
void idAnimatedEntity::Event_SetJointAngle( jointHandle_t jointnum, jointModTransform_t transform_type, const anAngles &angles ) {
	anMat3 mat;
...
void idAnimatedEntity::Event_GetJointAxis( jointHandle_t jointnum, int index ) {
	...
	idProgram::ReturnVector( axis[ index ] );
}
...
================
*/
void idAnimatedEntity::Event_JointToWorldTransform( jointHandle_t jointnum, const anVec3 &vector ) {
	anMat3 axes;
	if ( !GetWorldAxis( jointnum, axes ) ) {
		idProgram::ReturnVector( vector );
		return;
	}
	idProgram::ReturnVector( axes * vector );
}
...
================
*/
void idAnimatedEntity::Event_WorldToModelSpace( const anVec3 &vector ) {
	idProgram::ReturnVector( vector * renderEntity.axis.Transpose() );
}
...
void idAnimatedEntity::Event_AnimName( int index ) {
	...
	idProgram::ReturnString( anim->Name() );
}
...
void idAnimatedEntity::Event_GetSurfaceId( const char *surfaceName ) {
	if ( renderEntity.hModel == nullptr ) {
		...
		idProgram::ReturnInteger( -1 );
		return;
	}

	idProgram::ReturnInteger( renderEntity.hModel->FindSurfaceId( surfaceName ) );
}
...
void idAnimatedEntity::Event_IsSurfaceHidden( int index ) {
	...
	idProgram::ReturnBoolean( renderEntity.hideSurfaceMask.Get( index ) != 0 );
}
...
void idAnimatedEntity::Event_GetNumFrames( const char *animName ) {
	int anim = animator.GetAnim( animName );
	idProgram::ReturnInteger( animator.NumFrames( anim ) );
}
