...
*/
typedef struct {
	arcNetFile*							filestream;				/* io structore of the zipfile */
...
public:
	...
	static aRcFileMemory * CombineFiles( const arcNetList< aRcFileMemory * > & srcFiles );
						// extracts multiple in-memory files from a single memory file
	static bool			ExtractFiles( aRcFileMemory * & srcFile, arcNetList< aRcFileMemory * > & destFiles );

	void				CleanSourceFolder();

	bool				CreateZipFileFromFileList( const char *name, const arcNetList< aRcFileMemory * > & srcFiles );

	zipFile				CreateZipFile( const char *name );
	bool				AddFile( zipFile zf, aRcFileMemory *fm, bool deleteFile );
	void				CloseZipFile( zipFile zf );
private:
	bool				CreateZipFile( bool appendFiles );
	bool				CreateZipFileFromFiles( const arcNetList< aRcFileMemory * > & srcFiles );
	bool				GetFileTime( const arcNetString &filename, unsigned long *dostime ) const;
	bool				IsFiltered( const arcNetString &filename ) const;
	bool				IsUncompressed( const arcNetString &filename ) const;

private:
	arcNetString				zipFileName;				// os path to the zip file
	arcNetString				sourceFolderName;			// source folder of files to zip or add
	arcStringList			filterExts;					// file extensions we want to compressed
	arcStringList			uncompressedFilterExts;		// file extensions we don't want to compress
};
