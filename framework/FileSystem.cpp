...
typedef bool (*pureExclusionFunc_t)( const struct pureExclusion_s &excl, int l, const anString &name );
...
bool excludeExtension( const pureExclusion_t &excl, int l, const anString &name ) {
	if ( l > excl.extLen && !anString::Icmp( name.c_str() + l - excl.extLen, excl.ext ) ) {
		return true;
	}
	return false;
}

bool excludePathPrefixAndExtension( const pureExclusion_t &excl, int l, const anString &name ) {
	if ( l > excl.nameLen && !anString::Icmp( name.c_str() + l - excl.extLen, excl.ext ) && !name.IcmpPrefixPath( excl.name ) ) {
		return true;
	}
	return false;
}

bool excludeFullName( const pureExclusion_t &excl, int l, const anString &name ) {
	...
	anInitExclusions() {
		for ( int i = 0; pureExclusions[i].func != nullptr; i++ ) {
			if ( pureExclusions[i].name ) {
				pureExclusions[i].nameLen = anString::Length( pureExclusions[i].name );
			}
			if ( pureExclusions[i].ext ) {
				pureExclusions[i].extLen = anString::Length( pureExclusions[i].ext );
			}
		}
	}
};
...
typedef struct {
	anString			pakFilename;				// c:\doom\base\pak0.pk5
	anPaKBFile				handle;
	int					checksum;
	int					numfiles;
	int					length;
...
typedef struct {
	anString				path;						// c:\doom
	anString				gamedir;					// base
} directory_t;
...
long anFileSystem::HashFileName( const char *fname ) const {
	...
	int i = 0;
	while ( fname[i] != '\0' ) {
		char letter = anString::ToLower( fname[i] );
		if ( letter == '.' ) {
...
=================
*/
bool FS_CompareArchiveFileChecksum( const char *arcFile ) {
	int index, checksum;

	pk4 = FS_LoadZipFile( arcFile, "" );
	...
	for ( index = 0; index < enginePakChecksum; index++ ) {
...
================
*/
bool anFileSystem::IsBinaryModel( const anString &resName ) const {
	idStrStatic< 32 > ext;
...
================
*/
FILE *anFileSystem::OpenOSFile( const char *fileName, const char *mode, anString *caseSensitiveName ) {
	int i;
	FILE *fp;
	anString fpath, entry;
	anStringList list;
...
================
*/
FILE *anFileSystem::OpenOSFileCorrectName( anString &path, const char *mode ) {
	anString caseName;
...
=================
*/
void anFileSystem::CopyFile( anFile *src, const char *toOSPath ) {
	FILE	*f;
	int		len;
	byte	*buf;
...
====================
*/
void anFileSystem::ReplaceSeparators( anString &path, char sep ) {
	...
	if ( fs_caseSensitiveOS.GetBool() || com_developer.GetBool() ) {
		...
		if ( testPath.HasUpper() ) {
			...
			if ( fs_caseSensitiveOS.GetBool() ) {
				...
				anString::Copynz( OSPath, newPath, sizeof( OSPath ) );
				return OSPath;
			}
		}
	}

	anString strBase = base;
...
int anFileSystem::DeleteFile( const char *name ) const {
	...
	if ( anString::Ncmp( name, "save/", 5 ) != 0 ) {
		return 0;
	}
...
=================
*/
pack_t *anFileSystem::LoadZipFile( const char *zipfile ) {
	fileInPack_t *	buildBuffer;
	pack_t *		pack;
	anPaKBFile			uf;
	int				err;
...
int anFileSystem::AddZipFile( const char *path ) {
	anString fullpath = fs_savepath.GetString();

	fullpath.AppendPath( path );
	pack_t *pak = LoadZipFile( fullpath );
	if ( !pak ) {
...
void anFileSystem::GetExtensionList( const char *extension, anStringList &extensionList ) const {
	int l = anString::Length( extension );
	int s = 0;
	while ( 1 ) {
		int e = anString::FindChar( extension, '|', s, l );
		if ( e != -1 ) {
			extensionList.Append( anString( extension, s, e ) );
			s = e + 1;
		} else {
			extensionList.Append( anString( extension, s, l ) );
			break;
		}
	}
}
	...
	for ( search = searchPaths; search != nullptr; search = search->next ) {
		if ( search->dir ) {
			...
			for ( int i = 0; i < pak->numfiles; i++ ) {
				...
				if ( pathLength && anString::Icmpn( name, relativePath, pathLength - 1 ) != 0 ) {
					continue;
				}
...
idFileSystem::ListSortedFiles( "/path/to/directory", fileList)
or 
...
================
*/
void idFileSystem::ListSortedFiles( const char *directory, idFileList &fileList ) const {
...
anModList *anFileSystem::ListMods( void ) {
	...
	for ( isearch = 0; isearch < 4; isearch++ ) {
		...
		for ( int i = 0; i < dirs.Num(); i++ ) {
			anString gamepath = BuildOSPath( search[isearch], dirs[i], "" );
	...
	for ( i = 0; i < list->mods.Num(); i++ ) {
		for ( isearch = 0; isearch < 4; isearch++ ) {
			anString descfile = BuildOSPath( search[isearch], list->mods[i], "description.txt" );
...
===============
*/
void anFileSystem::FollowAddonDependencies( pack_t *pak ) {
	assert( pak );
...
void anFileSystem::Startup( void ) {
	...
	if ( fs_game_base.GetString()[0] &&
		 anString::Icmp( fs_game_base.GetString(), BASE_GAMEDIR ) ) {
		SetupGameDirectories( fs_game_base.GetString() );
	}

	// fs_game override
	if ( fs_game.GetString()[0] &&
		 anString::Icmp( fs_game.GetString(), BASE_GAMEDIR ) &&
		 anString::Icmp( fs_game.GetString(), fs_game_base.GetString() ) ) {
...
=====================
*/
pack_t* anFileSystem::GetPackForChecksum( int checksum, bool searchAddons ) {
	searchpath_t	*search;
	for ( search = searchPaths; search; search = search->next ) {
		if ( !search->pack ) {
			continue;
...
int anFileSystem::ValidateDownloadPakForChecksum( int checksum, char path[ MAX_STRING_CHARS ], bool isBinary ) {
	...
	anString::Copynz( path, relativePath, MAX_STRING_CHARS );
	return pak->length;
}
...
void anFileSystem::SetRestartChecksums( const int pureChecksums[ MAX_PURE_PAKS ], int enginePakChecksum ) {
	...
	int i = 0;
	while ( pureChecksums[i] ) {
		pack_t *pack = GetPackForChecksum( pureChecksums[i], true );
		if ( !pack ) {
...
===========
*/
pureStatus_t anFileSystem::GetPackStatus( pack_t *pak ) {
	int				l, hashindex;
	bool			abrt;
	anString		name;
...
===========
*/
anCompressedArchive * anFileSystem::ReadFileFromZip( pack_t *pak, fileInPack_t *pakFile, const char *relativePath ) {
	pakBFile_s *			zfi;
	FILE *			fp;
...
===========
*/
anFile *anFileSystem::OpenFileReadFlags( const char *relativePath, int searchFlags, pack_t **foundInPak, bool allowCopyFiles, const char* gamedir ) {
	searchpath_t *	search;
	anString			netpath;
	pack_t *		pak;
	fileInPack_t *	pakFile;
	...
	if (  || relativePath[0] == '\0' && common->IsInitialized() && ( anString::Icmp( relativePath, CDKEY_FILE ) == 0 || anString::Icmp( relativePath, XPKEY_FILE ) == 0 ) ) {
		return nullptr;
	}
...
anFile *anFileSystem::OpenFileAppend( const char *relativePath, bool sync, const char *basePath ) {
	...
	anString OSpath = BuildOSPath( path, engineFolder, relativePath );
	CreateOSPath(OSpath);
	...
	while( 1 ) {
		...
		if ( bgl->opcode == DLTYPE_FILE ) {
			...
			if ( ret ) {
				...
				anString::Copynz( bgl->url.dlerror, error_buf, MAX_STRING_CHARS );
				bgl->url.dlstatus = ret;
				bgl->url.status = DL_FAILED;
				bgl->completed = true;
				continue;
...
const char *anFileSystem_local::CaseSearch( const char *inDir ) {
	...
	anString walk_path = inDir;
	anString current_dir;
	int list_ret;
	do {
	...
	if ( list_ret == -1 ) {
		common->DPrintf( "WARNING: didn't find any matching root directory for '%s'\n", inDir );
...
unsigned int anFileSystem::GetTimestamp( const char *relativePath ) {
    if ( relativePath == nullptrptr || relativePath[0] == '\0' ) {
        return FILE_NOT_FOUND_TIMESTAMP;
    }

    unsigned int timestamp = 0;
    ReadFile( relativePath, nullptrptr, &timestamp );
...
=================
*/
pack_t *anFileSystem::FindPakForFileChecksum( const char *relativePath, int findChecksum, bool bReference ) {
	searchpath_t	*search;
	pack_t			*pak;
	fileInPack_t	*pakFile;
	int				hash;
...
void anFileSystem::FindDLL( const char *name, char _dllPath[ MAX_OSPATH ], bool updateChecksum ) {
	...
	anString::snPrintf( _dllPath, MAX_OSPATH, dllPath.c_str() );
}
...
anFile * anFileSystem::MakeTemporaryFile( void ) {
	FILE *f = tmpfile();
	if ( !f ) {
		common->Warning( "anFileSystem::MakeTemporaryFile failed: %s", strerror( errno ) );
		return nullptr;
	}
	anFilePermanent *file = new anFilePermanent();
	file->o = f;
...
bool anFileSystem::IsExtension( const char *fileName, const char *ext, int nLength ) const {
	int extLength = anString::Length( ext );
	...
	return !anString::Icmp( fileName, ext );
}
...
void anFileSystem::FindMapScreenshot( const char *path, char *buf, int len ) {
	anFile	*file;
	anString	mapname = path;

	mapname.StripPath();
	mapname.StripFileExtension();
	
	anString::snPrintf( buf, len, "guis/assets/splash/%s.tga", mapname.c_str() );
	if ( ReadFile( buf, nullptr, nullptr ) == -1 ) {
		...
		if ( file ) {
			...
			file->Read( data, dlen );
			CloseFile( file );
			anString::snPrintf( buf, len, "guis/assets/splash/addon/%s.tga", mapname.c_str() );
			WriteFile( buf, data, dlen );
			delete[] data;
		} else {
			anString::Copynz( buf, "guis/assets/splash/pdtempa", len );
		}
	}
}
