...
static ARC_INLINE void GetCropBounds( const frustumCorners_t &corners, const float *renderMat, anBounds &bounds ) {
    ...
    bounds.AddPoint(anVec3(outXs[0], outYs[0], outZs[0]));
    bounds.AddPoint(anVec3(outXs[1], outYs[1], outZs[1]));
    bounds.AddPoint(anVec3(outXs[2], outYs[2], outZs[2]));
    bounds.AddPoint(anVec3(outXs[3], outYs[3], outZs[3]));
    bounds.AddPoint(anVec3(outXs[4], outYs[4], outZs[4]));
    bounds.AddPoint(anVec3(outXs[5], outYs[5], outZs[5]));
    bounds.AddPoint(anVec3(outXs[6], outYs[6], outZs[6]));
    bounds.AddPoint(anVec3(outXs[7], outYs[7], outZs[7]));
}
/*
===================
arcSIMD_AVX::
