...
anPerlin::anPerlin( float _persistence, int _octaves, float _frequency ) {
...
float anPerlin::RawNoise( int x, int y, int z ) {
	int n = x + y * 57 + z * 227;
	n = ( n << 13 ) ^ n;
	//if (alternative)
...
float anPerlin::NoiseFloat( const anVec3 &pos ) {
      float rez = 0.0f;
...
float anPerlin3::Noise( float x ) {
	return 0.f;
}

float anPerlin3::Noise( float x, float y ) {
	return 0.f;
}

float anPerlin3::Noise( float x, float y, float z ) {
	// Find unit cube that contains point
	int ucX = static_cast< int >( anMath::Floor( x ) ) & 255;
	int ucY = static_cast< int >( anMath::Floor( y ) ) & 255;
	int ucZ = static_cast< int >( anMath::Floor( z ) ) & 255;
