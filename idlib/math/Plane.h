...
ARC_INLINE void anPlane::Set( float a, float b, float c, float d ) {
	this->a = a;
	this->b = b;
	this->c = c;
	this->d = d;
...
ARC_INLINE bool anPlane::FromPointsHighPrecision( const anVec3 &p0, const anVec3 &p1, const anVec3 &p2, bool fixDegenerate ) {
...
ARC_INLINE bool anPlane::LineIntersection( const anVec3 &start, const anVec3 &end, float &fraction ) const {
	float d1 = Normal() * start + d;
	float d2 = Normal() * end + d;
	if ( d1 == d2 ) {
		return false;
