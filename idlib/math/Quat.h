...
inline anQuats::anQuats( void ) {
}

inline anQuats::anQuats( float x, float y, float z, float w ) {
	this->x = x;
	this->y = y;
	this->z = z;
	this->w = w;
...
inline float anQuats::operator[]( int index ) const {
	assert( ( index >= 0 ) && ( index < 4 ) );
	return ( &x )[index];
}

inline float& anQuats::operator[]( int index ) {
	assert( ( index >= 0 ) && ( index < 4 ) );
	return ( &x )[index];
}

inline anQuats anQuats::operator-() const {
	return anQuats( -x, -y, -z, -w );
}

inline anQuats &anQuats::operator=( const anQuats &a ) {
	x = a.x;
	y = a.y;
	z = a.z;
	w = a.w;
...
inline anQuats anQuats::operator+( const anQuats &a ) const {
...
inline anVec3 anQuats::operator*( const anVec3 &a ) const {
#if 0
...
inline anQuats anQuats::operator*( float a ) const {
	return anQuats( x * a, y * a, z * a, w * a );
}

inline anQuats operator*( const float a, const anQuats &b ) {
	return b * a;
}

inline anVec3 operator*( const anVec3 &a, const anQuats &b ) {
	return b * a;
}
...
inline anQuats& anQuats::operator*=( float a ) {
	x *= a;
	y *= a;
	z *= a;
	w *= a;
...
inline bool anQuats::Compare( const anQuats &a ) const {
	return ( ( x == a.x ) && ( y == a.y ) && ( z == a.z ) && ( w == a.w ) );
}

inline bool anQuats::Compare( const anQuats &a, const float epsilon ) const {
...
inline bool anQuats::operator==( const anQuats &a ) const {
	return Compare( a );
}
...
inline void anQuats::Set( float x, float y, float z, float w ) {
	this->x = x;
	this->y = y;
	this->z = z;
	this->w = w;
...
inline anQuats anQuats::Inverse( void ) const {
	return anQuats( -x, -y, -z, w );
}

inline float anQuats::Length( void ) const {
	float len;
...
inline anQuats& anQuats::Normalize( void ) {
	float len;
	float ilength;
...
inline float anQuats::CalcW( void ) const {
	// take the absolute value because floating point rounding may cause the dot of x,y,z to be larger than 1
	return sqrt( fabs( 1.0f - ( x * x + y * y + z * z ) ) );
}

inline int anQuats::GetDimension( void ) const {
	return 4;
}

inline const float *anQuats::ToFloatPtr( void ) const {
	return &x;
}

inline float *anQuats::ToFloatPtr( void ) {
	return &x;
}

inline anVec3 anQuats::GetColumn0() const {
	return anVec3(
		2 * ( v.x * v.x + w * w ) - 1,
		2 * ( v.y * v.x + v.z * w ),
		2 * ( v.z * v.x - v.y * w ) );
...
inline anVec3 anQuats::GetColumn1() const {
	return anVec3(
		2 * ( v.x * v.y - v.z * w ),
		2 * ( v.y * v.y + w * w ) - 1,
		2 * ( v.z * v.y + v.x * w ) );
...
inline anVec3 anQuats::GetColumn2() const {
	return anVec3(
		2 * ( v.x * v.z + v.y * w ),
		2 * ( v.y * v.z - v.x * w ),
		2 * ( v.z * v.z + w * w ) - 1 );
...
inline anVec3 anQuats::GetRow0() const {
	return anVec3(
		2 * ( v.x * v.x + w * w ) - 1,
		2 * ( v.x * v.y - v.z * w ),
		2 * ( v.x * v.z + v.y * w ) );
...
inline anVec3 anQuats::GetRow1() const {
	return anVec3(
		2 * ( v.y * v.x + v.z * w ),
		2 * ( v.y * v.y + w * w ) - 1,
		2 * ( v.y * v.z - v.x * w ) );
...
inline anVec3 anQuats::GetRow2() const {
	return anVec3(
		2 * ( v.z * v.x - v.y * w ),
		2 * ( v.z * v.y + v.x * w ),
 		2 * ( v.z * v.z + w * w ) - 1 );
...
inline anVec3 anQuats::GetFwdX() const {
	return anVec3( 2 * ( v.x * v.y - v.z * w ) );
}

inline float anQuats::GetFwdY() const {
	return anVec3( 2 * ( v.y * v.y + w * w ) - 1 );
}

inline float anQuats::GetFwdZ() const {
	return anVec3( 2 * ( v.z * v.y + v.x * w ) );
}

inline float anQuats::GetRotZ() const {
...
inline anCQuats::anCQuats( void ) {
}

inline anCQuats::anCQuats( float x, float y, float z ) {
	this->x = x;
	this->y = y;
	this->z = z;
}

inline void anCQuats::Set( float x, float y, float z ) {
	this->x = x;
	this->y = y;
	this->z = z;
}

inline float anCQuats::operator[]( int index ) const {
	assert( ( index >= 0 ) && ( index < 3 ) );
	return ( &x )[index];
}

inline float& anCQuats::operator[]( int index ) {
	assert( ( index >= 0 ) && ( index < 3 ) );
	return ( &x )[index];
}

inline bool anCQuats::Compare( const anCQuats &a ) const {
	return ( ( x == a.x ) && ( y == a.y ) && ( z == a.z ) );
}

inline bool anCQuats::Compare( const anCQuats &a, const float epsilon ) const {
	if ( anMath::Fabs( x - a.x ) > epsilon ) {
		return false;
	}
	if ( anMath::Fabs( y - a.y ) > epsilon ) {
...
inline bool anCQuats::operator==( const anCQuats &a ) const {
	return Compare( a );
}
...
inline int anCQuats::GetDimension( void ) const {
	return 3;
}

inline anQuats anCQuats::ToQuat( void ) const {
	// take the absolute value because floating point rounding may cause the dot of x,y,z to be larger than 1
	return anQuats( x, y, z, sqrt( fabs( 1.0f - ( x * x + y * y + z * z ) ) ) );
}

inline const float *anCQuats::ToFloatPtr( void ) const {
	return &x;
}

inline float *anCQuats::ToFloatPtr( void ) {
	return &x;
}
