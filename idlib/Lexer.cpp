...
================
*/
int anLexer::ReadString( anToken *token, int quote ) {
	int tmpline;
...
================
*/
int anLexer::ReadName( anToken *token ) {
	char c;
...
================
*/
int anLexer::ReadNumber( anToken *token ) {
	int i;
	int dot;
	char c, c2;
...
================
*/
int anLexer::ExpectAnyToken( anToken *token ) {
	if ( !anLexer::ReadToken( token ) ) {
		anLexer::Error( "[ERROR] couldn't read expected token" );
		return 0;
	} else {
...
================
*/
int anLexer::CheckTokenType( int type, int subtype, anToken *token ) {
	anToken tok;
...
================
*/
int anLexer::PeekTokenType( int type, int subtype, anToken *token ) {
	anToken tok;
...
================
*/
void anLexer::UnreadToken( const anToken *token ) {
	if ( anLexer::tokenavailable ) {
		anLibrary::common->FatalError( "anBinaryLexer::UnreadToken, unread token twice\n" );
	}
	anLexer::token = *token;
...
================
*/
int anLexer::ReadTokenOnLine( anToken *token ) {
	anToken tok;
...
void anLexer::Reset( void ) {
	...
	anLexer::whiteSpaceStart_p = nullptr;
	// end of white space
	anLexer::whiteSpaceEnd_p = nullptr;
...
int anLexer::LoadFile( const char *filename, bool OSPath ) {
	...
	if ( anLexer::loaded ) {
		anLibrary::common->Error( "[WARNING] anBinaryLexer::LoadFile: another script already loaded" );
		return false;
	}
...
void anLexer::FreeSource( void ) {
	...
	if ( anLexer::punctuationtable && anLexer::punctuationtable != default_punctuationtable ) {
		...
		anLexer::punctuationtable = nullptr;
	}
	if ( anLexer::nextpunctuation && anLexer::nextpunctuation != default_nextpunctuation ) {
		Mem_Free( (void *) anLexer::nextpunctuation );
		anLexer::nextpunctuation = nullptr;
	}
#endif //PUNCTABLE
	if ( anLexer::allocated ) {
		Mem_Free( (void *) anLexer::buffer );
		anLexer::buffer = nullptr;
		anLexer::allocated = false;
	}
	anLexer::tokenavailable = 0;
	anLexer::token = "";
...
anLexer::anLexer( void ) {
	...
	anLexer::filename = "";
	anLexer::flags = 0;
	anLexer::SetPunctuations( nullptr );
	anLexer::allocated = false;
	anLexer::fileTime = 0;
	anLexer::length = 0;
	anLexer::line = 0;
