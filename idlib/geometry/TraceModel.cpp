		...
		for ( int k = triBins[0][0]; k <= triBins[1][0]; k++ ) {
			for ( int l = triBins[0][1]; l <= triBins[1][1]; l++ ) {
				for ( int m = triBins[0][2]; m <= triBins[1][2]; m++ ) {
					hashTriangle_t *tri = triangleAllocator.Alloc();
					tri->triIndex = i;
...
bool anSurface_Traceable::RayIntersection( /*arcList< int >& tracedTris,*/ const anVec3& start, const anVec3& dir, float& scale, anDrawVertex& dv, bool backFaceCull ) const {
	...
	for ( int i = 0; i < RAY_STEPS; i++, point += step ) {
		...
		for ( TraceableTriHash::hashTriangle_t *tri = bin->triangleList; tri; tri = tri->next ) {
...
void anSurface_Traceable::GenerateIntersectionDrawVert( const anVec3 &intersection, const int intersectedTriIndex, anDrawVertex &dv ) const {
	...
	float a = idWinding::TriangleArea( intersection, v1->xyz, v2->xyz ) / denom;
	float b = idWinding::TriangleArea( v0->xyz, intersection, v2->xyz ) / denom;
	float c = idWinding::TriangleArea( v0->xyz, v1->xyz, intersection ) / denom;
