...
static anCVar jobs_longJobMicroSec( "jobs_longJobMicroSec", "10000", CVAR_INTEGER, "print a warning for jobs that take more than this number of microseconds" );
...
private:
	...
	anList< job_t, TAG_JOBLIST >		jobList;
	anList< idSysInterlockedInteger, TAG_JOBLIST >	signalJobCount;
...
static anCVar jobs_prioritize( "jobs_prioritize", "1", CVAR_BOOL | CVAR_NOCHEAT, "prioritize job lists" );
...
anCVar jobs_numThreads( "jobs_numThreads", NUM_JOB_THREADS, CVAR_INTEGER | CVAR_NOCHEAT, "number of threads used to crunch through jobs", 0, MAX_JOB_THREADS );
...
private:
	...
	int								numCpuPackages;
	anStaticList< idParallelJobList *, MAX_JOBLISTS >	jobLists;
};
...
void idParallelJobManagerLocal::Submit( idParallelJobList_Threads * jobList, int parallelism ) {
	if ( jobs_numThreads.IsModified() ) {
		maxThreads = anMath::ClampInt( 0, MAX_JOB_THREADS, jobs_numThreads.GetInteger() );
