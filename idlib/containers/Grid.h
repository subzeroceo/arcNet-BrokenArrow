...
template<class objType> class idGranBoundsObjectList;
...
class idGridObject {
	...
	friend class idGranBoundsObjectList<objType>;
...
//===============================================================
//
//	idGranBoundsObjectList
//
//===============================================================

template<class objType>
class idGranBoundsObjectList {
	friend class idGrid3D<objType>;
...
private:
	int								checkCount;		// current check count
	mutable idGridLink<objType> *	nextObject;		// next object in cell
	mutable anList<idGridLink<objType> *>cells;		// list with cell to iterate
};
...
public:
	...
	void							Init( const anBounds &bounds, float desiredCellSize );

									// Adds an object to the grid.
	idGridObject<objType> *			AddObject( objType object, const anBounds &bounds, const float epsilon );
	...
	void							ChangeObjectBounds( idGridObject<objType> *gridObject, const anBounds &bounds, const float epsilon );
	...
	void							GetBoundsObjects( const anBounds &bounds, const float epsilon, idGranBoundsObjectList<objType> &list );

	const anBounds &				GetGranBounds() const { return granBounds; }
...
private:
	...
	anList<idGridObject<objType> *>	gridObjects;
	int								checkCount;

	anBlockAlloc<idGridObject<objType>, 1024>	gridObjectAllocator;
	anBlockAlloc<idGridLink<objType>, 1024>		gridLinkAllocator;

	void							GetGridCells( const anBounds &bounds, int iBounds[2][3] ) const;
	void							LinkObject( idGridObject<objType> *gridObject, const anBounds &bounds, const float epsilon );
...
ARC_INLINE idGridObject<objType> * idGridNearbyObjectList<objType>::GetNextObject() const {
	...
	} while( object == nullptr || object->checkCount == checkCount || object == gridObject );
...
ARC_INLINE idGridObject<objType> * idGranBoundsObjectList<objType>::GetNextObject() const {
	...
	} while( object == nullptr || object->checkCount == checkCount );
	object->checkCount = checkCount;
	...
	for ( i = iBounds[0][0]; i <= iBounds[1][0]; i++ ) {
		for ( j = iBounds[0][1]; j <= iBounds[1][1]; j++ ) {
			for ( k = iBounds[0][2]; k <= iBounds[1][2]; k++ ) {
				...
				if ( gridLink->nextObject != nullptr ) {
...
ARC_INLINE void idGrid3D<objType>::Test() {
	...
	for ( i = 0; i < grid.GetNumObjects(); i++ ) {
		...
		for ( obj2 = list.GetNextObject(); obj2 != nullptr; obj2 = list.GetNextObject() ) {
		}
	}
}
