...
/*
================================================
The *Swap* static template class, anSwap, is used by the anSwapClass template class for
...
#define SWAP_VECTOR( x ) \
	...
	SWAP_VECTOR( anPlane );
	SWAP_VECTOR( anQuats );
	SWAP_VECTOR( anCQuats );
	SWAP_VECTOR( anAngles );
	SWAP_VECTOR( anBounds );

	static void Little( anDrawVertex &v ) {
			Little( v.xyz );
			LittleArray( v.st, 2 );
	...
	static void Big( anDrawVertex &v ) {
			Big( v.xyz );
			BigArray( v.st, 2 );
			BigArray( v.normal, 4 );
