...
anSecondaryHeap defaultHeap;					// this is the default system heap

static anSecondaryHeap *systemHeapArray[MAX_SYSTEM_HEAPS];		// array of pointers to idHeapSecondarys that are common to anLibrary, Game, and executable
...
==================
*/
void idSetSystemHeap( sysHeap_ID_t sysHeapID, anSecondaryHeap *heapPtr ) {
...
==================
*/
void idPushSystemHeap( sysHeap_ID_t sysHeapID 01) {
	assert( ( uint ) sysHeapID < ( uint ) maxHeapCount );
...
void Mem_Init( void ) {
	if ( nullptr == currentHeapArena ) {
		...
		PushSystemHeapID( TIDHEAP_ID_DEFAULT );
...
void Mem_ShowMemAlloc_f( const anCommandArgs &args ) {
	...
	for ( int tag = 1; tag < MA_DO_NOT_USE; tag++ ) {
...
void Mem_DumpCompressed( const char *fileName, memorySortType_t memSort, int sortCallStack, int numFrames, bool verbose  ) {
	...
	for ( b = mem_debugMemory; b; b = b->next ) {
		...
		for ( a = allocInfo; a; a = a->next ) {
			...
			if ( anStr::Cmp( a->fileName, b->fileName ) != 0 ) {
				continue;
			}
	...
	for ( a = allocInfo; a; a = nexta ) {
		...
		switch ( memSort ) {
			...
			case MEMSORT_LOCATION: {
				for ( nextSorted = sortedAllocInfo; nextSorted; nextSorted = nextSorted->next ) {
					r = anStr::Cmp( Mem_CleanupFileName( a->fileName ), Mem_CleanupFileName( nextSorted->fileName ) );
	...
	for ( a = sortedAllocInfo; a; a = nexta ) {
		...
		if ( ( a->size >> 10 ) > 0 ) {
			anStr::snPrintf( buff, sizeof( buff ), "%6d K",( a->size >> 10 ) );
		} else {
			anStr::snPrintf( buff, sizeof( buff ), "%6d B", a->size );
		}
...
void Mem_DumpCompressed_f( const anCommandArgs &args ) {
	...
	while( arg[0] == '-' ) {
		arg = args.Argv( ++argNum );
		if ( anStr::Icmp( arg, "s" ) == 0 ) {
			memSort = MEMSORT_SIZE;
		} else if ( anStr::Icmp( arg, "l" ) == 0 ) {
			memSort = MEMSORT_LOCATION;
		} else if ( anStr::Icmp( arg, "a" ) == 0 ) {
			memSort = MEMSORT_NUMALLOCS;
		} else if ( anStr::Icmp( arg, "cs1" ) == 0 ) {
			memSort = MEMSORT_CALLSTACK;
			sortCallStack = 2;
		} else if ( anStr::Icmp( arg, "cs2" ) == 0 ) {
			memSort = MEMSORT_CALLSTACK;
			sortCallStack = 1;
		} else if ( anStr::Icmp( arg, "cs3" ) == 0 ) {
			memSort = MEMSORT_CALLSTACK;
			sortCallStack = 0;
		} else if ( anStr::Icmp( arg, "h" ) == 0 ) {
			memSort = MEMSORT_HEAPID;
		} else if ( anStr::Icmp( arg, "v" ) == 0 ) {
			verbose = true;
		} else if ( arg[0] == 'f' ) {
...
==================
*/
anSecondaryHeap* GetSysHeap( sysHeap_ID_t sysHeapID ) {
	assert( ( uint ) sysHeapID < ( uint ) maxHeapCount );
...
==================
*/
void GetAllSysHeaps( anSecondaryHeap *destSystemHeapArray[MAX_SYSTEM_HEAPS] ) {
...
==================
*/
void idSetAllSysHeaps( anSecondaryHeap *srcSystemHeapArray[MAX_SYSTEM_HEAPS] ) {
...
==================
*/
anSecondaryHeap* idGetHeapContainingMemory( const void *mem ) {
...
bool PushHeapContainingMemory( const void *mem ) {
	if ( currentHeapArena ) {
		anSecondaryHeap *heap = urrentHeapArena->GetHeap( const_cast<void*>( mem ) );
		if ( heap ) {
			heap->PushCurrent();
			return true;
		}
