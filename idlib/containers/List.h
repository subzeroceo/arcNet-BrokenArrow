...
*/
template< class type >
ARC_INLINE void idSwap( type &a, type &b ) {
	type c = a;
	a = b;
	b = c;
}
...
public:
	...
	typedef type	new_t( void );
					...
					arcNetList( int newgranularity = 16 );
					arcNetList( const arcNetList<type> &other );
					~arcNetList<type>( void );
	...
	arcNetList<type> &	operator=( const arcNetList<type> &other );
	...
	int				Append( const arcNetList<type> &other );				// append list
	...
	void			Swap( arcNetList<type> &other );						// swap the contents of the lists
...
/*
================
arcNetList<type>::arcNetList( int )
================
*/
template< class type >
ARC_INLINE arcNetList<type>::arcNetList( int newgranularity ) {
...
/*
================
arcNetList<type>::arcNetList( const arcNetList<type> &other )
================
*/
template< class type >
ARC_INLINE arcNetList<type>::arcNetList( const arcNetList<type> &other ) {
	list = NULL;
	*this = other;
}
...
/*
================
arcNetList<type>::~arcNetList<type>
================
*/
template< class type >
ARC_INLINE arcNetList<type>::~arcNetList( void ) {
	Clear();
}
...
*/
template< class type >
ARC_INLINE void arcNetList<type>::Clear( void ) {
	if ( list ) {
		delete[] list;
	}
...
/*
================
arcNetList<type>::DeleteContents
...
*/
template< class type >
ARC_INLINE void arcNetList<type>::DeleteContents( bool clear ) {
	int i;
...
/*
================
arcNetList<type>::Allocated

return total memory allocated for the list in bytes, but doesn't take into account additional memory allocated by type
================
*/
template< class type >
ARC_INLINE size_t arcNetList<type>::Allocated( void ) const {
	return size * sizeof( type );
}

/*
================
arcNetList<type>::Size

return total size of list in bytes, but doesn't take into account additional memory allocated by type
================
*/
template< class type >
ARC_INLINE size_t arcNetList<type>::Size( void ) const {
	return sizeof( arcNetList<type> ) + Allocated();
}

/*
================
arcNetList<type>::MemoryUsed
================
*/
template< class type >
ARC_INLINE size_t arcNetList<type>::MemoryUsed( void ) const {
...
*/
template< class type >
ARC_INLINE int arcNetList<type>::Num( void ) const {
	return num;
}

/*
================
arcNetList<type>::NumAllocated

Returns the number of elements currently allocated for.
================
*/
template< class type >
ARC_INLINE int arcNetList<type>::NumAllocated( void ) const {
	return size;
}

/*
================
arcNetList<type>::SetNum

Resize to the exact size specified irregardless of granularity
================
*/
template< class type >
ARC_INLINE void arcNetList<type>::SetNum( int newnum, bool resize ) {
	assert( newnum >= 0 );
...
/*
================
arcNetList<type>::SetGranularity

Sets the base size of the array and resizes the array to match.
================
*/
template< class type >
ARC_INLINE void arcNetList<type>::SetGranularity( int newgranularity ) {
	int newsize;
...
/*
================
arcNetList<type>::GetGranularity

Get the current granularity.
================
*/
template< class type >
ARC_INLINE int arcNetList<type>::GetGranularity( void ) const {
	return granularity;
}

/*
================
arcNetList<type>::Condense

Resizes the array to exactly the number of elements it contains or frees up memory if empty.
================
*/
template< class type >
ARC_INLINE void arcNetList<type>::Condense( void ) {
	if ( list ) {
		if ( num ) {
			Resize( num );
		} else {
...
*/
template< class type >
ARC_INLINE void arcNetList<type>::Resize( int newsize ) {
	type	*temp;
	int		i;
...
*/
template< class type >
ARC_INLINE void arcNetList<type>::Resize( int newsize, int newgranularity ) {
	type	*temp;
	int		i;
...
/*
================
arcNetList<type>::AssureSize

Makes sure the list has at least the given number of elements.
================
*/
template< class type >
ARC_INLINE void arcNetList<type>::AssureSize( int newSize ) {
	int newNum = newSize;
...
*/
template< class type >
ARC_INLINE void arcNetList<type>::AssureSize( int newSize, const type &initValue ) {
	int newNum = newSize;
...
/*
================
arcNetList<type>::AssureSizeAlloc
...
*/
template< class type >
ARC_INLINE void arcNetList<type>::AssureSizeAlloc( int newSize, new_t *allocator ) {
	int newNum = newSize;
...
/*
================
arcNetList<type>::operator=

Copies the contents and size attributes of another list.
================
*/
template< class type >
ARC_INLINE arcNetList<type> &arcNetList<type>::operator=( const arcNetList<type> &other ) {
	int	i;
...
/*
================
arcNetList<type>::operator[] const
...
*/
template< class type >
ARC_INLINE const type &arcNetList<type>::operator[]( int index ) const {
	assert( index >= 0 );
	assert( index < num );
...
*/
template< class type >
ARC_INLINE type &arcNetList<type>::operator[]( int index ) {
	assert( index >= 0 );
	assert( index < num );
...
*/
template< class type >
ARC_INLINE type *arcNetList<type>::Ptr( void ) {
	return list;
}
...
*/
template< class type >
const ARC_INLINE type *arcNetList<type>::Ptr( void ) const {
	return list;
}

/*
================
arcNetList<type>::Alloc

Returns a reference to a new data element at the end of the list.
================
*/
template< class type >
ARC_INLINE type &arcNetList<type>::Alloc( void ) {
	if ( !list ) {
		Resize( granularity );
	}
...
*/
template< class type >
ARC_INLINE int arcNetList<type>::Append( type const & obj ) {
	if ( !list ) {
		Resize( granularity );
	}
...
*/
template< class type >
ARC_INLINE int arcNetList<type>::Insert( type const & obj, int index ) {
	if ( !list ) {
		Resize( granularity );
	}
...
*/
template< class type >
ARC_INLINE int arcNetList<type>::Append( const arcNetList<type> &other ) {
	if ( !list ) {
		if ( granularity == 0 ) {	// this is a hack to fix our memset classes
			granularity = 16;
		}
...
/*
================
arcNetList<type>::AddUnique

Adds the data to the list if it doesn't already exist.  Returns the index of the data in the list.
================
*/
template< class type >
ARC_INLINE int arcNetList<type>::AddUnique( type const & obj ) {
	int index;
...
/*
================
arcNetList<type>::FindIndex

Searches for the specified data in the list and returns it's index.  Returns -1 if the data is not found.
================
*/
template< class type >
ARC_INLINE int arcNetList<type>::FindIndex( type const & obj ) const {
	int i;
...
*/
template< class type >
ARC_INLINE int arcNetList<type>::FindNull( void ) const {
	int i;
...
*/
template< class type >
ARC_INLINE int arcNetList<type>::IndexOf( type const *objptr ) const {
	int index;
...
*/
template< class type >
ARC_INLINE bool arcNetList<type>::RemoveIndex( int index ) {
	int i;
...
*/
template< class type >
ARC_INLINE bool arcNetList<type>::Remove( type const & obj ) {
	int index;
...
*/
template< class type >
ARC_INLINE void arcNetList<type>::Sort( cmp_t *compare ) {
	if ( !list ) {
		return;
	}
...
/*
================
arcNetList<type>::SortSubSection

Sorts a subsection of the list.
================
*/
template< class type >
ARC_INLINE void arcNetList<type>::SortSubSection( int startIndex, int endIndex, cmp_t *compare ) {
	if ( !list ) {
		return;
	}
	if ( startIndex < 0 ) {
...
/*
================
arcNetList<type>::Swap

Swaps the contents of two lists
================
*/
template< class type >
ARC_INLINE void arcNetList<type>::Swap( arcNetList<type> &other ) {
