...
anBoundsShort bounds_short_zero;

/*
================
anBoundsShort::PlaneDistance
================
*/
float anBoundsShort::PlaneDistance( const idPlane &plane ) const {
	// Gordon: FIXME: Don't convert to float?
	const anVec3 center( ( b[ 0 ][ 0 ] + b[ 1 ][ 0 ] ) * 0.5f,
						( b[ 0 ][ 1 ] + b[ 1 ][ 1 ] ) * 0.5f,
						( b[ 0 ][ 2 ] + b[ 1 ][ 2 ] ) * 0.5f );

	const anVec3 planeNormal = plane.Normal();
	const float planeD = plane[ 3 ];

	const float d2 = anMath::Fabs( ( b[1][0] - center[0] ) * planeNormal[0] ) +
					anMath::Fabs( ( b[1][1] - center[1] ) * planeNormal[1] ) +
					anMath::Fabs( ( b[1][2] - center[2] ) * planeNormal[2] );
...
int anBoundsShort::PlaneSide( const idPlane &plane, const float epsilon ) const {
#if defined( ID_WIN_X86_SSE )
	__asm {
		mov			edi, this
		mov			esi, plane
		...
		// anMath::Fabs( ( b[1] - center ) * plane.Normal() )
		subps		xmm5, xmm7
		mulps		xmm5, xmm1
		...
		// xmm5 contains final anMath::Fabs( ( b[1] - center ) * plane.Normal() )
