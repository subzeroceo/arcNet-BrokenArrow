...
============
*/
void ARCEncodeBase64::Encode( const anString &src ) {
...
============
*/
void ARCEncodeBase64::Decode( anString &dest ) const {
...
============
*/
void ARCEncodeBase64::Decode( anFile *dest ) const {
	byte *buf = new byte[ DecodeLength()+1 ]; // +1 for trailing \0
	int out = Decode( buf );
	dest->Write( buf, out );
	delete[] buf;
...
void idBase64_TestBase64() {
	...
	anLibrary::common->Printf( "arcDictionary encoded to %s\n", dest.c_str() );
	...
	anLibrary::common->Printf( "arcDictionary reconstructed after base64 decode\n" );
	dest_dict.Print();

	// test arcDictionary read from file - from python generated files, see arcDictionary.py
	anFile *file = anLibrary::fileSystem->OpenFileRead( "arcDictionary.test" );
	if ( file ) {
		arcDictionary test_dict;
		test_dict.ReadFromFileHandle( file );

		anLibrary::common->Printf( "read arcDictionary.test:\n" );
		test_dict.Print();
		anLibrary::fileSystem->CloseFile( file );
		file = NULL;
	} else {
		anLibrary::common->Printf( "arcDictionary.test not found\n" );
	}

	ARCEncodeBase64 base64_src;
	void *buffer;
	if ( anLibrary::fileSystem->ReadFile( "arcDictionary.base64.test", &buffer ) != -1 ) {
		...
		anLibrary::common->Printf( "read arcDictionary.base64.test:\n" );
		test_dict.Print();
		anLibrary::fileSystem->FreeFile( buffer );
	} else {
		anLibrary::common->Printf( "arcDictionary.base64.test not found\n" );
	}
}
