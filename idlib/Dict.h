...
public:
	const arcNetString &		GetKey( void ) const { return *key; }
	const arcNetString &		GetValue( void ) const { return *value; }
...
public:
	...
	void				SetVector( const char *key, const arcVec3 &val );
	void				SetVec2( const char *key, const arcVec2 &val );
	void				SetVec4( const char *key, const arcVec4 &val );
	void				SetAngles( const char *key, const arcAngles &val );
	void				SetMatrix( const char *key, const arcMat3 &val );
	...
	arcVec3				GetVector( const char *key, const char *defaultString = NULL ) const;
	arcVec2				GetVec2( const char *key, const char *defaultString = NULL ) const;
	arcVec4				GetVec4( const char *key, const char *defaultString = NULL ) const;
	arcAngles			GetAngles( const char *key, const char *defaultString = NULL ) const;
	arcMat3				GetMatrix( const char *key, const char *defaultString = NULL ) const;

	bool				GetString( const char *key, const char *defaultString, const char **out ) const;
	bool				GetString( const char *key, const char *defaultString, arcNetString &out ) const;
	...
	bool				GetVector( const char *key, const char *defaultString, arcVec3 &out ) const;
	bool				GetVec2( const char *key, const char *defaultString, arcVec2 &out ) const;
	bool				GetVec4( const char *key, const char *defaultString, arcVec4 &out ) const;
	bool				GetAngles( const char *key, const char *defaultString, arcAngles &out ) const;
	bool				GetMatrix( const char *key, const char *defaultString, arcMat3 &out ) const;
	...
	void				WriteToFileHandle( arcNetFile *f ) const;
	void				ReadFromFileHandle( arcNetFile *f );
	...
	static void			ListKeys_f( const arcCommandArgs &args );
	static void			ListValues_f( const arcCommandArgs &args );

private:
	arcNetList<idKeyValue>	args;
	ARCHashIndex			argHash;

	static arcStringPool	globalKeys;
	static arcStringPool	globalValues;
};
...
ARC_INLINE void arcDictionary::SetVector( const char *key, const arcVec3 &val ) {
	Set( key, val.ToString() );
}

ARC_INLINE void arcDictionary::SetVec4( const char *key, const arcVec4 &val ) {
	Set( key, val.ToString() );
}

ARC_INLINE void arcDictionary::SetVec2( const char *key, const arcVec2 &val ) {
	Set( key, val.ToString() );
}
...
ARC_INLINE void arcDictionary::SetMatrix( const char *key, const arcMat3 &val ) {
	Set( key, val.ToString() );
}
...
ARC_INLINE bool arcDictionary::GetString( const char *key, const char *defaultString, arcNetString &out ) const {
	const idKeyValue *kv = FindKey( key );
	if ( kv ) {
		out = kv->GetValue();
		return true;
...
ARC_INLINE arcVec3 arcDictionary::GetVector( const char *key, const char *defaultString ) const {
	arcVec3 out;
	GetVector( key, defaultString, out );
	return out;
}

ARC_INLINE arcVec2 arcDictionary::GetVec2( const char *key, const char *defaultString ) const {
	arcVec2 out;
	GetVec2( key, defaultString, out );
	return out;
}

ARC_INLINE arcVec4 arcDictionary::GetVec4( const char *key, const char *defaultString ) const {
	arcVec4 out;
...
ARC_INLINE arcMat3 arcDictionary::GetMatrix( const char *key, const char *defaultString ) const {
	arcMat3 out;
