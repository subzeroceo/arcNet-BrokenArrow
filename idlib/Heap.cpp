...
void aRcHeap::Free( void *p ) {
	...
	switch( ( ( byte * )( p ) )[-1] ) {
		...
		default: {
			anLibrary::common->FatalError( "aRcHeap::Free: invalid memory block (%s)", anLibrary::sys->GetCallStackCurStr( 4 ) );
			break;
		}
	}
#endif
...
dword aRcHeap::Msize( void *p ) {
	...
	switch( ( ( byte * )( p ) )[-1] ) {
		...
		default: {
			anLibrary::common->FatalError( "aRcHeap::Msize: invalid memory block (%s)", anLibrary::sys->GetCallStackCurStr( 4 ) );
			return 0;
		}
	}
#endif
