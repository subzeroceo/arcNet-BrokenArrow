...
public:
	...
	void					SetPlane( const arcPlane &p ) { plane = p; }
	void					SetTextureMatrix( const arcVec3 mat[2] ) { texMat[0] = mat[0]; texMat[1] = mat[1]; }
	void					GetTextureMatrix( arcVec3 &mat1, arcVec3 &mat2 ) { mat1 = texMat[0]; mat2 = texMat[1]; }
	void					GetTextureVectors( arcVec4 v[2] ) const;
...
public:
	...
	static TectMapBrush *	Parse( arcLexer &src, const arcVec3 &origin, bool newFormat = true, float version = CURRENT_MAP_VERSION );
	static TectMapBrush *	ParseQ3( arcLexer &src, const arcVec3 &origin );
	bool					Write( arcNetFile *fp, int primitiveNum, const arcVec3 &origin ) const;
...
protected:
	int						numSides;
	arcNetList<aRcMapBrushSides*> sides;
};
...
public:
							...
							~aRcMapPatch( void ) { }
	static aRcMapPatch *	Parse( arcLexer &src, const arcVec3 &origin, bool patchDef3 = true, float version = CURRENT_MAP_VERSION );
	bool					Write( arcNetFile *fp, int primitiveNum, const arcVec3 &origin ) const;
	const char *			GetMaterial( void ) const { return material; }
	void					SetMaterial( const char *p ) { material = p; }
...
public:
	...
	static aRcMapEnt *		Parse( arcLexer &src, bool worldSpawn = false, float version = CURRENT_MAP_VERSION );
	bool					Write( arcNetFile *fp, int entityNum ) const;
...
protected:
	arcNetList<TectMapPrim*>	primitives;
};
