...
class arcDynamicAlloc {
public:
	type *							GetMemory( void ) const { return (type *)( ( ( byte * ) this ) + sizeof( arcDynamicAlloc<type> ) ); }
	...
	arcDynamicAlloc<type> *			prev;					// previous memory block
	arcDynamicAlloc<type> *			next;					// next memory block
	arcBTreeNode<arcDynamicAlloc<type>,int> *node;			// node in the B-Tree with free blocks
};
...
private:
	arcDynamicAlloc<type> *			firstBlock;				// first block in list in order of increasing address
	arcDynamicAlloc<type> *			lastBlock;				// last block in list in order of increasing address
	arcBTree<arcDynamicAlloc<type>,int,4>freeTree;			// B-Tree with free memory blocks
	...
	void							Clear( void );
	arcDynamicAlloc<type> *			AllocInternal( const int num );
	arcDynamicAlloc<type> *			ResizeInternal( arcDynamicAlloc<type> *block, const int num );
	void							FreeInternal( arcDynamicAlloc<type> *block );
	void							LinkFreeInternal( arcDynamicAlloc<type> *block );
	void							UnlinkFreeInternal( arcDynamicAlloc<type> *block );
...
void arcDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::Shutdown( void ) {
	...
	for ( block = firstBlock; block != NULL; block = firstBlock ) {
		...
		if ( lockMemory ) {
			anLibrary::sys->UnlockMemory( block, block->GetSize() + ( int )sizeof( arcDynamicAlloc<type> ) );
		}
		Mem_Free16( block );
	}
...
void arcDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::SetFixedBlocks( int numBlocks ) {
	...
	for ( int i = numBaseBlocks; i < numBlocks; i++ ) {
		block = ( arcDynamicAlloc<type> * ) Mem_Alloc16( baseBlockSize );
		if ( lockMemory ) {
...
#endif
		block->SetSize( baseBlockSize - ( int )sizeof( arcDynamicAlloc<type> ), true );
		block->next = NULL;
...
void arcDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::FreeEmptyBaseBlocks( void ) {
	arcDynamicAlloc<type> *block, *next;
	...
	for ( block = firstBlock; block != NULL; block = next ) {
		...
		if ( block->IsBaseBlock() && block->node != NULL && ( next == NULL || next->IsBaseBlock() ) ) {
			...
			numBaseBlocks--;
			baseBlockMemory -= block->GetSize() + ( int )sizeof( arcDynamicAlloc<type> );
			Mem_Free16( block );
		}
	}
...
type *arcDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::Resize( type *ptr, const int num ) {
	...
	arcDynamicAlloc<type> *block = ( arcDynamicAlloc<type> * ) ( ( ( byte * ) ptr ) - ( int )sizeof( arcDynamicAlloc<type> ) );
...
const char *arcDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::CheckMemory( const type *ptr ) const {
	...
	block = ( arcDynamicAlloc<type> * ) ( ( ( byte * ) ptr ) - ( int )sizeof( arcDynamicAlloc<type> ) );
...
template<class type, int baseBlockSize, int minBlockSize>
arcDynamicAlloc<type> *arcDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::AllocInternal( const int num ) {
	arcDynamicAlloc<type> *block;
	...
	if ( block != NULL ) {
		...
		int allocSize = Max( baseBlockSize, alignedBytes + ( int )sizeof( arcDynamicAlloc<type> ) );
		block = ( arcDynamicAlloc<type> * ) Mem_Alloc16( allocSize );
		if ( lockMemory ) {
			anLibrary::sys->LockMemory( block, baseBlockSize );
		}
#ifdef DYNAMIC_BLOCK_ALLOC_CHECK
		...
		block->allocator = (void*)this;
#endif
		block->SetSize( allocSize - ( int )sizeof( arcDynamicAlloc<type> ), true );
		block->next = NULL;
		block->prev = lastBlock;
		if ( lastBlock ) {
			lastBlock->next = block;
...
template<class type, int baseBlockSize, int minBlockSize>
arcDynamicAlloc<type> *arcDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::ResizeInternal( arcDynamicAlloc<type> *block, const int num ) {
	int alignedBytes = ( num * sizeof( type ) + 15 ) & ~15;
	...
	if ( alignedBytes > block->GetSize() ) {
		...
		arcDynamicAlloc<type> *nextBlock = block->next;

		// try to annexate the next block if it's free
		if ( nextBlock && !nextBlock->IsBaseBlock() && nextBlock->node != NULL &&
				block->GetSize() + ( int )sizeof( arcDynamicAlloc<type> ) + nextBlock->GetSize() >= alignedBytes ) {

			UnlinkFreeInternal( nextBlock );
			block->SetSize( block->GetSize() + ( int )sizeof( arcDynamicAlloc<type> ) + nextBlock->GetSize(), block->IsBaseBlock() );
			...
			arcDynamicAlloc<type> *oldBlock = block;
			block = AllocInternal( num );
			if ( block == NULL ) {
				return NULL;
			}
	...
	if ( block->GetSize() - alignedBytes - ( int )sizeof( arcDynamicAlloc<type> ) < Max( minBlockSize, ( int )sizeof( type ) ) ) {
		return block;
	}

	arcDynamicAlloc<type> *newBlock;

	newBlock = ( arcDynamicAlloc<type> * ) ( ( ( byte * ) block ) + ( int )sizeof( arcDynamicAlloc<type> ) + alignedBytes );
#ifdef DYNAMIC_BLOCK_ALLOC_CHECK
	memcpy( newBlock->id, blockId, sizeof( newBlock->id ) );
	newBlock->allocator = (void*)this;
#endif
	newBlock->SetSize( block->GetSize() - alignedBytes - ( int )sizeof( arcDynamicAlloc<type> ), false );
...
template<class type, int baseBlockSize, int minBlockSize>
void arcDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::FreeInternal( arcDynamicAlloc<type> *block ) {
	...
	arcDynamicAlloc<type> *prevBlock = block->prev;
	if ( prevBlock && !block->IsBaseBlock() && prevBlock->node != NULL ) {
		UnlinkFreeInternal( prevBlock );
		prevBlock->SetSize( prevBlock->GetSize() + ( int )sizeof( arcDynamicAlloc<type> ) + block->GetSize(), prevBlock->IsBaseBlock() );
...
template<class type, int baseBlockSize, int minBlockSize>
ARC_INLINE void arcDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::LinkFreeInternal( arcDynamicAlloc<type> *block ) {
...
template<class type, int baseBlockSize, int minBlockSize>
ARC_INLINE void arcDynamicBlockAlloc<type, baseBlockSize, minBlockSize>::UnlinkFreeInternal( arcDynamicAlloc<type> *block ) {
