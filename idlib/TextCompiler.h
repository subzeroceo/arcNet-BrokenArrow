...
namespace TextCompiler {
	...
	// specialization write for anString's
	template <> inline void WriteValue( anString const * const ptr, anFile *out, bool byteSwap ) {
	...
	// specialization read for anString's
	template <> inline anString ReadValue( anFile *in ) {
		char c;
		anString str;
