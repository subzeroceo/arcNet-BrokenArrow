...
================
*/
void ARCParser::PushScript( anLexer *script ) {
	anLexer *s;
...
int ARCParser::ReadSourceToken( arcNetToken *token ) {
	...
	if ( !ARCParser::scriptStack ) {
		anLibrary::common->FatalError( "ARCParser::ReadSourceToken: not loaded" );
		return false;
	}
	changedScript = 0;
...
=================
*/
const char *ARCParser::ParseBracedSectionExact( anString &out, int tabs ) {
...
=================
*/
const char *ARCParser::ParseBracedSection( anString &out, int tabs ) {
	arcNetToken token;
	int i, depth;
	bool doTabs = false;
	if (tabs >= 0 ) {
...
=================
*/
const char *ARCParser::ParseRestOfLine( anString &out ) {
	arcNetToken token;
...
================
*/
int ARCParser::GetLastWhiteSpace( anString &whiteSpace ) const {
	if ( scriptStack ) {
...
================
*/
void ARCParser::GetStringFromMarker( anString& out, bool clean ) {
	char*	p;
	char	save;
...
int ARCParser::LoadFile( const char *filename, bool OSPath ) {
	...
	if ( ARCParser::loaded ) {
		anLibrary::common->FatalError( "ARCParser::loadFile: another source already loaded" );
		return false;
	}
...
int ARCParser::LoadMemory(const char *ptr, int length, const char *name ) {
	...
	if ( ARCParser::loaded ) {
		anLibrary::common->FatalError( "ARCParser::loadMemory: another source already loaded" );
		return false;
	}
