...
void R_ListRenderLightDefs_f( const anCommandArgs &args ) {
	...
	for ( i = 0; i < tr.primaryWorld->lightDefs.Num(); i++ ) {
		...
		int	iCount = 0;
		for ( an Interaction *inter = ldef->firstInteraction; inter != NULL; inter = inter->lightNext ) {
			iCount++;
		}
		totalIntr += iCount;
...
void R_ListRenderEntityDefs_f( const anCommandArgs &args ) {
	...
	for ( i = 0; i < tr.primaryWorld->entityDefs.Num(); i++ ) {
		...
		// count up the interactions
		int	iCount = 0;
		for ( an Interaction *inter = mdef->firstInteraction; inter != NULL; inter = inter->entityNext ) {
			iCount++;
		}
		totalIntr += iCount;
...
void anRenderWorldLocal::UpdateEntityDef( arcNetHandle_t entityHandle, const renderEntity_t *re ) {
	...
	if ( !re->hModel && !re->callback ) {
		common->Error( "anRenderWorld::UpdateEntityDef: NULL hModel" );
	}
...
void anRenderWorldLocal::FreeEntityDef( arcNetHandle_t entityHandle ) {
	...
	anRenderEntityLocal	*def = entityDefs[entityHandle];
	if ( !def ) {
		common->Printf( "anRenderWorld::FreeEntityDef: handle %i is NULL\n", entityHandle );
		return;
	}
...
const renderEntity_t *anRenderWorldLocal::GetRenderEntity( arcNetHandle_t entityHandle ) const {
	...
	anRenderEntityLocal	*def = entityDefs[entityHandle];
	if ( !def ) {
		common->Printf( "anRenderWorld::GetRenderEntity: handle %i is NULL\n", entityHandle );
		return NULL;
	}
...
void anRenderWorldLocal::FreeLightDef( arcNetHandle_t lightHandle ) {
	...
	anRenderLightsLocal	*light = lightDefs[lightHandle];
	if ( !light ) {
		common->Printf( "anRenderWorld::FreeLightDef: handle %i is NULL\n", lightHandle );
		return;
	}
...
const renderLight_t *anRenderWorldLocal::GetRenderLight( arcNetHandle_t lightHandle ) const {
	...
	if ( !def ) {
		common->Printf( "anRenderWorld::GetRenderLight: handle %i is NULL\n", lightHandle );
		return NULL;
	}
...
================
*/
void anRenderWorldLocal::ProjectDecalOntoWorld( const arcFixedWinding &winding, const anVec3 &projectionOrigin, const bool parallel, const float fadeDepth, const anMaterial *material, const int startTime ) {
...
====================
*/
void anRenderWorldLocal::ProjectDecal( arcNetHandle_t entityHandle, const arcFixedWinding &winding, const anVec3 &projectionOrigin, const bool parallel, const float fadeDepth, const anMaterial *material, const int startTime ) {
	decalProjectionInfo_t info, localInfo;
...
guiPoint_t	anRenderWorldLocal::GuiTrace( arcNetHandle_t entityHandle, const anVec3 start, const anVec3 end ) const {
	...
	anRenderEntityLocal *def = entityDefs[entityHandle];
	if ( !def ) {
		common->Printf( "anRenderWorld::GuiTrace: handle %i is NULL\n", entityHandle );
		return pt;
	}
...
void anRenderWorldLocal::AddEntityRefToArea( anRenderEntityLocal *def, portalArea_t *area ) {
	if ( !def ) {
		common->Error( "anRenderWorldLocal::AddEntityRefToArea: NULL def" );
	}
...
==============
*/
void anRenderWorldLocal::PushVolumeIntoTree( anRenderEntityLocal *def, anRenderLightsLocal *light, int numPoints, const anVec3 (*points) ) {	if ( areaNodes == NULL ) {
		return;
	}
...
===============
*/
const anMaterial *R_RemapShaderBySkin( const anMaterial *shader, const arcDeclSkin *skin, const anMaterial *customShader ) {
	if ( !shader ) {
		return NULL;
	}
