...
void arcInteractiveGuiModel::EmitSurface( guiModelSurface_t *surf, float modelMatrix[16], float modelViewMatrix[16], bool depthHack ) {
	...
	guiSpace->ViewDepthHack = depthHack;
...
=============
*/
void arcInteractiveGuiModel::DrawStretchPic( const anDrawVertex *dverts, const qglIndex_t *dindexes, int vertCount, int indexCount, const anMaterial *hShader, bool clip, float min_x, float min_y, float max_x, float max_y ) {
	if ( !qglConfig.isInitialized ) {
		return;
	}
...
=============
*/
void arcInteractiveGuiModel::DrawStretchPic( float x, float y, float w, float h, float s1, float t1, float s2, float t2, const anMaterial *hShader ) {
	...
	if ( r_useImprovedStretchPicCode ) {
		for ( int i = 0; i < 4; i++ ) {
		    verts[i].xyz[0] = (i % 2 == 0) ? x : x + w;
		    verts[i].xyz[1] = (i < 2) ? y : y + h;
		    verts[i].xyz[2] = 0;

		    verts[i].st[0] = (i % 2 == 0) ? s1 : s2;
		    verts[i].st[1] = (i < 2) ? t1 : t2;
...
=============
*/
void arcInteractiveGuiModel::DrawStretchTri( anVec2 p1, anVec2 p2, anVec2 p3, anVec2 t1, anVec2 t2, anVec2 t3, const anMaterial *material ) {
	anDrawVertex tempVerts[3];
	qglIndex_t tempIndexes[3];
	int vertCount = 3;
	int indexCount = 3;

	if ( !qglConfig.isInitialized || !material ) {
		return;
	}
