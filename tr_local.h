...
typedef struct drawSurf_s {
	...
	const anMaterial		*material;			// may be NULL for shadow volumes
...
typedef struct areaReference_s {
	...
	anRenderEntityLocal *	entity;					// only one of entity / light will be non-NULL
	anRenderLightsLocal *	light;					// only one of entity / light will be non-NULL
	struct portalArea_s	*	area;					// so owners can find all the areas they are in
} areaReference_t;

// ARCRenderLights should become the new public interface replacing the arcNetHandle_t to light defs in the anRenderWorld interface
...
// ARCRenderEntity should become the new public interface replacing the arcNetHandle_t to entity defs in the anRenderWorld interface
...
class anRenderLightsLocal : public ARCRenderLights {
public:
							anRenderLightsLocal();
...
class anRenderEntityLocal : public ARCRenderEntity {
public:
							anRenderEntityLocal();
...
extern anCVarSystem r_skipRenderContext;		// NULL the rendering context during backend 3D rendering
...
void R_AddDrawSurf( const surfTriangles_t *tri, const viewEntity_t *space, const renderEntity_t *renderEntity, const anMaterial *shader, const anScreenRect &scissor );

void R_LinkLightSurf( const drawSurf_t **link, const surfTriangles_t *tri, const viewEntity_t *space, const anRenderLightsLocal *light, const anMaterial *shader, const anScreenRect &scissor, bool viewInsideShadow );

bool R_CreateAmbientCache( surfTriangles_t *tri, bool needsLighting );
bool R_CreateLightingCache( const anRenderEntityLocal *ent, const anRenderLightsLocal *light, surfTriangles_t *tri );
...
localTrace_t R_LocalTrace( const anVec3 &start, const anVec3 &end, const float radius, const surfTriangles_t *tri );
