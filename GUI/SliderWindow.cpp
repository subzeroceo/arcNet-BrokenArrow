...
idSliderWindow::idSliderWindow(idDeviceContext *d, anUserInterface *g) : idWindow(d, g) {
	dc = d;
	gui = g;
	CommonInit();
}

idSliderWindow::idSliderWindow(anUserInterface *g) : idWindow(g) {
	gui = g;
	CommonInit();
}
...
void idSliderWindow::InitWithDefaults(const char *_name, const idRectangle &_rect, const anVec4 &_foreColor, const anVec4 &_matColor, const char *_background, const char *thumbShader, bool _vertical, bool _scrollbar) {
...
void idSliderWindow::Draw( inttime, float x, float y) {
