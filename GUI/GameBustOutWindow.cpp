...
======================
*/
void BOEntity::WriteToSaveGame( anFile *savefile ) {
...
======================
*/
void BOEntity::ReadFromSaveGame( anFile *savefile, idGameBustOutWindow* _game ) {
	game = _game;
...
======================
*/
void BOBrick::WriteToSaveGame( anFile *savefile ) {
...
======================
*/
void BOBrick::ReadFromSaveGame( anFile *savefile, idGameBustOutWindow *game ) {
...
======================
*/
void BOBrick::SetColor( anVec4 bcolor ) {
...
======================
*/
collideDir_t BOBrick::checkCollision( anVec2 pos, anVec2 vel ) {
	anVec2	ptA, ptB;
	float	dist;
	...
	if ( vel.y < 0 && pos.y > ptA.y ) {
		if ( pos.x > ptA.x && pos.x < ptB.x ) {
			...
			if ( (anMath::Fabs(vec.y) > anMath::Fabs(vec.x)) && (vec.LengthFast() < BALL_RADIUS) ) {
				result = COLLIDE_DOWN;
			}
		}
	}
		...
		if ( result == COLLIDE_NONE ) {
			...
			if ( vel.x > 0 && pos.x < ptA.x ) {
				if ( pos.y > ptA.y && pos.y < ptB.y ) {
					...
					if ( (anMath::Fabs(vec.x) >= anMath::Fabs(vec.y)) && (vec.LengthFast() < BALL_RADIUS) ) {
						result = COLLIDE_LEFT;
					}
				}
			}
...
****************************************************************************
*/
idGameBustOutWindow::idGameBustOutWindow(idDeviceContext *d, anUserInterface *g) : idWindow(d, g) {
	dc = d;
	gui = g;
	CommonInit();
}

idGameBustOutWindow::idGameBustOutWindow(anUserInterface *g) : idWindow(g) {
	gui = g;
	CommonInit();
}
...
=============================
*/
void idGameBustOutWindow::WriteToSaveGame( anFile *savefile ) {
...
=============================
*/
void idGameBustOutWindow::ReadFromSaveGame( anFile *savefile ) {
...
=============================
*/
void idGameBustOutWindow::Draw( inttime, float x, float y) {
	int i;
...
void idGameBustOutWindow::UpdateBall( void ) {
	...
	for ( ballnum = 0; ballnum < balls.Num(); ballnum++ ) {
		...
		anVec2 ballCenter = ball->position + anVec2( BALL_RADIUS, BALL_RADIUS );
		collideDir_t collision = paddle->checkCollision( ballCenter, ball->velocity );

		if ( collision == COLLIDE_UP ) {
			if ( ball->velocity.y > 0 ) {
				anVec2	paddleVec( paddleVelocity*2, 0 );
				float	centerX;
