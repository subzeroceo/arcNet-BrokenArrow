...
idMarkerWindow::idMarkerWindow(idDeviceContext *d, anUserInterface *g) : idWindow(d, g) {
	dc = d;
	gui = g;
	CommonInit();
}

idMarkerWindow::idMarkerWindow(anUserInterface *g) : idWindow(g) {
	gui = g;
	CommonInit();
}
...
void idMarkerWindow::Draw( inttime, float x, float y) {
	float pct;
...
void idMarkerWindow::Point( intx, int y, dword *out, dword color) {
...
void idMarkerWindow::Line( intx1, int y1, int x2, int y2, dword* out, dword color) {
