...
public:
	...
	void SetGuiInfo(anDict *gd, const char *_name);
	...
	anDict *GetDict() const { return guiDict; }
...
public:
	...
	virtual void Init(const char *_name, idWindow *win) {
		idWinVar::Init(_name, win);
		if (guiDict) {
			anVec4 v = guiDict->GetVec4(GetName());
			data.x = v.x;
			data.y = v.y;
			data.w = v.z;
			data.h = v.w;
	...
	idRectangle &operator=(	const anVec4 &other ) {
		data = other;
		if (guiDict) {
...
public:
	...
	int	operator==(	const anVec2 &other ) const {
		return (other == data);
	}
	...
	anVec2 &operator=(	const anVec2 &other ) {
		data = other;
		if (guiDict) {
...
public:
	...
	int	operator==(	const anVec4 &other ) const {
		return (other == data);
	}
	...
	anVec4 &operator=(	const anVec4 &other ) {
		data = other;
		if (guiDict) {
...
public:
	...
	int	operator==(	const anVec3 &other ) const {
		return (other == data);
	}
	...
	anVec3 &operator=(	const anVec3 &other ) {
		data = other;
		if (guiDict) {
...
public:
	...
	void SetMaterialPtr( const anMaterial **m ) {
		mat = m;
	}
...
================
*/
class idMultiWinVar : public anList< idWinVar * > {
public:
	void Set( const char *val );
	void Update( void );
	void SetGuiInfo( anDict *dict );
};
