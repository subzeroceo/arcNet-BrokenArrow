...
struct idTransitionData {
	idWinVar *data;
	int	offset;
	idInterpolateAccelDecelLinear<anVec4> interp;
};
...
class idWindow {
public:
	idWindow(anUserInterface *gui);
	idWindow(idDeviceContext *d, anUserInterface *gui);
	virtual ~idWindow();
	...
	static const anRegEntry RegisterVars[];
	...
	anUserInterface *GetGui() {return gui;};
	...
	virtual void ReadFromDemoFile( class anSavedGamesFile *f, bool rebuild = true );
	...
	void AddTransition(idWinVar *dest, anVec4 from, anVec4 to, int time, float accelTime, float decelTime);
	void ResetTime( inttime);
	void ResetCinematics();

	int NumTransitions();

	bool ParseScript(anParser *src, idGuiScriptList &list, int *timeParm = nullptr, bool allowIf = false);
	bool RunScript( intn);
	bool RunScriptList(idGuiScriptList *src);
	void SetRegs(const char *key, const char *val);
	int ParseExpression( anParser *src, idWinVar *var = nullptr, int component = 0 );
	int ExpressionConstant(float f);
	anRegisterList *RegList() { return &regList; }
...
protected:
	...
	void Transition();
	void Time();
	bool RunTimeEvents( inttime);
	void Dump();

	int ExpressionTemporary();
	wexpOp_t *ExpressionOp();
	int EmitOp( int a, int b, wexpOpType_t opType, wexpOp_t **opp = nullptr );
	int ParseEmitOp( anParser *src, int a, wexpOpType_t opType, int priority, wexpOp_t **opp = nullptr );
	int ParseTerm( anParser *src, idWinVar *var = nullptr, int component = 0 );
	int ParseExpressionPriority( anParser *src, int priority, idWinVar *var = nullptr, int component = 0 );
	void EvaluateRegisters(float *registers);
	void SaveExpressionParseState();
	void RestoreExpressionParseState();
	void ParseBracedExpression(anParser *src);
	...
	anList<idWinVar*> definedVars;
	anList<idWinVar*> updateVars;

	idRectangle textRect;			// text extented rect
	const anMaterial *background;         // background asset  

	idWindow *parent;				// parent window
	anList<idWindow*> children;		// child windows	
	anList<drawWin_t> drawWindows;		
	...
	static anCVar gui_debug;
	static anCVar gui_edit;

	idGuiScriptList *scripts[SCRIPT_COUNT];
	bool *saveTemps;

	anList<anTimeLineEvent*> timeLineEvents;
	anList<idTransitionData> transitions;

	static bool registerIsTemporary[MAX_EXPRESSION_REGISTERS]; // statics to assist during parsing

	anList<wexpOp_t> ops;			   	// evaluate to make expressionRegisters
	anList<float> expressionRegisters;
	anList<wexpOp_t> *saveOps;			   	// evaluate to make expressionRegisters
	anList<rvNamedEvent*>		namedEvents;		//  added named events
	anList<float> *saveRegs;

	anRegisterList regList;
