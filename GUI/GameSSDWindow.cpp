...
void SSDCrossHair::WriteToSaveGame( anFile *savefile ) {
...
void SSDCrossHair::ReadFromSaveGame( anFile *savefile ) {
	InitCrosshairs();
...
void SSDCrossHair::Draw(idDeviceContext *dc, const anVec2& cursor) {
	float x,y;
...
void SSDEntity::WriteToSaveGame( anFile *savefile ) {
...
void SSDEntity::ReadFromSaveGame( anFile *savefile,  idGameSSDWindow* _game ) {
...
void SSDEntity::SetPosition(const anVec3& _position) {
	position = _position;
}

void SSDEntity::SetSize(const anVec2& _size) {
	size = _size;
}
...
bool SSDEntity::HitTest(const anVec2& pt) {

	if ( noHi t) {
		return false;
	}

	anVec3 screenPos = WorldToScreen(position);
	...
	anVec2 diff = screenPos.ToVec2()-pt;
	float dist = anMath::Fabs(diff.LengthSqr());
...
void SSDEntity::Draw(idDeviceContext *dc) {
	...
	anBounds bounds;
	bounds[0] = anVec3(position.x - ( size.x/2.0f), position.y - ( size.y/2.0f), position.z);
	bounds[1] = anVec3(position.x + ( size.x/2.0f), position.y + ( size.y/2.0f), position.z);

	anBounds screenBounds = WorldToScreen(bounds);
	persize.x = anMath::Fabs( screenBounds[1].x - screenBounds[0].x);
	persize.y = anMath::Fabs( screenBounds[1].y - screenBounds[0].y);

	anVec3 center = screenBounds.GetCenter();
...
anBounds SSDEntity::WorldToScreen(const anBounds worldBounds) {
	anVec3 screenMin = WorldToScreen(worldBounds[0]);
	anVec3 screenMax = WorldToScreen(worldBounds[1]);
	anBounds screenBounds( screenMin, screenMax);
	return screenBounds;
}

anVec3 SSDEntity::WorldToScreen(const anVec3& worldPos) {
	float d = 0.5f*V_WIDTH*anMath::Tan(DEG2RAD(90.0f)/2.0f);

	//World To Camera Coordinates
	anVec3 cameraTrans(0,0,d);
	anVec3 cameraPos;
...
anVec3 SSDEntity::ScreenToWorld( const anVec3& screenPos ) {
	anVec3 worldPos;
...
void SSDMover::MoverInit( const anVec3& _speed, float _rotationSpeed) {
	speed = _speed;
...
void SSDMover::EntityUpdate() {
	...
	anVec3 moved = ( ( float )elapsed/1000.0f)*speed;
	position += moved;
...
void SSDAsteroid::WriteToSaveGame( anFile *savefile ) {
	SSDMover::WriteToSaveGame( savefile);
	savefile->Write(&health, sizeof(health));
}

void SSDAsteroid::ReadFromSaveGame( anFile *savefile,  idGameSSDWindow* _game  ) {
	SSDMover::ReadFromSaveGame( savefile, _game);
	savefile->Read(&health, sizeof(health));
}

void SSDAsteroid::Init(idGameSSDWindow* _game, const anVec3& startPosition, const anVec2& _size, float _speed, float rotate, int _health) {
	EntityInit();
	MoverInit(anVec3(0,0, -_speed), rotate);
...
SSDAsteroid* SSDAsteroid::GetNewAsteroid(idGameSSDWindow* _game, const anVec3& startPosition, const anVec2& _size, float _speed, float rotate, int _health) {
...
void SSDAsteroid::WriteAsteroids(anFile* savefile) {
	int count = 0;
	for ( int i = 0; i < MAX_ASTEROIDS; i++ ) {
		if (asteroidPool[i].inUse) {
			count++;
...
void SSDAsteroid::ReadAsteroids(anFile* savefile, idGameSSDWindow* _game) {
	int count;
...
void SSDAstronaut::WriteToSaveGame( anFile *savefile ) {
	SSDMover::WriteToSaveGame( savefile);
	savefile->Write(&health, sizeof(health));
}

void SSDAstronaut::ReadFromSaveGame( anFile *savefile,  idGameSSDWindow* _game  ) {
	SSDMover::ReadFromSaveGame( savefile, _game);
	savefile->Read(&health, sizeof(health));
}

void SSDAstronaut::Init(idGameSSDWindow* _game, const anVec3& startPosition, float _speed, float rotate, int _health) {
	EntityInit();
	MoverInit(anVec3(0,0, -_speed), rotate);
...
SSDAstronaut* SSDAstronaut::GetNewAstronaut(idGameSSDWindow* _game, const anVec3& startPosition, float _speed, float rotate, int _health) {
...
void SSDAstronaut::WriteAstronauts(anFile* savefile) {
	int count = 0;
	for ( int i = 0; i < MAX_ASTRONAUT; i++ ) {
		if (astronautPool[i].inUse) {
			count++;
...
void SSDAstronaut::ReadAstronauts(anFile* savefile, idGameSSDWindow* _game) {
...
void SSDExplosion::WriteToSaveGame( anFile *savefile ) {
	SSDEntity::WriteToSaveGame( savefile);
...
void SSDExplosion::ReadFromSaveGame( anFile *savefile,  idGameSSDWindow* _game  ) {
	SSDEntity::ReadFromSaveGame( savefile, _game);
...
void SSDExplosion::Init(idGameSSDWindow* _game, const anVec3& _position, const anVec2& _size, int _length, int _type, SSDEntity* _buddy, bool _killBuddy, bool _followBuddy) {
	EntityInit();
...
SSDExplosion* SSDExplosion::GetNewExplosion(idGameSSDWindow* _game, const anVec3& _position, const anVec2& _size, int _length, int _type, SSDEntity* _buddy, bool _killBuddy, bool _followBuddy) {
...
void SSDExplosion::WriteExplosions(anFile* savefile) {
	int count = 0;
	for ( int i = 0; i < MAX_EXPLOSIONS; i++ ) {
		if (explosionPool[i].inUse) {
			count++;
...
void SSDExplosion::ReadExplosions(anFile* savefile, idGameSSDWindow* _game) {
	int count;
	savefile->Read(&count, sizeof(count));
	for ( int i = 0; i < count; i++ ) {
		int id;
...
void SSDPoints::WriteToSaveGame( anFile *savefile ) {
	SSDEntity::WriteToSaveGame( savefile);
...
void SSDPoints::ReadFromSaveGame( anFile *savefile,  idGameSSDWindow* _game  ) {
	SSDEntity::ReadFromSaveGame( savefile, _game);
...
void SSDPoints::Init(idGameSSDWindow* _game, SSDEntity* _ent, int _points, int _length, int _distance, const anVec4& color) {
...
SSDPoints* SSDPoints::GetNewPoints(idGameSSDWindow* _game, SSDEntity* _ent, int _points, int _length, int _distance, const anVec4& color) {
...
void SSDPoints::WritePoints(anFile* savefile) {
	int count = 0;
	for ( int i = 0; i < MAX_POINTS; i++ ) {
		if (pointsPool[i].inUse) {
			count++;
...
void SSDPoints::ReadPoints(anFile* savefile, idGameSSDWindow* _game) {
...
void SSDProjectile::WriteToSaveGame( anFile *savefile ) {
	SSDEntity::WriteToSaveGame( savefile);
...
void SSDProjectile::ReadFromSaveGame( anFile *savefile,  idGameSSDWindow* _game  ) {
	SSDEntity::ReadFromSaveGame( savefile, _game);
...
void SSDProjectile::Init(idGameSSDWindow* _game, const anVec3& _beginPosition, const anVec3& _endPosition, float _speed, float _size) {
	EntityInit();
...
void SSDProjectile::EntityUpdate() {
	...
	//Move forward based on speed (units per second)
	anVec3 moved = dir*(( float )elapsed/1000.0f)*speed.z;
	position += moved;
...
SSDProjectile* SSDProjectile::GetNewProjectile(idGameSSDWindow* _game, const anVec3& _beginPosition, const anVec3& _endPosition, float _speed, float _size) {
...
void SSDProjectile::WriteProjectiles(anFile* savefile) {
	int count = 0;
	for ( int i = 0; i < MAX_PROJECTILES; i++ ) {
		if (projectilePool[i].inUse) {
			count++;
...
void SSDProjectile::ReadProjectiles(anFile* savefile, idGameSSDWindow* _game) {
...
void SSDPowerup::WriteToSaveGame( anFile *savefile ) {
	SSDMover::WriteToSaveGame( savefile);
...
void SSDPowerup::ReadFromSaveGame( anFile *savefile,  idGameSSDWindow* _game  ) {
	SSDMover::ReadFromSaveGame( savefile, _game);
...
void SSDPowerup::Init(idGameSSDWindow* _game, float _speed, float _rotation) {
	...
	EntityInit();
	MoverInit(anVec3(0,0, -_speed), _rotation);

	SetGame(_game);
	SetSize(anVec2(200,200));
	SetRadius(Max( size.x, size.y), 0.3f);

	type = SSD_ENTITY_POWERUP;
	
	anVec3 startPosition;
...
void SSDPowerup::WritePowerups(anFile* savefile) {
	int count = 0;
	for ( int i = 0; i < MAX_POWERUPS; i++ ) {
		if (powerupPool[i].inUse) {
			count++;
...
void SSDPowerup::ReadPowerups(anFile* savefile, idGameSSDWindow* _game) {
...
idGameSSDWindow::idGameSSDWindow(idDeviceContext *d, anUserInterface *g) : idWindow(d, g) {
	dc = d;
	gui = g;
	CommonInit();
}

idGameSSDWindow::idGameSSDWindow(anUserInterface *g) : idWindow(g) {
	gui = g;
	CommonInit();
}
...
void idGameSSDWindow::WriteToSaveGame( anFile *savefile ) {
...
void idGameSSDWindow::ReadFromSaveGame( anFile *savefile ) {
...
idWinVar *idGameSSDWindow::GetWinVarByName( const char *_name, bool winLookup, drawWin_t** owner) {
	...
	if (anString::Icmp(_name, "beginLevel" ) == 0) {
		retVar = &beginLevel;
	}

	if (anString::Icmp(_name, "resetGame" ) == 0) {
		retVar = &resetGame;
	}

	if (anString::Icmp(_name, "continueGame" ) == 0) {
		retVar = &continueGame;
	}
	if (anString::Icmp(_name, "refreshGuiData" ) == 0) {
...
void idGameSSDWindow::Draw( inttime, float x, float y) {
...
bool idGameSSDWindow::ParseInternalVar(const char *_name, anParser *src) {
	if (anString::Icmp(_name, "beginLevel" ) == 0) {
	...
	if (anString::Icmp(_name, "levelcount" ) == 0) {
	...
	if (anString::Icmp(_name, "weaponCount" ) == 0) {
	...
	if (anString::FindText(_name, "leveldata", false) >= 0) {
		anString tempName = _name;
	...
	if (anString::FindText(_name, "asteroiddata", false) >= 0) {
		anString tempName = _name;
		int level = atoi(tempName.Right(2))-1;

		anString asteroidData;
		ParseString( src, asteroidData);
		ParseAsteroidData(level, asteroidData);
		return true;
	}

	if (anString::FindText(_name, "weapondata", false) >= 0) {
		anString tempName = _name;
	...
	if (anString::FindText(_name, "astronautdata", false) >= 0) {
		anString tempName = _name;
		int level = atoi(tempName.Right(2))-1;

		anString astronautData;
		ParseString( src, astronautData);
		ParseAstronautData(level, astronautData);
		return true;
	}

	if (anString::FindText(_name, "powerupdata", false) >= 0) {
		anString tempName = _name;
		int level = atoi(tempName.Right(2))-1;

		anString powerupData;
...
void idGameSSDWindow::ParseLevelData( intlevel, const anString& levelDataString) {
	anParser parser;
	anToken token;
...
void idGameSSDWindow::ParseAsteroidData( intlevel, const anString& asteroidDataString) {
	anParser parser;
	anToken token;
...
void idGameSSDWindow::ParsePowerupData( intlevel, const anString& powerupDataString) {
	anParser parser;
	anToken token;
...
void idGameSSDWindow::ParseWeaponData( intweapon, const anString& weaponDataString) {
	anParser parser;
	anToken token;
...
void idGameSSDWindow::ParseAstronautData( intlevel, const anString& astronautDataString) {
...
void idGameSSDWindow::CommonInit() {
	...
	levelCount = 0;
	weaponCount = 0;
	screenBounds = anBounds(anVec3(-320,-240,0), anVec3(320,240,0));
...
void idGameSSDWindow::BeginLevel( intlevel) {
	ResetLevelStats();
	...
	for ( int i = 0; i < entities.Num(); i++ ) {
		...
		if (ent->position.z <= Z_NEAR) {
			...
			if ( !ent->noPlayerDamage) {
				...
				anVec3 entPos = ent->position;
				entPos.z = 0;

				anBounds entBounds(entPos);
				...
				if ( screenBounds.IntersectsBounds(entBounds)) {
					...
					if (ent->type == SSD_ENTITY_ASTEROID) {
						AsteroanStringuckPlayer( static_cast<SSDAsteroid*>(ent));
...
void idGameSSDWindow::SpawnAsteroid() {
	...
	SSDAsteroid* asteroid = SSDAsteroid::GetNewAsteroid(this, startPosition, anVec2( size, size), speed, rotate, asteroidData[gameStats.currentLevel].asteroidHealth);
...
void idGameSSDWindow::FireWeapon( intkey) {
	anVec2 cursorWorld = GetCursorWorld();
	anVec2 cursor;
	//GetCursor(cursor);
	cursor.x = gui->CursorX();
	cursor.y = gui->CursorY();
	...
	if (key == K_MOUSE1) {
		...
		if (gameStats.levelStats.targetEnt) {
			...
			//SSDProjectile* newProj = new SSDProjectile(this, anVec3(320,0,0), gameStats.levelStats.targetEnt->position, weaponData[gameStats.currentWeapon].speed, weaponData[gameStats.currentWeapon].size);
			SSDProjectile* newProj = SSDProjectile::GetNewProjectile(this, anVec3(0,-180,0), gameStats.levelStats.targetEnt->position, weaponData[gameStats.currentWeapon].speed, weaponData[gameStats.currentWeapon].size);
			entities.Append(newProj);
			//newProj = SSDProjectile::GetNewProjectile(this, anVec3(-320,-0,0), gameStats.levelStats.targetEnt->position, weaponData[gameStats.currentWeapon].speed, weaponData[gameStats.currentWeapon].size);
			//entities.Append(newProj);
			...
			//SSDProjectile* newProj = SSDProjectile::GetNewProjectile(this, anVec3(0,-180,0), anVec3(cursorWorld.x, cursorWorld.y, (Z_FAR-Z_NEAR)/2.0f), weaponData[gameStats.currentWeapon].speed, weaponData[gameStats.currentWeapon].size);

			//Aim the projectile so it crosses the cursor 1/4 of screen
			anVec3 vec = anVec3(cursorWorld.x, cursorWorld.y, (Z_FAR-Z_NEAR)/8.0f);
			vec *= 8;
			SSDProjectile* newProj = SSDProjectile::GetNewProjectile(this, anVec3(0,-180,0), vec, weaponData[gameStats.currentWeapon].speed, weaponData[gameStats.currentWeapon].size);
			entities.Append(newProj);	
		}
...
SSDEntity* idGameSSDWindow::EntityHitTest(const anVec2& pt) {
...
void idGameSSDWindow::AsteroanStringuckPlayer(SSDAsteroid* asteroid) {
...
void idGameSSDWindow::AddScore(SSDEntity* ent, int points) {
	...
	if (points > 0) {
		pointsEnt = SSDPoints::GetNewPoints(this, ent, points, 1000, 50, anVec4(0,1,0,1));
	} else {
		pointsEnt = SSDPoints::GetNewPoints(this, ent, points, 1000, 50, anVec4(1,0,0,1));
	}
...
void idGameSSDWindow::AddDamage( intdamage) {
...
void idGameSSDWindow::AddHealth( inthealth) {
...
anVec2 idGameSSDWindow::GetCursorWorld() {
	anVec2 cursor;
	//GetCursor(cursor);
	cursor.x = gui->CursorX();
	cursor.y = gui->CursorY();
...
SSDEntity* idGameSSDWindow::GetSpecificEntity( inttype, int id) {
