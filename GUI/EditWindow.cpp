...
idEditWindow::idEditWindow( idDeviceContext *d, anUserInterface *g ) : idWindow(d, g) {
	dc = d;
	gui = g;
	CommonInit();
}

idEditWindow::idEditWindow( anUserInterface *g ) : idWindow(g) {
	gui = g;
	CommonInit();
}
...
This is the same as in anListWindow
================
*/
...
void idEditWindow::EnsureCursorVisible()
{
	...
	if ( !wrap ) {
		...
		if ( forceScroll ) {
			...
			int topLine = anMath::FtoiFast( scroller->GetValue() );
