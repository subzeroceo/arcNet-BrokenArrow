...
anCVar bearTurretAngle( "bearTurretAngle", "0", CVAR_FLOAT, "" );
anCVar bearTurretForce( "bearTurretForce", "200", CVAR_FLOAT, "" );
...
======================
*/
void BSEntity::WriteToSaveGame( anFile *savefile ) {
...
======================
*/
void BSEntity::ReadFromSaveGame( anFile *savefile, idGameBearShootWindow* _game ) {
	game = _game;
...
****************************************************************************
*/
idGameBearShootWindow::idGameBearShootWindow(idDeviceContext *d, anUserInterface *g) : idWindow(d, g) {
	dc = d;
	gui = g;
	CommonInit();
}

idGameBearShootWindow::idGameBearShootWindow(anUserInterface *g) : idWindow(g) {
	gui = g;
	CommonInit();
}
...
=============================
*/
void idGameBearShootWindow::WriteToSaveGame( anFile *savefile ) {
...
=============================
*/
void idGameBearShootWindow::ReadFromSaveGame( anFile *savefile ) {
...
=============================
*/
bool idGameBearShootWindow::ParseInternalVar(const char *_name, anParser *src) {
...
=============================
*/
void idGameBearShootWindow::Draw( inttime, float x, float y) {
	int i;
...
void idGameBearShootWindow::UpdateTurret() {
	...
	turretAngle = anMath::ClampFloat( 0.f, 90.f, angle );
}
...
void idGameBearShootWindow::UpdateButtons() {
	...
	if ( onFire ) {
		...
		vec.x = anMath::Cos( DEG2RAD(turretAngle) );
		vec.x += ( 1 - vec.x ) * 0.18f;
		vec.y = -anMath::Sin( DEG2RAD(turretAngle) );
...
void idGameBearShootWindow::UpdateGame() {
	...
	if (gamerunning == true) {
		...
		if ( windUpdateTime < current_time ) {
			...
			scale = 1.f - ( ( MAX_WINDFORCE - anMath::Fabs(windForce) ) / MAX_WINDFORCE);
			width = 100*scale;
		...
		timeRemaining = anMath::ClampFloat( 0.f, 99999.f, timeRemaining );
