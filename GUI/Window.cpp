...
anCVar idWindow::gui_debug( "gui_debug", "0", CVAR_GUI | CVAR_BOOL, "" );
anCVar idWindow::gui_edit( "gui_edit", "0", CVAR_GUI | CVAR_BOOL, "" );

extern anCVar r_skipGuiShaders;		// 1 = don't render any gui elements on surfaces

//  made RegisterVars a member of idWindow
const anRegEntry idWindow::RegisterVars[] = {
	{ "forecolor", anRegister::VEC4 },
	{ "hovercolor", anRegister::VEC4 },
	{ "backcolor", anRegister::VEC4 },
	{ "bordercolor", anRegister::VEC4 },
	{ "rect", anRegister::RECTANGLE },
	{ "matcolor", anRegister::VEC4 },
	{ "scale", anRegister::VEC2 },
	{ "translate", anRegister::VEC2 },
	{ "rotate", anRegister::FLOAT },
	{ "textscale", anRegister::FLOAT },
	{ "visible", anRegister::BOOL },
	{ "noevents", anRegister::BOOL },
	{ "text", anRegister::STRING },
	{ "background", anRegister::STRING },
	{ "runscript", anRegister::STRING },
	{ "varbackground", anRegister::STRING },
	{ "cvar", anRegister::STRING },
	{ "choices", anRegister::STRING },
	{ "choiceVar", anRegister::STRING },
	{ "bind", anRegister::STRING },
	{ "modelRotate", anRegister::VEC4 },
	{ "modelOrigin", anRegister::VEC4 },
	{ "lightOrigin", anRegister::VEC4 },
	{ "lightColor", anRegister::VEC4 },
	{ "viewOffset", anRegister::VEC4 },
	{ "hideCursor", anRegister::BOOL}
};

const int idWindow::NumRegisterVars = sizeof(RegisterVars) / sizeof(anRegEntry);
...
void idWindow::CommonInit() {
	...
	textScale = 0.35f;
	backColor.Zero();
	foreColor = anVec4(1, 1, 1, 1);
	hoverColor = anVec4(1, 1, 1, 1);
	matColor = anVec4(1, 1, 1, 1);
	borderColor.Zero();
	background = nullptr;
	backGroundName = "";
...
================
*/
idWindow::idWindow( anUserInterface *ui ) {
	dc = nullptr;
	gui = ui;
	CommonInit();
}
...
================
*/
idWindow::idWindow(idDeviceContext *d, anUserInterface *ui) {
	dc = d;
	gui = ui;
	CommonInit();
}
...
void idWindow::Move(float x, float y) {
	...
	rct.x = x;
	rct.y = y;
	anRegister *reg = RegList()->FindReg( "rect" );
	if (reg) {
		reg->Enable(false);
	}
	rect = rct;
...
================
*/
bool idWindow::RunTimeEvents( inttime) {
...
================
*/
void idWindow::DebugDraw( inttime, float x, float y) {
...
================
*/
float idWindow::EvalRegs( inttest, bool force) {
...
================
*/
void idWindow::ArchiveToDictionary(anDict *dict, bool useNames) {
...
================
*/
void idWindow::InitFromDictionary(anDict *dict, bool byName) {
	//FIXME: rewrite without state
	int c = children.Num();
	for ( int i = 0; i < c; i++ ) {
...
================
*/
bool idWindow::ParseScript(anParser *src, idGuiScriptList &list, int *timeParm, bool elseBlock ) {
...
void idWindow::SaveExpressionParseState() {
	...
	if ( int i = 0; i < MAX_EXPRESSION_REGISTERS; i++ ) {
		saveTemps[i] = registerIsTemporary[i];
	}
	//delete[] savetemps;
}
...
void idWindow::DisableRegister(const char *_name) {
	anRegister *reg = RegList()->FindReg(_name);
	if (reg) {
		reg->Enable( false );
	}
}
...
================
*/
void idWindow::ParseVec4(anParser *src, anVec4 &out) {
	anToken tok;
	src->ReadToken(&tok);
	out.x = atof(tok);
...
bool idWindow::Parse( anParser *src, bool rebuild) {
	...
	while( token != "}" ) {
		...
		if ( token == "windowDef" || token == "animationDef" ) {
			...
			anListWindow *win = new anListWindow(dc, gui);
		  	SaveExpressionParseState();
			win->Parse( src, rebuild);	
		  	RestoreExpressionParseState();
			AddChild(win);
			...
			if ( com_editors & EDITOR_GUI ) {
				...
				anLexer src2( str, str.Length(), "", src->GetFlags() );
			...
			anTimeLineEvent *ev = new anTimeLineEvent;
			if ( !src->ReadToken( &token ) ) {
				src->Error( "Unexpected end of file" );
				return false;
			}
			...
			gui->GetDesktop()->regList.AddReg( work, anRegister::VEC4, src, gui->GetDesktop(), var );
...
================
*/
void idWindow::AddTransition( idWinVar *dest, anVec4 from, anVec4 to, int time, float accelTime, float decelTime) {
...
int idWindow::ExpressionConstant(float f) {
	for ( int i = WEXP_REG_NUM_PREDEFINED ; i < expressionRegisters.Num() ; i++ ) {
...
================
*/
int idWindow::ParseEmitOp( anParser *src, int a, wexpOpType_t opType, int priority, wexpOp_t **opp ) {
...
=================
*/
int idWindow::ParseTerm( anParser *src,	idWinVar *var, int component ) {
	anToken token;
	int		a, b;
...
int idWindow::ParseExpressionPriority( anParser *src, int priority, idWinVar *var, int component ) {
	anToken token;
	int		a;
...
================
*/
int idWindow::ParseExpression(anParser *src, idWinVar *var, int component) {
...
================
*/
void idWindow::ParseBracedExpression(anParser *src) {
	src->ExpectTokenString( "{" );
	ParseExpression( src);
	src->ExpectTokenString( "}" );
}
	...
	for ( int i = 0 ; i < oc ; i++ ) {
		...
		case WOP_TYPE_VAR:
			...
			if ( op->b >= 0 && registers[op->b] >= 0 && registers[op->b] < 4 ) {
				...
				registers[op->c] = ((anVec4&)var)[registers[op->b]];
			} else {
...
================
*/
void idWindow::ReadFromDemoFile( class anSavedGamesFile *f, bool rebuild ) {
	...
	if (rebuild) {
		...
		f->ReadInt( c );
		for ( int i = 0; i < c; i++ ) {
			anTimeLineEvent *tl = new anTimeLineEvent;
			f->ReadInt( tl->time );
...
================
*/
void idWindow::WriteToDemoFile( class anSavedGamesFile *f ) {
	// should never hit unless we re-enable WRITE_GUIS
#ifndef WRITE_GUIS
	assert( false );
#else
...
===============
*/
void idWindow::WriteSaveGameString( const char *string, anFile *savefile ) {
...
===============
*/
void idWindow::ReadSaveGameTransition( idTransitionData &trans, anFile *savefile ) {
	int offset;
...
===============
*/
void idWindow::WriteToSaveGame( anFile *savefile ) {
...
===============
*/
void idWindow::ReadFromSaveGame( anFile *savefile ) {
	transitions.Clear();
...
int idWindow::GetChildIndex( idWindow *window ) {
	for ( int find = 0; find < drawWindows.Num(); find ++ ) {
...
bool idWindow::UpdateFromDictionary( anDict& dict ) {
	...
	for ( i = 0; i < dict.GetNumKeyVals(); i ++ ) {
		...
		anParser src( kv->GetValue().c_str(), kv->GetValue().Length(), "",
