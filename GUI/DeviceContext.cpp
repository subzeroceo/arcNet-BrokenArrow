...
anVec4 idDeviceContext::colorPurple;
anVec4 idDeviceContext::colorOrange;
anVec4 idDeviceContext::colorYellow;
anVec4 idDeviceContext::colorGreen;
anVec4 idDeviceContext::colorBlue;
anVec4 idDeviceContext::colorRed;
anVec4 idDeviceContext::colorBlack;
anVec4 idDeviceContext::colorWhite;
anVec4 idDeviceContext::colorNone;


anCVar gui_smallFontLimit( "gui_smallFontLimit", "0.30", CVAR_GUI | CVAR_ARCHIVE, "" );
anCVar gui_mediumFontLimit( "gui_mediumFontLimit", "0.60", CVAR_GUI | CVAR_ARCHIVE, "" );


anList<fontInfoEx_t> idDeviceContext::fonts;
...
void idDeviceContext::Init() {
	...
	SetupFonts();
	activeFont = &fonts[0];
	colorPurple = anVec4(1, 0, 1, 1);
	colorOrange = anVec4(1, 1, 0, 1);
	colorYellow = anVec4(0, 1, 1, 1);
	colorGreen = anVec4(0, 1, 0, 1);
	colorBlue = anVec4(0, 0, 1, 1);
	colorRed = anVec4(1, 0, 0, 1);
	colorWhite = anVec4(1, 1, 1, 1);
	colorBlack = anVec4(0, 0, 0, 1);
	colorNone = anVec4(0, 0, 0, 0);
...
void idDeviceContext::SetTransformInfo(const anVec3 &org, const anMat3 &m) {
	origin = org;
	mat = m;
}
...
void idDeviceContext::DrawStretchPic(float x, float y, float w, float h, float s1, float t1, float s2, float t2, const anMaterial *shader) {
	anDrawVert verts[4];
	glIndex_t indexes[6];
	indexes[0] = 3;
	indexes[1] = 0;
...
void idDeviceContext::DrawMaterial(float x, float y, float w, float h, const anMaterial *mat, const anVec4 &color, float scalex, float scaley) {
...
void idDeviceContext::DrawMaterialRotated(float x, float y, float w, float h, const anMaterial *mat, const anVec4 &color, float scalex, float scaley, float angle) {
...
void idDeviceContext::DrawStretchPicRotated(float x, float y, float w, float h, float s1, float t1, float s2, float t2, const anMaterial *shader, float angle) {
	...
	anMat4 rotz;
	rotz.Identity();
	float sinAng = anMath::Sin(angle);
	float cosAng = anMath::Cos(angle);
	rotz[0][0] = cosAng;
	rotz[0][1] = sinAng;
	rotz[1][0] = -sinAng;
	rotz[1][1] = cosAng;
...
void idDeviceContext::DrawFilledRect( float x, float y, float w, float h, const anVec4 &color) {
...
void idDeviceContext::DrawRect( float x, float y, float w, float h, float size, const anVec4 &color) {
...
void idDeviceContext::DrawMaterialRect( float x, float y, float w, float h, float size, const anMaterial *mat, const anVec4 &color) {
...
void idDeviceContext::SetCursor( intn) {
...
void idDeviceContext::PaintChar(float x,float y,float width,float height,float scale,float	s,float	t,float	s2,float t2,const anMaterial *hShader) {
	float	w, h;
	w = width * scale;
	h = height * scale;
...
int idDeviceContext::DrawText(float x, float y, float scale, anVec4 color, const char *text, float adjust, int limit, int style, int cursor) {
	int			len, count;
	anVec4		newColor;
	...
	if ( text && color.w != 0.0f ) {
		...
		renderSystem->SetColor(color);
		memcpy(&newColor[0], &color[0], sizeof(anVec4));
		len = strlen(text);
...
int idDeviceContext::CharWidth( const char c, float scale ) {
	...
	return anMath::FtoiFast( glyph->xSkip * useScale );
}
...
int idDeviceContext::TextWidth( const char *text, float scale, int limit ) {
	...
	return anMath::FtoiFast( scale * useFont->glyphScale * width );
}
...
int idDeviceContext::TextHeight(const char *text, float scale, int limit) {
	...
	return anMath::FtoiFast( max * useScale );
}

int idDeviceContext::MaxCharWidth(float scale) {
	SetFontByScale( scale);
	float useScale = scale * useFont->glyphScale;
	return anMath::FtoiFast( activeFont->maxWidth * useScale );
}

int idDeviceContext::MaxCharHeight(float scale) {
	SetFontByScale( scale);
	float useScale = scale * useFont->glyphScale;
	return anMath::FtoiFast( activeFont->maxHeight * useScale );
}

const anMaterial *idDeviceContext::GetScrollBarImage( int index) {
...
int idDeviceContext::DrawText( const char *text, float textScale, int textAlign, anVec4 color, idRectangle rectDraw, bool wrap, int cursor, bool calcOnly, anList<int> *breaks, int limit ) {
	const char	*p, *textPtr, *newLinePtr;
	char		buff[1024];
	...
	if ( !calcOnly && !(text && *text)) {
		...
		return anMath::FtoiFast( rectDraw.w / charSkip );
	}
