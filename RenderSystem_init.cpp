...
anCVarSystem r_skipRenderContext( "r_skipRenderContext", "0", CVAR_RENDERER | CVAR_BOOL, "NULL the rendering context during backend 3D rendering" );
...
static void R_CheckPortableExtensions( void ) {
	if ( glSLAvailable ) {
		qglConfig.qglVersion = atof( qglConfig.glslVersionStr );
		strncpyz( qglConfig.qglVersion, (const char *)qglGetString( GL_SHADING_LANGUAGE_VERSION ), sizeof( qglConfig.qglVersion ) );
		sscanf( qglConfig.qglVersion, "%d.%d", &qglConfig.versionStr, &qglConfig.glslVersionStr );
	...
	if ( R_CheckExtension( "GL_ARB_texture_compression_rgtc" ) && R_CheckExtension( "GL_ARB_texture_compression_bptc" ) ) {
		qglConfig.textureCompression = true;
    ...
    qglDeleteFragmentShaderATI = (PFNGLDELETEFRAGMENTSHADERSATIPROC)GLimp_ExtensionPointer( "glDeleteFragmentShaderATI" );
	}
	if ( qglConfig.atiShaderFragmentOn ) {
		qglGenFragmentShadersATI = (PFNGLGENFRAGMENTSHADERSATIPROC)GLimp_ExtensionPointer( "glGenFragmentShadersATI" );
		qglBindFragmentShaderATI = (PFNGLBINDFRAGMENTSHADERATIPROC)GLimp_ExtensionPointer( "glBindFragmentShaderATI" );
	...
	if (R_CheckExtension( "GL_ARB_framebuffer_object" ) ) {
	...
	//if ( qglConfig.qglVersion >= 3.2 || R_CheckExtension( "GL_ARB_depth_clamp" ) ) {}
 	// GL_EXT_depth_bounds_test
