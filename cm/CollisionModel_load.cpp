...
================
*/
void arcCollisionModelManagerLocal::ParseProcNodes( anLexer *src ) {
		...
		for ( cm_polygonRef_t *pref = node->polygons; pref; pref = pref->next ) {
			...
			for ( int i = 0; i < p->numEdges; i++ ) {
				...
				if ( edge->normal[0] == 0.0f && edge->normal[1] == 0.0f && edge->normal[2] == 0.0f ) {
					...
					float dot = edge->normal * p->plane.Normal();
...
cm_node_t *arcCollisionModelManagerLocal::AllocNode( cm_model_t *model, int blockSize ) {
	if ( !model->nodeBlocks || !model->nodeBlocks->nextNode ) {
		...
		cm_node_t *node = nodeBlock->nextNode;
...
cm_polygonRef_t *arcCollisionModelManagerLocal::AllocPolygonReference( cm_model_t *model, int blockSize ) {
	if ( !model->polygonRefBlocks || !model->polygonRefBlocks->nextRef ) {
		...
		cm_polygonRef_t *pref = prefBlock->nextRef;
		for ( int i = 0; i < blockSize - 1; i++ ) {
			pref->next = pref + 1;
			pref = pref->next;
		}
...
cm_brushRef_t *arcCollisionModelManagerLocal::AllocBrushReference( cm_model_t *model, int blockSize ) {
	if ( !model->brushRefBlocks || !model->brushRefBlocks->nextRef ) {
		...
		cm_brushRef_t *bref = brefBlock->nextRef;
		for ( int i = 0; i < blockSize - 1; i++ ) {
			bref->next = bref + 1;
			bref = bref->next;
		}
...
cm_polygon_t *arcCollisionModelManagerLocal::AllocPolygon( cm_model_t *model, int numEdges ) {
	...
	if ( model->polygonBlock && model->polygonBlock->bytesRemaining >= size ) {
		cm_polygon_t *poly = (cm_polygon_t *) model->polygonBlock->next;
...
cm_brush_t *arcCollisionModelManagerLocal::AllocBrush( cm_model_t *model, int numPlanes ) {
	...
	if ( model->brushBlock && model->brushBlock->bytesRemaining >= size ) {
		brush *brush = (cm_brush_t *) model->brushBlock->next;
...
void arcCollisionModelManagerLocal::SetupTrmModelStructure( void ) {
	...
	cm_node_t *node = (cm_node_t *) AllocNode( model, 1 );
	node->planeType = -1;
	model->node = node;
...
================
*/
cmHandle_t arcCollisionModelManagerLocal::SetupTrmModel( const anTraceModel &trm, const anMaterial *material ) {
	assert( models );
	...
	cm_vertex_t *vertex = model->vertices;
	const traceModelVert_t *trmVert = trm.verts;
	for ( int i = 0; i < trm.numVerts; i++, vertex++, trmVert++ ) {
		vertex->p = *trmVert;
		vertex->sideSet = 0;
	}
	// edges
	model->numEdges = trm.numEdges;
	cm_edge_t *edge = model->edges + 1;
	const traceModelEdge_t *trmEdge = trm.edges + 1;
	for ( int i = 0; i < trm.numEdges; i++, edge++, trmEdge++ ) {
	...
	const traceModelPoly_t *trmPoly = trm.polys;
	for ( int i = 0; i < trm.numPolys; i++, trmPoly++ ) {
		cm_polygon_t *poly = trmPolygons[i]->p;
...
============
*/
int arcCollisionModelManagerLocal::R_ChoppedAwayByProcBSP( int nodeNum, arcFixedWinding *w, const anVec3 &normal, const anVec3 &origin, const float radius ) {
	int res;
...
============
*/
int arcCollisionModelManagerLocal::ChoppedAwayByProcBSP( const arcFixedWinding &w, const anPlane &plane, int contents ) {
...
============
*/
arcFixedWinding *arcCollisionModelManagerLocal::WindingOutsideBrushes( arcFixedWinding *w, const anPlane &plane, int contents, int primitiveNum, cm_node_t *headNode ) {
	int windingLeft;
	...
	windingLeft = -1;
	for ( int i = 0; i < cm_windingList->numWindings; i++ ) {
...
cm_polygon_t *arcCollisionModelManagerLocal::TryMergePolygons( cm_model_t *model, cm_polygon_t *p1, cm_polygon_t *p2 ) {
	...
	if ( !keep1 ) {
		...
		GetEdge( model, model->vertices[model->edges[abs(edgeNum1)].vertexNum[INTSIGNBITSET(edgeNum1)]].p,
					model->vertices[model->edges[abs(edgeNum2)].vertexNum[INTSIGNBITNOTSET(edgeNum2)]].p, &newEdgeNum1, -1 );
		if ( newEdgeNum1 == 0 ) {
			keep1 = true;
		}
	}
	if ( !keep2 ) {
		...
		GetEdge( model, model->vertices[model->edges[abs(edgeNum1)].vertexNum[INTSIGNBITSET(edgeNum1)]].p, model->vertices[model->edges[abs(edgeNum2)].vertexNum[INTSIGNBITNOTSET(edgeNum2)]].p, &newEdgeNum2, -1 );
		if ( newEdgeNum2 == 0 ) {
			keep2 = true;
		}
	}
...
=============
*/
bool arcCollisionModelManagerLocal::PointInsidePolygon( cm_model_t *model, cm_polygon_t *p, anVec3 &v ) {
	int i, edgeNum;
...
================
*/
cm_node_t *arcCollisionModelManagerLocal::R_CreateAxialBSPTree( cm_model_t *model, cm_node_t *node, const anBounds &bounds ) {
	float planeDist;
	...
	cm_node_t *frontNode = AllocNode( model, NODE_BLOCK_SIZE_LARGE );
	memset( frontNode, 0, sizeof( cm_node_t ) );
	frontNode->parent = node;
	frontNode->planeType = -1;

	cm_node_t *backNode = AllocNode( model, NODE_BLOCK_SIZE_LARGE );
...
void CM_R_TestOptimisation( cm_node_t *node ) {
	...
	if ( polyCount || brushCount ) {
		int numChildren = CM_R_CountChildren( node );
...
================
*/
void arcCollisionModelManagerLocal::ClearHash( anBounds &bounds ) {
	cm_vertexHash->Clear();
	cm_edgeHash->Clear();
	...
	for ( int i = 0; ( 1<<i ) < cm_vertexShift; i++ ) {
	}
	if ( i == 0 ) {
		cm_vertexShift = 1;
	} else {
...
================
*/
ARC_INLINE int arcCollisionModelManagerLocal::HashVec(const anVec3 &vec) {
/*	int x, y;
	...
	x = ( ( ( int ) ( vec[0] - cm_modelBounds[0].x + 0.5f ) ) + 2 ) >> 2;
	y = ( ( ( int ) ( vec[1] - cm_modelBounds[0].y + 0.5f ) ) + 2 ) >> 2;
	z = ( ( ( int ) ( vec[2] - cm_modelBounds[0].z + 0.5f ) ) + 2 ) >> 2;
...
================
*/
int arcCollisionModelManagerLocal::GetVertex( cm_model_t *model, const anVec3 &v, int *vertexNum ) {
	int i, hashKey, vn;
	anVec3 vert, *p;
...
================
*/
int arcCollisionModelManagerLocal::GetEdge( cm_model_t *model, const anVec3 &v1, const anVec3 &v2, int *edgeNum, int v1num ) {
	int v2num, hashKey, e;
	int found, *vertexNum;
...
================
*/
void arcCollisionModelManagerLocal::CreatePolygon( cm_model_t *model, arcFixedWinding *w, const anPlane &plane, const anMaterial *material, int primitiveNum ) {
	int polyEdges[MAX_POINTS_ON_WINDING];
	anBounds bounds;

	// turn the winding into a sequence of edges
	int numPolyEdges = 0;
	int v1num = -1;		// first vertex unknown
	for ( int i = 0, j = 1; i < w->GetNumPoints(); i++, j++ ) {
		if ( int j >= w->GetNumPoints() ) {
			j = 0;
		}
	...
	for ( int i = 0; i < numPolyEdges; i++ ) {
		for ( int j = i+1; j < numPolyEdges; j++ ) {
	...
	cm_polygon_t *p = AllocPolygon( model, numPolyEdges );
...
================
*/
void arcCollisionModelManagerLocal::PolygonFromWinding( cm_model_t *model, arcFixedWinding *w, const anPlane &plane, const anMaterial *material, int primitiveNum ) {
...
=================
*/
void arcCollisionModelManagerLocal::CreatePatchPolygons( cm_model_t *model, anSurface_Patch &mesh, const anMaterial *material, int primitiveNum ) {
	for ( int i = 0; i < mesh.GetWidth() - 1; i++ ) {
		for ( int j = 0; j < mesh.GetHeight() - 1; j++ ) {
			int v1 = j * mesh.GetWidth() + i;
			int v2 = v1 + 1;
			int v3 = v1 + mesh.GetWidth() + 1;
...
void arcCollisionModelManagerLocal::ConvertBrush( cm_model_t *model, const idMapBrush *mapBrush, int primitiveNum ) {
	...
	anPlane *planes;
	arcFixedWinding w;
	const anMaterial *material = NULL;
...
static void CM_R_GetNodeBounds( anBounds *bounds, cm_node_t *node ) {
	...
	while ( 1 ) {
		for ( cm_brushRef_t *pref = node->polygons; pref; pref = pref->next ) {
...
void arcCollisionModelManagerLocal::FinishModel( cm_model_t *model ) {
	...
	model->usedMemory = model->numVertices * sizeof(cm_vertex_t) + model->numEdges * sizeof(cm_edge_t) +
						model->polygonMemory + model->brushMemory + model->numNodes * sizeof(cm_node_t) +
...
cm_model_t *arcCollisionModelManagerLocal::LoadRenderModel( const char *fileName ) {
	...
	cm_model_t *model = AllocModel();
	model->name = fileName;
	cm_node_t *node = AllocNode( model, NODE_BLOCK_SIZE_SMALL );
	node->planeType = -1;
	model->node = node;
	...
	anBounds bounds = renderModel->Bounds( NULL );
...
===================
*/
bool arcCollisionModelManagerLocal::GetModelBounds( cmHandle_t model, anBounds &bounds ) const {
	if ( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		common->Printf( "arcCollisionModelManagerLocal::GetModelBounds: invalid model handle\n" );
		return false;
	}
...
===================
*/
bool arcCollisionModelManagerLocal::GetModelVertex( cmHandle_t model, int vertexNum, anVec3 &vertex ) const {
	if ( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
...
===================
*/
bool arcCollisionModelManagerLocal::GetModelEdge( cmHandle_t model, int edgeNum, anVec3 &start, anVec3 &end ) const {
	if ( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
...
const anPlane &arcCollisionModelManagerLocal::GetPolygonPlane( int polygonNum ) const {
	return;
}
/*
===================
...
int arcCollisionModelManagerLocal::GetSurfacePolyCount( const cm_model_t *model, const anString &entityName ) {
    // Iterate over the polygons and count the ones belonging to the entity
    int numPolygons = 0;
    for ( int i = 0; i < model->numPolygons; i++ ) {
        if ( model->polygons[i].entityName == entityName ) {
            numPolygons++;
        }
    }
    return numPolygons;
...
int arcCollisionModelManagerLocal::GetNumPolygons( void ) const {
	...
	for ( int i = 0; i < poly->numPolys; i++ ) {
		...
		for ( int j = 0; j < poly->numEdges; j++ ) {
			poly->polys[i].edges[j] = poly->edges[j];
		}
	}
	return surfaces.Num();
...
const anMaterial *arcCollisionModelManagerLocal::GetPolygonMaterial( int polygonNum ) const {
...
===================
*/
const anPlane &arcCollisionModelManagerLocal::GetBrushPlane( int planeNum ) const {
...
/*
================
anCollisionModelManagerLocal::FinishModel
================
*/
void anCollisionModelManagerLocal::FinishModel( cm_model_t* model )
{
	// try to merge polygons
	checkCount++;
	MergeTreePolygons( model, model->node );
...
/*
================
anCollisionModelManagerLocal::LoadBinaryModel
================
*/
cm_model_t* anCollisionModelManagerLocal::LoadBinaryModel( const char* fileName, ID_TIME_T sourceTimeStamp ) {
...
/*
================
anCollisionModelManagerLocal::WriteBinaryModel
================
*/
void anCollisionModelManagerLocal::WriteBinaryModelToFile( cm_model_t* model, idFile* file, ID_TIME_T sourceTimeStamp ) {
	...
	struct local
	{
		...
		static void WriteNodeTree( idFile* file, cm_node_t* node, anList< cm_polygon_t* >& polys, anList< cm_brush_t* >& brushes )
		{
	...
	anList< const idMaterial* > materials;
...
================
*/
void anCollisionModelManagerLocal::WriteBinaryModel( cm_model_t* model, const char* fileName, ID_TIME_T sourceTimeStamp )
{
...
/*
================
anCollisionModelManagerLocal::LoadRenderModel
================
*/
cm_model_t* anCollisionModelManagerLocal::LoadRenderModel( const char* fileName )
...
==================
*/
bool arcCollisionModelManagerLocal::TrmFromModel_r( anTraceModel &trm, cm_node_t *node ) {
	while ( 1 ) {
		for ( cm_polygonRef_t *pref = node->polygons; pref; pref = pref->next ) {
			cm_polygon_t *p = pref->p;
...
==================
*/
bool arcCollisionModelManagerLocal::TrmFromModel( const cm_model_t *model, anTraceModel &trm ) {
...
==================
*/
bool arcCollisionModelManagerLocal::TrmFromModel( const char *modelName, anTraceModel &trm ) {
