...
///////////////////////////////////////////////////////////////////////////////
// seasFeature_s::Normal
///////////////////////////////////////////////////////////////////////////////
anVec3&	seasFeature_s::Normal()
{
	static anVec3 n(0,0,0);
...
// rvTestSet
//
// Test sets are designed to be used by the anSEASTacticalSensorLocal class to compute
...
struct rvTestSet
{
	...
	anVec3					mOrigin;			// Position Of The Test Subject
	anVec3					mFacing;			// Orientation Of The Test Subject
	...
	anVec3					mDirection;			// Resulting Direction To Feature
...
//		mDistance.mMin = 0.0f;	// special case (default would be -1.0f because all others are dot products)
	...
	///////////////////////////////////////////////////////////////////////////
	bool			Test(seasFeature_t* f, float distance=0.0f)
	{
...
///////////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal
//
// This is the local implimentation of the anSEASTacticalSensor interface.  It
// contains all the various local data and functions needed to execute a
// search of the tactical data in AAS.
///////////////////////////////////////////////////////////////////////////////////
struct anSEASTacticalSensorLocal : anSEASTacticalSensor
{
	// Owner
	///////////////////////////////////////////////////////////////////////////////
	anActor*				mOwner;					// Owner Of The Sensor
	anSAAI*					mOwnerAI;				// Owner As Already Cast To AI Type
	...
	anVec3					mAssignmentDirection;	// Used By Assignment Test
	bool					mAssignmentValid;		// Turns On And Off The Assignment Test
	anEntityPtr<anEntity>	mEnemyOverride;			// Overrides Enemy Pointer To Any Entity

	// Search & Update Results
	///////////////////////////////////////////////////////////////////////////////
	anList<seasFeature_t*>	mFeatures;				// The list of all features found in the most recent search
	anVec3					mReservedOrigin;		// Origin of feature that is currently reserved
	seasFeature_t*			mReserved;				// Which feature is currently reserved
	seasFeature_t*			mNear;					// Which feature is closest
	seasFeature_t*			mLook;					// Which feature to look down
	int						mLookStartTime;
	float					mLookStopDist;
	...
	bool			TestValid(seasFeature_t* f, float walkDistanceToFeature);
	bool			TestValidWithCurrentState(seasFeature_t* f=0);

	// Feature Reservation
	///////////////////////////////////////////////////////////////////////////////
	void			Reserve(seasFeature_t* f);

	// Access To Results
	///////////////////////////////////////////////////////////////////////////////
	int				FeatureCount()			{return mFeatures.Num();}
	seasFeature_t*	Feature( int i)			{return mFeatures[i];}
	seasFeature_t*	Near() const			{return mNear;}
	seasFeature_t*	Look() const			{return mLook;}
	seasFeature_t*	Reserved() const		{return mReserved;}
	const anVec3&	ReservedOrigin() const	{return mReservedOrigin;}
};

// Global Objects
anSEASTacticalSensorLocal*			mSensor;
float								mDebugRadius;


// Global Typedefines
typedef	anEntityPtr<anEntity>		TEntPtr;
typedef	seasFeature_t*				TFeaturePtr;




// anSEASTacticalSensor::CREATE_SENSOR
anSEASTacticalSensor* anSEASTacticalSensor::CREATE_SENSOR(anActor* owner) {
	anSEASTacticalSensorLocal* nSensor = new anSEASTacticalSensorLocal();
	nSensor->mOwner		= owner;
	nSensor->mOwnerAI	= dynamic_cast<anSAAI*>(owner);
	return nSensor;
}
...
void	anSEASTacticalSensorLocal::Save(anSaveGame *savefile) {
	...
	savefile->WriteInt(mLookStartTime);
	savefile->WriteFloat(mLookStopDist);
}

void	anSEASTacticalSensorLocal::Restore( anRestoreGame *savefile ) {
	// Clear Old Data
	mFeatures.Clear();
	mNear		= 0;
	mLook		= 0;
...
//	Search();
	...
	savefile->ReadInt(mLookStartTime);
	savefile->ReadFloat(mLookStopDist);
}
...
void anSEASTacticalSensorLocal::Update() {
	...
	anVec3			velocityFwd			= mOwner->GetPhysics()->GetLinearVelocity();
	const anVec3&	ownerOrigin			= mOwner->GetPhysics()->GetOrigin();
	...
	seasArea_t&		area				= file->GetArea(ownerAreaNum);
	anActor*		teammate			= nullptr;
	...
	//-----------------------------------------------------------------
	if (area.numFeatures) {
		for ( intareaFeatureNum=0; areaFeatureNum<area.numFeatures; areaFeatureNum++ ) {
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::Reserve
///////////////////////////////////////////////////////////////////////////////
void anSEASTacticalSensorLocal::Reserve(seasFeature_t* f) {
...
void anSEASTacticalSensorLocal::TestSetupCurrentValues() {
	// Owner Test Set
	//----------------
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::TestValidReserved
///////////////////////////////////////////////////////////////////////////////
bool anSEASTacticalSensorLocal::TestValidWithCurrentState(seasFeature_t* f) {
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::TestValid
//
...
bool anSEASTacticalSensorLocal::TestValid( seasFeature_t* f, float walkDistanceToFeature ) {
 	static anVec3 LeanNormal;
 	static anVec3 Up(0.0f,0.0f,1.0f);
...
// build upon them.
void anSEASTacticalSensorLocal::SearchReset( anEntity *enemyOverride, float ownerRangeMin, float ownerRangeMax) {
	if ( !mOwner ) {
		return;
	}
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::SearchDebug
//
...
///////////////////////////////////////////////////////////////////////////////
void anSEASTacticalSensorLocal::SearchDebug()
{
	SearchCover();
}
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::SearchCover
///////////////////////////////////////////////////////////////////////////////
void anSEASTacticalSensorLocal::SearchCover(float rangeMin, float rangeMax)
{
	SearchReset(0, rangeMin, rangeMax);
	mSearchName							= "Cover";
	Search();
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::SearchFlank
///////////////////////////////////////////////////////////////////////////////
void anSEASTacticalSensorLocal::SearchFlank()
{
	SearchReset();
	mSearchName							= "Flank";
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::SearchAdvance
///////////////////////////////////////////////////////////////////////////////
void anSEASTacticalSensorLocal::SearchAdvance()
{
	SearchReset();
	mSearchName							= "Advance";
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::SearchRetreat
///////////////////////////////////////////////////////////////////////////////
void anSEASTacticalSensorLocal::SearchRetreat()
{
	SearchReset();
	mSearchName							= "Retreat";
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::SearchAmbush
///////////////////////////////////////////////////////////////////////////////
void anSEASTacticalSensorLocal::SearchAmbush()
{
	SearchReset();
	mSearchName							= "Ambush";
	mFromOwner.mDistance.mMin			=  0.35f;	// Must Be A Good Distance From Where We Are
...
///////////////////////////////////////////////////////////////////////////
float anSEASTacticalSensorLocal::SearchComputeWeightRange(float& rangeNegative)
{
...
///////////////////////////////////////////////////////////////////////////////
float anSEASTacticalSensorLocal::SearchComputeWeight()
{
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::Search
///////////////////////////////////////////////////////////////////////////////
void anSEASTacticalSensorLocal::Search()
{
	...
	static seasFeature_t*		featurePtr;
	static anVec3				featureOrigin;
	static anReachability*		reach;
	static anList<rvSortReach>	searchHeap;
...
//-----------------------------------------------------------------------------
 	...
 	while (searchHeap.Num() && areaVisitCount<MAX_AREAS_TOUCHED && mFeatures.Num()<mFeaturesSearchMax)
	{
		...
		//-------------------------
		const seasArea_t& area = file->GetArea(sr.mAreaNum);
		if (area.numFeatures)
		{
...
///////////////////////////////////////////////////////////////////////////////
// seasFeature_t::DrawDebugInfo
///////////////////////////////////////////////////////////////////////////////
void seasFeature_t::DrawDebugInfo( int index )
{
	static anVec3 Height;
	static anVec3 Orig;
	static anVec3 Norm;
	static anVec3 Text;
	static anVec4 color;
	...
	Left		=  Normal().Cross(anVec3(0,0,-1)) * mDebugRadius;
...
///////////////////////////////////////////////////////////////////////////////
// anSEASTacticalSensorLocal::DrawDebugInfo
///////////////////////////////////////////////////////////////////////////////
void anSEASTacticalSensorLocal::DrawDebugInfo()
{
	...
	{
		anVec3 n = mLook->Normal();
		n *= 64.0f;
		gameRenderWorld->DebugArrow(colorYellow, mOwner->GetPhysics()->GetOrigin() + anVec3(0,0,32), mOwner->GetPhysics()->GetOrigin() + anVec3(0,0,32) + n, 8);
	}
}
