...
public:
	virtual						~anCollisionModel() { }
								// Returns the name of the model.
	virtual const char *		GetName( void ) const = 0;
								// Gets the bounds of the model.
	virtual const anBounds&		GetBounds( void ) const = 0;
								// Gets the bounds of the model, excluding/including surfaces of the appropriate surface type
	virtual void				GetBounds( anBounds& bounds, int surfaceMask, bool inclusive ) const = 0;
	...
	virtual const anVec3&		GetVertex( int vertexNum ) const = 0;
								// Gets an edge of the model.
	virtual void				GetEdge( int edgeNum, anVec3& start, anVec3& end ) const = 0;
								// Gets a polygon of the model.
	virtual void				GetPolygon( int polygonNum, anFixedWinding &winding ) const = 0;
	...
	virtual const anPlane&		GetBrushPlane( int planeNum ) const = 0;

	virtual const anMaterial*	GetPolygonMaterial( int polygonNum ) const = 0;
	virtual const anPlane&		GetPolygonPlane( int polygonNum ) const = 0;
...
public:
	...
	virtual void			FreeModel( anCollisionModel *model ) = 0;
							// Purge all unused models.
	virtual void			PurgeModels( void ) = 0;

	// Sets up a trace model for collision with other trace models.
	virtual cmHandle_t		SetupTrmModel( const anTraceModel &trm, const anMaterial *material ) = 0;
	// Creates a trace model from a collision model, returns true if succesfull.
	virtual bool			TrmFromModel( const char *modelName, anTraceModel &trm ) = 0;

							// Sets up a trace model for collision with other trace models.
	virtual anCollisionModel *ModelFromTrm( const char *mapName, const char *modelName, const anTraceModel &trm, const anMaterial *material ) = 0;
							// Creates a trace model from a collision model, returns true if succesfull.
	virtual bool			TrmFromModel( const char *mapName, const char *modelName, anTraceModel &trm ) = 0;
							// Creates a trace model for each primitive of the collision model, returns the number of trace models.
	virtual int				CompoundTrmFromModel( const char *mapName, const char *modelName, anTraceModel *trms, int maxTrms ) = 0;
	// Gets the name of a model.
	virtual const char *	GetModelName( cmHandle_t model ) const = 0;
	// Gets the bounds of a model.
	virtual bool			GetModelBounds( cmHandle_t model, anBounds &bounds ) const = 0;
	...
	virtual bool			GetModelVertex( cmHandle_t model, int vertexNum, anVec3 &vertex ) const = 0;
	// Gets an edge of a model.
	virtual bool			GetModelEdge( cmHandle_t model, int edgeNum, anVec3 &start, anVec3 &end ) const = 0;
	...
	virtual void			Translation( trace_t *results, const anVec3 &start, const anVec3 &end,
								const anTraceModel *trm, const anMat3 &trmAxis, int contentMask,
								cmHandle_t model, const anVec3 &modelOrigin, const anMat3 &modelAxis ) = 0;
	// Rotates a trace model and reports the first collision if any.
	virtual void			Rotation( trace_t *results, const anVec3 &start, const arcRotation &rotation,
								const anTraceModel *trm, const anMat3 &trmAxis, int contentMask,
								cmHandle_t model, const anVec3 &modelOrigin, const anMat3 &modelAxis ) = 0;
	// Returns the contents touched by the trace model or 0 if the trace model is in free space.
	virtual int				Contents( const anVec3 &start,
								const anTraceModel *trm, const anMat3 &trmAxis, int contentMask,
								cmHandle_t model, const anVec3 &modelOrigin, const anMat3 &modelAxis ) = 0;
	// Stores all contact points of the trace model with the model, returns the number of contacts.
	virtual int				Contacts( contactInfo_t *contacts, const int maxContacts, const anVec3 &start, const anVec6 &dir, const float depth,
								const anTraceModel *trm, const anMat3 &trmAxis, int contentMask,
								cmHandle_t model, const anVec3 &modelOrigin, const anMat3 &modelAxis ) = 0;
	...
	virtual void			DebugOutput( const anVec3 &origin ) = 0;
	// Draws a model.
	virtual void			DrawModel( cmHandle_t model, const anVec3 &modelOrigin, const anMat3 &modelAxis,
												const anVec3 &viewOrigin, const float radius ) = 0;
