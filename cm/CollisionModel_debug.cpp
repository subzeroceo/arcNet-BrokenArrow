...
arcCVarSystem cm_drawMask(			"cm_drawMask",			"none",		CVAR_GAME,				"collision mask", cm_contentsNameByIndex, idCmdSystem::ArgCompletion_String<cm_contentsNameByIndex> );
arcCVarSystem cm_drawColor(		"cm_drawColor",			"1 0 0 .5",	CVAR_GAME,				"color used to draw the collision models" );
arcCVarSystem cm_drawFilled(		"cm_drawFilled",		"0",		CVAR_GAME | CVAR_BOOL,	"draw filled polygons" );
arcCVarSystem cm_drawInternal(		"cm_drawInternal",		"1",		CVAR_GAME | CVAR_BOOL,	"draw internal edges green" );
arcCVarSystem cm_drawNormals(		"cm_drawNormals",		"0",		CVAR_GAME | CVAR_BOOL,	"draw polygon and edge normals" );
arcCVarSystem cm_backFaceCull(		"cm_backFaceCull",		"0",		CVAR_GAME | CVAR_BOOL,	"cull back facing polygons" );
arcCVarSystem cm_debugCollision(	"cm_debugCollision",	"0",		CVAR_GAME | CVAR_BOOL,	"debug the collision detection" );

static arcVec4 cm_color;
...
int arcCollisionModelManagerLocal::ContentsFromString( const char *string ) const {
	int contents = 0;
	arcLexer src( string, arcNetString::Length( string ), "ContentsFromString" );
	arcNetToken token;
...
const char *arcCollisionModelManagerLocal::StringFromContents( const int contents ) const {
	...
	for ( int i = 1; cm_contentsFlagByIndex[i] != 0; i++ ) {
		if ( contents & cm_contentsFlagByIndex[i] ) {
			if ( length != 0 ) {
				length += arcNetString::snPrintf( contentsString + length, sizeof( contentsString ) - length, "," );
			}
			length += arcNetString::snPrintf( contentsString + length, sizeof( contentsString ) - length, cm_contentsNameByIndex[i] );
		}
	}
...
================
*/
void arcCollisionModelManagerLocal::DrawEdge( cm_model_t *model, int edgeNum, const arcVec3 &origin, const arcMat3 &axis ) {
	int side;
	cm_edge_t *edge;
	arcVec3 start, end, mid;
	bool isRotated;
...
================
*/
void arcCollisionModelManagerLocal::DrawPolygon( cm_model_t *model, cm_polygon_t *p, const arcVec3 &origin, const arcMat3 &axis, const arcVec3 &viewOrigin ) {
	if ( cm_backFaceCull.GetBool() ) {
		int edgeNum = p->edges[0];
		cm_edge_t *edge = model->edges + abs( edgeNum );
		arcVec3 dir = model->vertices[edge->vertexNum[0]].p - viewOrigin;
	...
	if ( cm_drawNormals.GetBool() ) {
		...
		if ( axis.IsRotated() ) {
			arcVec3 center = center * axis + origin;
			arcVec3 end = center + 5 * ( axis * p->plane.Normal() );
		} else {
			arcVec3 center += origin;
			arcVec3 end = center + 5 * p->plane.Normal();
		}
...
================
*/
void arcCollisionModelManagerLocal::DrawNodePolygons( cm_model_t *model, cm_node_t *node, const arcVec3 &origin, const arcMat3 &axis, const arcVec3 &viewOrigin, const float radius ) {
	while ( 1 ) {
...
================
*/
void arcCollisionModelManagerLocal::DrawModel( cmHandle_t handle, const arcVec3 &modelOrigin, const arcMat3 &modelAxis, const arcVec3 &viewOrigin, const float radius ) {
	...
	arcVec3 viewPos = (viewOrigin - modelOrigin) * modelAxis.Transpose();
	checkCount++;
...
static arcCVarSystem cm_testCollision(		"cm_testCollision",		"0",					CVAR_GAME | CVAR_BOOL,		"" );
static arcCVarSystem cm_testRotation(		"cm_testRotation",		"1",					CVAR_GAME | CVAR_BOOL,		"" );
static arcCVarSystem cm_testModel(			"cm_testModel",			"0",					CVAR_GAME | CVAR_INTEGER,	"" );
static arcCVarSystem cm_testTimes(			"cm_testTimes",			"1000",					CVAR_GAME | CVAR_INTEGER,	"" );
static arcCVarSystem cm_testRandomMany(	"cm_testRandomMany",	"0",					CVAR_GAME | CVAR_BOOL,		"" );
static arcCVarSystem cm_testOrigin(		"cm_testOrigin",		"0 0 0",				CVAR_GAME,					"" );
static arcCVarSystem cm_testReset(			"cm_testReset",			"0",					CVAR_GAME | CVAR_BOOL,		"" );
static arcCVarSystem cm_testBox(			"cm_testBox",			"-16 -16 0 16 16 64",	CVAR_GAME,					"" );
static arcCVarSystem cm_testBoxRotation(	"cm_testBoxRotation",	"0 0 0",				CVAR_GAME,					"" );
static arcCVarSystem cm_testWalk(			"cm_testWalk",			"1",					CVAR_GAME | CVAR_BOOL,		"" );
static arcCVarSystem cm_testLength(		"cm_testLength",		"1024",					CVAR_GAME | CVAR_FLOAT,		"" );
static arcCVarSystem cm_testRadius(		"cm_testRadius",		"64",					CVAR_GAME | CVAR_FLOAT,		"" );
static arcCVarSystem cm_testAngle(			"cm_testAngle",			"60",					CVAR_GAME | CVAR_FLOAT,		"" );
...
static arcVec3 *testend;

#include "../sys/sys_public.h"

void arcCollisionModelManagerLocal::DebugOutput( const arcVec3 &origin ) {
	int i, k, t;
	char buf[128];
	arcVec3 end;
	arcAngles boxAngles;
	arcMat3 modelAxis, boxAxis;
	arcBounds bounds;
	trace_t trace;
	...
	testend = (arcVec3 *) Mem_Alloc( cm_testTimes.GetInteger() * sizeof(arcVec3) );
	...
	if ( cm_testRotation.GetBool() ) {
		...
		arcVec3 vec( random.CRandomFloat(), random.CRandomFloat(), random.RandomFloat() );
		vec.Normalize();
