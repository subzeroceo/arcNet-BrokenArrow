...
void idMD6Mesh::ParseMesh(idLexer& parser, int numJoints, const idJointMat* joints) {
	...
	for ( intj = 0, i = 0; i < texCoords.Num(); i++ ) {
	...
	for ( inti = 0; i < jointIsUsed.Num(); i++ ) {
		jointIsUsed[i] = false;
	}
	...
	for ( inti = 0; i < deformInfo->numOutputVerts; i++ ) {
		for ( intj = 0; j < 4; j++ ) {
...
====================
*/
anBounds idMD6Mesh::CalcBounds(const idJointMat* entJoints) {
...
====================
*/
int idMD6Mesh::NearestJoint( inta, int b, int c) const {
...
====================
*/
anBounds idRenderModelMD6::Bounds(const idRenderEntity* ent) const {
#if 0
...
static void TransformJoints(idJointMat *__restrict outJoints, const int numJoints, const idJointMat *__restrict inJoints1, const idJointMat *__restrict inJoints2 ) {
	...
	for ( inti = 0; i < numJoints; i += 2, inFloats1 += 2 * 12, inFloats2 += 2 * 12, outFloats += 2 * 12) {
...
idRenderModel* idRenderModelMD6::InstantiateDynamicModel( const class idRenderEntity *ent, const struct viewDef_s *view, idRenderModel *cachedModel ) {
	...
	if ( cachedModel ) {
		assert( dynamic_cast<idRenderModelMD6Instance*>(cachedModel) != nullptr );
	...
	if ( staticModel->jointsInverted == nullptr ) {
		...
		staticModel->jointsInverted = (idJointMat *)Mem_ClearedAlloc( ALIGN( numInvertedJoints * sizeof( anJointMat ), alignment ) );
	}
	else {
...
====================
*/
void idRenderModelMD6Instance::CreateStaticMeshSurfaces( const anList<idMD6Mesh> &meshes ) {
	int	i, surfaceNum;
	const idMD6Mesh* mesh;
...
====================
*/
void idRenderModelMD6Instance::UpdateSurfaceGPU( struct deformInfo_s *deformInfo, const idRenderEntityParms *ent, modelSurface_t* surf, const anMaterial *shader ) {
	int i, base;
	srfTriangles_t* tri;
