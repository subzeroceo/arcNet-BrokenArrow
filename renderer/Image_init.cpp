...
arcCVarSystem arcImageManager::image_filter( "image_filter", imageFilter[1], CVAR_RENDERER | CVAR_ARCHIVE, "changes texture filtering on mipmapped images", imageFilter, arcCmdSystem::ArgCompletion_String<imageFilter> );
arcCVarSystem arcImageManager::image_anisotropy( "image_anisotropy", "1", CVAR_RENDERER | CVAR_ARCHIVE, "set the maximum texture anisotropy if available" );
arcCVarSystem arcImageManager::image_lodbias( "image_lodbias", "0", CVAR_RENDERER | CVAR_ARCHIVE, "change lod bias on mipmapped images" );
arcCVarSystem arcImageManager::image_downSize( "image_downSize", "0", CVAR_RENDERER | CVAR_ARCHIVE, "controls texture downsampling" );
arcCVarSystem arcImageManager::image_forceDownSize( "image_forceDownSize", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "" );
arcCVarSystem arcImageManager::image_roundDown( "image_roundDown", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "round bad sizes down to nearest power of two" );
arcCVarSystem arcImageManager::image_colorMipLevels( "image_colorMipLevels", "0", CVAR_RENDERER | CVAR_BOOL, "development aid to see texture mip usage" );
arcCVarSystem arcImageManager::image_preload( "image_preload", "1", CVAR_RENDERER | CVAR_BOOL | CVAR_ARCHIVE, "if 0, dynamically load all images" );
arcCVarSystem arcImageManager::image_useCompression( "image_useCompression", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "0 = force everything to high quality" );
arcCVarSystem arcImageManager::image_useAllFormats( "image_useAllFormats", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "allow alpha/intensity/luminance/luminance+alpha" );
arcCVarSystem arcImageManager::image_useNormalCompression( "image_useNormalCompression", "2", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "2 = use rxgb compression for normal maps, 1 = use 256 color compression for normal maps if available" );
arcCVarSystem arcImageManager::image_usePrecompressedTextures( "image_usePrecompressedTextures", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "use .dds files if present" );
arcCVarSystem arcImageManager::image_writePrecompressedTextures( "image_writePrecompressedTextures", "0", CVAR_RENDERER | CVAR_BOOL, "write .dds files if necessary" );
arcCVarSystem arcImageManager::image_writeNormalTGA( "image_writeNormalTGA", "0", CVAR_RENDERER | CVAR_BOOL, "write .tgas of the final normal maps for debugging" );
arcCVarSystem arcImageManager::image_writeNormalTGAPalletized( "image_writeNormalTGAPalletized", "0", CVAR_RENDERER | CVAR_BOOL, "write .tgas of the final palletized normal maps for debugging" );
arcCVarSystem arcImageManager::image_writeTGA( "image_writeTGA", "0", CVAR_RENDERER | CVAR_BOOL, "write .tgas of the non normal maps for debugging" );
arcCVarSystem arcImageManager::image_useOffLineCompression( "image_useOfflineCompression", "0", CVAR_RENDERER | CVAR_BOOL, "write a batch file for offline compression of DDS files" );
arcCVarSystem arcImageManager::image_cacheMinK( "image_cacheMinK", "200", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "maximum KB of precompressed files to read at specification time" );
arcCVarSystem arcImageManager::image_cacheMegs( "image_cacheMegs", "20", CVAR_RENDERER | CVAR_ARCHIVE, "maximum MB set aside for temporary loading of full-sized precompressed images" );
arcCVarSystem arcImageManager::image_useCache( "image_useCache", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "1 = do background load image caching" );
arcCVarSystem arcImageManager::image_showBackgroundLoads( "image_showBackgroundLoads", "0", CVAR_RENDERER | CVAR_BOOL, "1 = print number of outstanding background loads" );
arcCVarSystem arcImageManager::image_downSizeSpecular( "image_downSizeSpecular", "0", CVAR_RENDERER | CVAR_ARCHIVE, "controls specular downsampling" );
arcCVarSystem arcImageManager::image_downSizeBump( "image_downSizeBump", "0", CVAR_RENDERER | CVAR_ARCHIVE, "controls normal map downsampling" );
arcCVarSystem arcImageManager::image_downSizeSpecularLimit( "image_downSizeSpecularLimit", "64", CVAR_RENDERER | CVAR_ARCHIVE, "controls specular downsampled limit" );
arcCVarSystem arcImageManager::image_downSizeBumpLimit( "image_downSizeBumpLimit", "128", CVAR_RENDERER | CVAR_ARCHIVE, "controls normal map downsample limit" );
arcCVarSystem arcImageManager::image_ignoreHighQuality( "image_ignoreHighQuality", "0", CVAR_RENDERER | CVAR_ARCHIVE, "ignore high quality setting on materials" );
arcCVarSystem arcImageManager::image_downSizeLimit( "image_downSizeLimit", "256", CVAR_RENDERER | CVAR_ARCHIVE, "controls diffuse map downsample limit" );
// do this with a pointer, in case we want to make the actual manager
// a private virtual subclass
arcImageManager	imageManager;
arcImageManager	*globalImages = &imageManager;
...
typedef arcNetList<int> intList;

const imageClassificate_t IC_Info[] = {
	{ "models/char", "Characters", IC_NPC, 512, 512 },
	{ "models/weapons", "Weapons", IC_WEAPON, 512, 512 },
	{ "models/objects", "Global Model Geometry", IC_MODELGEOMETRY, 512, 512 },
	{ "models/items", "Items", IC_ITEMS, 512, 512 },
	{ "models", "Other model textures", IC_MODELSOTHER, 512, 512 },
	{ "guis/assets", "Gui Assets", IC_GUIS, 256, 256 },
	{ "textures", "Worldly Geometry", IC_WORLDGEOMETRY, 256, 256 },
	{ "", "Other", IC_OTHER, 256, 256 }
};

static int ClassifyImage( const char *name ) {
	arcNetString str = name;
...
void R_PurgeImage( ARCImage *image ) {
...
ARCImage *R_FindCachedImage( const char *name, int hash ) {
	ARCImage *bImage, *bImagePrev;
...
void VK_ShowTexturePolarity( VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout, VkDescriptorSet* descriptorSets, drawSurf_t** drawSurfs, int numDrawSurfs) {
	 if ( !r_showTexturePolarity.GetBool() ) {
		  return;
	 }
	 globalImages->BindNull();

	 // Set up Vulkan states
	 VK_SetupPipelineStates( commandBuffer, VK_BLEND_FACTOR_SRC_ALPHA, VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA );
	 VK_SetColor( commandBuffer, 1, 1, 1 );

	 for ( int i = 0; i < numDrawSurfs; i++ ) {
		  rawSurf_t *drawSurf = drawSurfs[i];
		  const srfTriangles_t* tri = drawSurf->frontEndGeo;
		  if ( !tri->verts ) {
				continue;
		  }

		  VK_SimpleSurfaceSetup( commandBuffer, drawSurf);

		  vkCmdBeginRenderPass( commandBuffer, &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE );

		  VkBuffer vertexBuffer = tri->vertexBuffer;
		  VkDeviceSize vertexOffset = tri->vertexOffset;

		  vkCmdBindPipeline( commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline );

		  VkDescriptorSet descriptorSet = descriptorSets[i];

		  vkCmdBindDescriptorSets(c ommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet, 0, nullptr );

		  vkCmdBindVertexBuffers( commandBuffer, 0, 1, &vertexBuffer, &vertexOffset );
		  vkCmdDraw( commandBuffer, tri->numVertices, 1, 0, 0 );

		  vkCmdEndRenderPass( commandBuffer );
	 }

	 // Reset Vulkan states
	 VK_ResetPipelineStates( commandBuffer );
}
#endif
*/
...
================
*/
static void R_RampImage( ARCImage *image ) {
	GLbyte data[256][4];
...
================
*/
static void R_SpecularTableImage( ARCImage *image ) {
	GLbyte data[256][4];
...
================
*/
static void R_Specular2DTableImage( ARCImage *image ) {
...
================
*/
static void R_AlphaRampImage( ARCImage *image ) {
	GLbyte data[256][4];
...
static void R_DefaultImage( ARCImage *image ) {
	image->MakeDefault();
}

static void R_WhiteImage( ARCImage *image ) {
	GLbyte data[DEFAULT_SIZE][DEFAULT_SIZE][4];
...
static void R_BlackImage( ARCImage *image ) {
	GLbyte data[DEFAULT_SIZE][DEFAULT_SIZE][4];
...
static void R_BorderClampImage( ARCImage *image ) {
...
static void R_RGBA8Image( ARCImage *image ) {
	GLbyte data[DEFAULT_SIZE][DEFAULT_SIZE][4];
...
static void R_RGB8Image( ARCImage *image ) {
	GLbyte data[DEFAULT_SIZE][DEFAULT_SIZE][4];
...
static void R_AlphaNotchImage( ARCImage *image ) {
	GLbyte data[2][4];
...
static void R_FlatNormalImage( ARCImage *image ) {
	GLbyte data[DEFAULT_SIZE][DEFAULT_SIZE][4];
...
static void R_AmbientNormalImage( ARCImage *image ) {
	GLbyte data[DEFAULT_SIZE][DEFAULT_SIZE][4];
	int red = ( globalImages->image_useNormalCompression.GetInteger() == 1 ) ? 0 : 3;
	int alpha = ( red == 0 ) ? 3 : 0;
...
static void CreateSquareLight( void ) {
	...
	for ( int x = 0; x < 128; x++ ) {
		...
		for ( int y = 0; y < 128; y++ ) {
			...
			int d = ( GLbyte )arcMath::Sqrt( dx * dx + dy * dy );
...
static void R_CreateDlightImage( void ) {
	...
	R_StaticFree( buffer );
	//lightImage = R_CreateImage( "*dlight", (GLbyte *)data, DLIGHT_SIZE, DLIGHT_SIZE, qfalse, qfalse, GL_CLAMP );
}
...
static void GetCubesVector( int i, int cubeSize, int x, int y, arcVec3 *vector ) {
	...
	switch ( i ) {
	 ...
	 float mag = 1.0f / arcMath::Sqrt( vector.x * vector.x + vector.y * vector.y + vector.z * vector.z );
	//float mag = arcMath::InvSqrt( vector[0]*vector[0] + vector[1]*vector[1] + vector[2]*vector[2] );
	vector.x *= mag;
	vector.y *= mag;
	vector.z *= mag;
	return vector;
...
// access the cube map.
static void MakeNormalizeVectorCubeMap( ARCImage *image ) {
	float vector[3];
	GLbyte *pixels[6];
	...
	for ( int i = 0; i < 6; i++ ) {
		...
		for ( int y = 0; y < size; y++ ) {
		  for ( int x = 0; x < size; x++ ) {
					 //arcVec3 V = GetCubesVector(i, size, x, y);
...
/*void VK_MakeNormalizeVectorCubeMap( VkImage image ) {
	 float vector[3];
	 uint8_t *pixels[6];
	 VkDeviceSize size = NORMAL_MAP_SIZE;

	 for ( int i = 0; i < 6; i++ ) {
		  pixels[i] = new uint8_t[size * size * 4];
		  for ( int y = 0; y < size; y++ ) {
				for ( int x = 0; x < size; x++ ) {
					 arcVec3 V = GetCubesVector( i, size, x, y );
					 vector[0] = V.x;
					 ...
					 pixels[i][4 * ( y * size + x ) + 0] = static_cast<uint8_t>( 128 + 127 * vector[0] ) ;
					 pixels[i][4 * ( y * size + x ) + 1] = static_cast<uint8_t>( 128 + 127 * vector[1] );
					 pixels[i][4 * ( y * size + x ) + 2] = static_cast<uint8_t>( 128 + 127 * vector[2] );
					 pixels[i][4 * ( y * size + x )  + 3] = 255;
				}
		  }
	 }

	 // Update the Vulkan image with the pixel data
	 // ...
...
================
*/
static void R_CreateNoFalloffImage( ARCImage *image ) {
...
void R_FogImage( ARCImage *image ) {
	...
	for ( int x = 0; x < FOG_SIZE; x++ ) {
		for ( int y = 0; y < FOG_SIZE; y++ ) {
			float d = arcMath::Sqrt( ( x - FOG_SIZE/2 ) * ( x - FOG_SIZE/2 ) + ( y - FOG_SIZE/2 ) * ( y - FOG_SIZE / 2 ) ); d /= FOG_SIZE/2-1;
			int b = ( GLbyte )( d * 255 );
			if ( b <= 0 ) {
				b = 0;
			} else if ( b > 255 ) {
...
static float FogFraction( float viewHeight, float targetHeight ) {
	float total = arcMath::Fabs( targetHeight - viewHeight );
...
================
*/
void R_FogEnterImage( ARCImage *image ) {
...
void R_QuadraticImage( ARCImage *image ) {
...
static void R_AddNormalMaps( GLbyte *data1, int width1, int height1, GLbyte *data2, int width2, int height2 ) {
	...
	for ( int i = 0; i < height1; i++ ) {
		for ( int j = 0; j < width1; j++ ) {
			...
			if ( len < 1.0f ) {
				n[2] = arcMath::Sqrt(1.0 - (n[0]*n[0] ) - (n[1]*n[1] ) );
			}
...
===================
*/
static void AppendToken( arcNetToken &token ) {
	// add a leading space if not at the beginning
	if ( parseBuffer[0] ) {
		arcNetString::Append( parseBuffer, MAX_IMAGE_NAME, " " );
	}
	arcNetString::Append( parseBuffer, MAX_IMAGE_NAME, token.c_str() );
}
...
===================
*/
static void MatchAndAppendToken( arcLexer &src, const char *match ) {
	if ( !src.ExpectTokenString( match ) ) {
		return;
	}
	// a matched token won't need a leading space
	arcNetString::Append( parseBuffer, MAX_IMAGE_NAME, match );
}
...
===================
*/
static bool R_ParseImageProgram_r( arcLexer &src, GLbyte **pic, int *width, int *height, ARC_TIME_T *timeStamps, textureDepth_t *depth ) {
	arcNetToken token;
...
void R_LoadImageProgram( const char *name, GLbyte **pic, int *width, int *height, ARC_TIME_T *timeStamps, textureDepth_t *depth ) {
	arcLexer src.LoadMemory( name, strlen( name ), name );
...
===============
*/
void arcImageManager::ChangeTextureFilter( void ) {
static filterName_t textureFilters[] = {
	...
	for ( int i = 0; i < 6; i++ ) {
		if ( !arcNetString::Icmp( textureFilters[i].name, string ) ) {
			break;
		}
	}
	...
	for ( i = 0; i < images.Num(); i++ ) {
		...
		if ( glt->texnum == ARCImage::TEXTURE_NOT_LOADED ) {
			continue;
		}
...
===============
*/
void ARCImage::Reload( bool checkPrecompressed, bool force ) {
...
===============
*/
void R_ReloadImages_f( const arcCommandArgs &args ) {
	...
	if ( args.Argc() == 2 ) {
		if ( !arcNetString::Icmp( args.Argv(1 ), "all" ) ) {
			all = true;
		} else if ( !arcNetString::Icmp( args.Argv(1 ), "reload" ) ) {
			all = true;
	...
	for ( int i = 0; i < globalImages->images.Num(); i++ ) {
		...
		ARCImage *image->Reload( checkPrecompressed, all );
	}
...
static int R_QSortImageSizes( const void *a, const void *b ) {
	...
	return arcNetString::Icmp( ea->image->imgName, eb->image->imgName );
}
...
static int R_QsortImageName( const void* a, const void* b ) {
	...
	return arcNetString::Icmp( ea->image->GetName(), eb->image->GetName() );
}
...
===============
*/
void R_ListImages_f( const arcCommandArgs &args ) {
	...
	if ( args.Argc() == 1 ) {
		...
		if ( arcNetString::Icmp( args.Argv( 1 ), "uncompressed" ) == 0 ) {
			uncompressedOnly = true;
		} else if ( arcNetString::Icmp( args.Argv( 1 ), "sorted" ) == 0 ) {
			sorted = true;
		} else if ( arcNetString::Icmp( args.Argv( 1 ), "partial" ) == 0 ) {
			partial = true;
		} else if ( arcNetString::Icmp( args.Argv( 1 ), "unloaded" ) == 0 ) {
			unloaded = true;
		} else if ( arcNetString::Icmp( args.Argv( 1 ), "cached" ) == 0 ) {
			cached = true;
		} else if ( arcNetString::Icmp( args.Argv( 1 ), "uncached" ) == 0 ) {
			uncached = true;
		} else if ( arcNetString::Icmp( args.Argv( 1 ), "tagged" ) == 0 ) {
			matchTag = 1;
		} else if ( arcNetString::Icmp( args.Argv( 1 ), "duplicated" ) == 0 ) {
			duplicated = true;
		} else if ( arcNetString::Icmp( args.Argv( 1 ), "touched" ) == 0 ) {
			touched = true;
		} else if ( arcNetString::Icmp( args.Argv( 1 ), "classify" ) == 0 ) {
			byClassification = true;
			sorted = true;
		} else if ( arcNetString::Icmp( args.Argv( 1 ), "oversized" ) == 0 ) {
			byClassification = true;
			sorted = true;
			overSized = true;
		} else {
	...
	for ( int i = 0; i < globalImages->images.Num(); i++ ) {
		...
		if ( unloaded && image->texnum != ARCImage::TEXTURE_NOT_LOADED ) {
			continue;
		}
		if ( partial && !image->isPartialImage ) {
			continue;
		}
		if ( cached && ( !image->partialImage || image->texnum == ARCImage::TEXTURE_NOT_LOADED ) ) {
			continue;
		}
		if ( uncached && ( !image->partialImage || image->texnum != ARCImage::TEXTURE_NOT_LOADED ) ) {
			continue;
		}
		...
		if ( duplicated ) {
			for ( int j = i + 1; j < globalImages->images.Num(); j++ ) {
				if ( arcNetString::Icmp( image->imgName, globalImages->images[ j ]->imgName ) == 0 ) {
					break;
				}
			}
	...
	if ( byClassification ) {
		arcNetList< int > classifications[IC_COUNT];
		for ( int i = 0; i < count; i++ ) {
...
==================
*/
void arcImageManager::SetNormalPalette( void ) {
	...
	for ( int i = 0; i < 16; i++ ) {
		for ( int j = 0; j < 16; j++ ) {
			...
			temptable[( i*16+j )*3+0] = ( GLbyte )(128 + floor( 127 * v[0] + 0.5 ) );
...
Allocates an ARCImage, adds it to the list,
copies the name, and adds it to the hash chain.
==============
*/
ARCImage *arcImageManager::AllocImage( const char *name ) {
	if ( strlen( name ) >= MAX_IMAGE_NAME ) {
		common->Error( "ImageManager::AllocImage: \"%s\" is too long\n", name );
	}

	int hash = arcNetString( name ).FileNameHash();
	ARCImage *image = new ARCImage;
	images.Append( image );
...
Allocates an arcImage, does not add it to the list or hash chain
==============
*/
arcImage *arcImageManager::AllocStandaloneImage( const char *name ) {
	if  ( strlen( name ) >= MAX_IMAGE_NAME ) {
		common->Error("ImageManager::AllocImage: \"%s\" is too long\n", name );
	}

	arcImage * image = new (TAG_IMAGE) arcImage( name );
...
==================
*/
ARCImage *arcImageManager::ImageFromFunction( const char *_name, void (*generatorFunction)( ARCImage *image ) ) {
	int	hash;

	if ( !name ) {
		common->FatalError( "arcImageManager::ImageFromFunction: NULL name" );
	}

	// strip any .tga file extensions from anywhere in the _name
	arcNetString name = _name;
	name.Replace( ".tga", "" );
	name.BackSlashesToSlashes();

	// see if the image already exists
	int hash = name.FileNameHash();
	for ( ARCImage * image = imageHashTable[hash]; image; image = image->hashNext ) {
	...
	ARCImage *image = AllocImage( name );
...
==============
*/
ARCImage *arcImageManager::ImageFromFile( const char *_name, textureFilter_t filter, bool allowDownSize, textureRepeat_t repeat, textureDepth_t depth, cubeFiles_t cubeMap ) {
	if ( !_name || !_name[0] || arcNetString::Icmp( _name, "default" ) == 0 || arcNetString::Icmp( _name, "_default" ) == 0 ) {
	...
	if ( image->ShouldImageBePartialCached() ) {
		// if we only loaded part of the file, create a new ARCImage for the shrunken version
		image->partialImage = new ARCImage;
...
==============
*/
arcImage	*arcImageManager::GetImageWithParameters( const char *_name, textureFilter_t filter, textureRepeat_t repeat, textureUsage_t usage, cubeFiles_t cubeMap ) const {
	if ( !_name || !_name[0] || arcNetString::Icmp( _name, "default" ) == 0 || arcNetString::Icmp( _name, "_default" ) == 0 ) {
		declManager->MediaPrint( "DEFAULTED\n" );
		return globalImages->defaultImage;
	}

	if ( arcNetString::Icmpn( _name, "fonts", 5 ) == 0 || arcNetString::Icmpn( _name, "newfonts", 8 ) == 0 ) {
		usage = TD_FONT;
	}

	if ( arcNetString::Icmpn( _name, "lights", 6 ) == 0 ) {
		usage = TD_LIGHT;
	}

	// strip any .tga file extensions from anywhere in the _name, including image program parameters
	arcStaticString< MAX_OSPATH > name = _name;
...
/*
===============
arcImageManager::GetImage
===============
*/
ARCImage *arcImageManager::GetImage( const char *_name ) const {
	if ( !_name || !_name[0] || arcNetString::Icmp( _name, "default" ) == 0 || arcNetString::Icmp( _name, "_default" ) == 0 ) {
		declManager->MediaPrint( "DEFAULTED\n" );
		return globalImages->defaultImage;
	}
...
===============
*/
void arcImageManager::PurgeAllImages() {
	for ( int i = 0; i < images.Num(); i++ ) {
		ARCImage *image = images[i];
		image->PurgeImage();
	}
...
===============
*/
void arcImageManager::ReloadAllImages() {
	// build the compressed normal map palette
	SetNormalPalette();
	arcCommandArgs args.TokenizeString( "reloadImages reload", false );
...
===============
*/
void arcImageManager::ReloadImages( bool all ) {
	for ( int i = 0 ; i < globalImages->images.Num() ; i++ ) {
...
===============
*/
void R_CombineCubeImages_f( const arcCommandArgs &args ) {
	if ( args.Argc() != 2 ) {
	...
	arcNetString baseName = args.Argv( 1 );
	...
	for ( int frameNum = 1; frameNum < 10000; frameNum++ ) {
		...
		GLbyte *combined = (GLbyte *)Mem_Alloc( width*height*6*4 );
		for ( int side = 0; side < 6; side++ ) {
...
/*
==================
ARCImage::StartBackgroundImageLoad
==================
*/
void ARCImage::StartBackgroundImageLoad() {
	if ( imageManager.numActiveBackgroundImageLoads >= arcImageManager::MAX_BACKGROUND_IMAGE_LOADS ) {
		return;
	}
	if ( globalImages->image_showBackgroundLoads.GetBool() ) {
		common->Printf( "ARCImage::StartBackgroundImageLoad: %s\n", imgName.c_str() );
	}
	backgroundLoadInProgress = true;

	if ( !precompressedFile ) {
		common->Warning( "arcImageManager::StartBackgroundImageLoad: %s wasn't a precompressed file", imgName.c_str() );
		return;
	}
	...
	if ( !bgl.f ) {
		common->Warning( "arcImageManager::StartBackgroundImageLoad: Couldn't load %s", imgName.c_str() );
		return;
	}
	bgl.file.position = 0;
	bgl.file.length = bgl.f->Length();
	if ( bgl.file.length < sizeof( ddsFileProperties_t ) ) {
		common->Warning( "arcImageManager::StartBackgroundImageLoad: %s had a bad file length", imgName.c_str() );
		return;
	}
	...
	int		totalSize = 0;
	for ( ARCImage *check = globalImages->cacheLRU.cacheUsageNext; check != &globalImages->cacheLRU; check = check->cacheUsageNext ) {
	...
	while ( ( totalSize + needed ) > globalImages->image_cacheMegs.GetFloat() * 1024 * 1024 ) {
		...
		ARCImage	*check = globalImages->cacheLRU.cacheUsagePrev;
...
==================
*/
void arcImageManager::CompleteBackgroundImageLoads() {
	ARCImage *remainingList = NULL;
	ARCImage *next;

	for ( ARCImage *image = backgroundImageLoads; image; image = next ) {
		next = image->bglNext;
...
===============
*/
void arcImageManager::CheckCvars() {
	// textureFilter stuff
...
===============
*/
int arcImageManager::SumOfUsedImages() {
	int total = 0;
	for ( int i = 0; i < images.Num(); i++ ) {
		ARCImage *image = images[i];
...
====================
*/
void arcImageManager::BeginLevelLoad() {//( bool pacifier ) {
...
/*
====================
arcImageManager::ExcludePreloadImage
====================
*/
bool arcImageManager::ExcludePreloadImage( const char *name ) {
	arcStaticString< MAX_OSPATH > imgName = name;
	imgName.ToLower();
...
/*
====================
arcImageManager::Preload
====================
*/
void arcImageManager::Preload( const idPreloadManifest &manifest, const bool & mapPreload ) {
...
/*
===============
arcImageManager::LoadLevelImages

i dont think were gonna use this combine with begin level load?
===============
*/
int arcImageManager::LoadLevelImages( bool pacifier ) {
	int	loadCount = 0;
	for ( int i = 0 ; i < images.Num() ; i++ ) {
		if ( pacifier ) {
			common->UpdateLevelLoadPacifier();
...
====================
*/
void arcImageManager::EndLevelLoad() {
	insideLevelLoad = false;

	int start = Sys_Milliseconds();
	int	loadCount = LoadLevelImages( true );
	int	end = Sys_Milliseconds();
	common->Printf( "----- arcImageManager::EndLevelLoad -----\n" );
	...
	for ( int i = 0; i < images.Num(); i++ ) {
		...
		if ( !image->levelLoadReferenced && !image->referencedOutsideLevelLoad ) {
			//common->Printf( "Purging %s\n", image->imgName.c_str() );
			int purgeCount++;
			int image->PurgeImage();
		} else if ( image->texnum != ARCImage::TEXTURE_NOT_LOADED ) {
	...
	for ( int i = 0; i < images.Num(); i++ ) {
		...
		if ( image->levelLoadReferenced && image->texnum == ARCImage::TEXTURE_NOT_LOADED && !image->partialImage ) {
...
/*
===============
arcImageManager::StartBuild
===============
*/
void arcImageManager::StartBuild() {
	ddsList.Clear();
	ddsHash.Free();
}
...
/*
===============
arcImageManager::FinishBuild
===============
*/
void arcImageManager::FinishBuild( bool removeDups ) {
	...
	if ( removeDups ) {
		...
		if ( buffer ) {
			arcNetString str = buffer;
			while ( str.Length() ) {
				int n = str.Find( '\n' );
				if ( n > 0 ) {
					arcNetString line = str.Left( n + 1 );
					arcNetString right;
	...
	arcNetFile *batchFile = fileSystem->OpenFileWrite( ( removeDups ) ? "makedds2.bat" : "makedds.bat" );
	if ( batchFile ) {
...
/*
===============
arcImageManager::AddDDSCommand
===============
*/
void arcImageManager::AddDDSCommand( const char *cmd ) {
	int i, key;
...
/*
===============
arcImageManager::PrintMemInfo
===============
*/
void arcImageManager::PrintMemInfo( MemInfo_t *mi ) {
	int i, j, total = 0;
	int *sortIndex;
	arcNetFile *f;
	...
	// print next
	for ( i = 0; i < images.Num(); i++ ) {
		ARCImage *im = images[sortIndex[i]];
		int size;

		size = im->StorageSize();
		total += size;

		f->Printf( "%s %3i %s\n", arcNetString::FormatNumber( size ).c_str(), im->refCount, im->imgName.c_str() );
	}
...
================
*/
int ARCImage::BitsForInternalFormat( int internalFormat ) const {
...
========================
*/
void arcImage::AllocImage( textureFilter_t tf, textureRepeat_t tr ) {
	filter = tf;
	repeat = tr;
	//opts = imgOpts;
	DeriveOpts();
...
==================
*/
void ARCImage::UploadCompressedNormalMap( int width, int height, const GLbyte *rgba, int mipLevel ) {
...
===============
*/
GLenum ARCImage::SelectInternalFormat( const GLbyte **dataPtrs, int numDataPtrs, int width, int height, textureDepth_t minimumDepth, bool *monochromeResult ) const {
...
==================
*/
void ARCImage::SetImageFilterAndRepeat() const {
...
================
*/
void ARCImage::GetDownsize( int &scaledWidth, int &scaledHeight ) const {
	int size = 0;
...
================
*/
void arcImage::GenerateAlternateImage( const GLbyte *pic, int width, int height, textureFilter_t filterParm, textureRepeat_t repeatParm, textureUsage_t usageParm ) {
	PurgeImage();
...
================
*/
void ARCImage::GenerateImage( const GLbyte *pic, int width, int height, textureFilter_t filterParm, bool allowDownSizeParm, textureRepeat_t repeatParm, textureDepth_t depthParm ) {
	PurgeImage();
...
==================
*/
void ARCImage::Generate3DImage( const GLbyte *pic, int width, int height, int picDepth, textureFilter_t filterParm, bool allowDownSizeParm, textureRepeat_t repeatParm, textureDepth_t minDepthParm ) {
	PurgeImage();
...
====================
*/
void ARCImage::GenerateCubeImage( const GLbyte *pic[6], int size, textureFilter_t filterParm, bool allowDownSizeParm, textureDepth_t depthParm ) {
	PurgeImage();
	...
	for ( int i = 0; i < 6; i++ ) {
		qglTexImage2D( GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT+i, 0, internalFormat, scaledWidth, scaledHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, pic[i] );
	}
	...
	while ( scaledWidth > 1 ) {
		for ( i = 0; i < 6; i++ ) {
			qglTexImage2D( GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT+i, miplevel, internalFormat,
				scaledWidth / 2, scaledHeight / 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, shrunk[i] );

			if ( scaledWidth > 2 ) {
				GLbyte *shrunken = R_MipMap( shrunk[i], scaledWidth/2, scaledHeight/2, false );
			} else {
				GLbyte *shrunken = NULL;
			}
...
/*void arcImage::GenerateShadowArray( int width, int height, textureFilter_t filterParm, textureRepeat_t repeatParm, textureUsage_t usageParm ) {
	PurgeImage();
...
==================
*/
int	ARCImage::NumLevelsForImageSize( int width, int height ) const {
	int	numLevels = 1;
...
int arcImage::NumLevelsForImageSize( int width, int height, int internalFormat ) {
	int numLevels = 1;
	int minSize;
...
/*
==================
arcImage::DataSizeForImageSize

  FIXME: the switch statement is far from complete
==================
*/
int arcImage::DataSizeForImageSize( int width, int height, int internalFormat ) {
...
================
*/
void ARCImage::ImageProgramStringToCompressedFileName( const char *imageProg, char *fileName ) const {
	strcpy( fileName, "dds/" );
...
================
*/
void ARCImage::WritePrecompressedImage() {
	...
	if ( globalImages->image_useOffLineCompression.GetBool() && FormatIsDXT( altInternalFormat ) ) {
		arcNetString outFile = fileSystem->RelativePathToOSPath( filename, "fs_basepath" );
		arcNetString inFile = outFile;
...
================
*/
bool ARCImage::ShouldImageBePartialCached() {
	if ( !qglConfig.textureCompression ) {
		return false;
	}
...
================
*/
bool ARCImage::CheckPrecompressedImage( bool fullLoad ) {
...
===================
*/
void ARCImage::UploadPrecompressedImage( GLbyte *data, int len ) {
...
On exit, the ARCImage will have a valid OpenGL texture number that can be bound
===============
*/
void ARCImage::ActuallyLoadImage( bool checkForPrecompressed, bool fromBackEnd ) {
	int width, height;
	GLbyte *pic;
...
void ARCImage::PurgeImage() {
	...
	for ( int i = 0; i < MAX_MULTITEXTURE_UNITS; i++ ) {
		backEnd.qglState.tmu[i].current2DMap = -1;
		backEnd.qglState.tmu[i].current3DMap = -1;
		backEnd.qglState.tmu[i].currentCubeMap = -1;
	}
...
==============
*/
void ARCImage::BindFragment() {
	if ( tr.logFile ) {
		RB_LogComment( "ARCImage::BindFragment %s )\n", imgName.c_str() );
	}
...
====================
*/
void ARCImage::CopyFramebuffer( int x, int y, int imageWidth, int imageHeight, bool useOversizedBuffer ) {
	Bind();
...
====================
*/
void ARCImage::CopyDepthbuffer( int x, int y, int imageWidth, int imageHeight ) {
	Bind();
...
=============
*/
void ARCImage::UploadScratch( const GLbyte *data, int cols, int rows ) {
	// if rows = cols * 6, assume it is a cube map animation
	if ( rows == cols * 6 ) {
		if ( type != TT_CUBIC ) {
			type = TT_CUBIC;
		...
		if ( cols != uploadWidth || rows != uploadHeight ) {
			...
			// upload the base level
			for ( int i = 0; i < 6; i++ ) {
				qglTexImage2D( GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT+i, 0, GL_RGB8, cols, rows, 0, GL_RGBA, GL_UNSIGNED_BYTE, data + cols*rows*4*i );
			}
		} else {
			// otherwise, just subimage upload it so that drivers can tell we are going to be changing
...
void ARCImage::SetClassification( int tag ) {
	classification = tag;
}
...
==================
*/
int ARCImage::StorageSize() const {
	if ( texnum == TEXTURE_NOT_LOADED ) {
		return 0;
	}
...
int ARCImage::GetImageId( const char *name ) const {
	//GetName();
	//Printf( PRINT_ALL, "ImageId [%s].\n", name );
	for ( int i = 0 ; i < images.Num() ; i++ ) {
		if ( !strcmp( name, images[ i ]->imgName ) ) {
			//Printf( PRINT_ALL, "Found ImageId %d\n", i );
			return i;
		}
	}

	//Printf( PRINT_ALL, "Image not found.\n" );
	return -1;
}
