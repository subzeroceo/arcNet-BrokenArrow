...
typedef struct renderEntity_s {
	ARCRenderModel					*hModel;				// this can only be null if callback is set
	...
	arcBounds					bounds;					// only needs to be set for deferred models and md5s
	...
	// texturing
	const arcMaterial 				*customShader;	// if non-0, all surfaces will use this
	const arcMaterial 				*shaderRef;		// used so flares can reference the proper light shader
	...
	class arcUserInterfaces			*gui[ MAX_RENDERENTITY_GUI ];
...
typedef struct renderLight_s {
	arcMat3					axis;				// rotation vectors, must be unit length
	arcVec3					origin;
	...
	arcVec3					lightRadius;		// xyz radius for point lights
	arcVec3					lightCenter;		// offset the lighting direction for shading and
	...
	const arcMaterial		*shader;				// NULL = either lights/defaultPointLight or lights/defaultProjectedLight
...
typedef struct renderView_s {
	...
	float					fov_x, fov_y;
	arcVec3					viewOrg;
	arcMat3					viewAxis;			// transformation matrix, view looks down the positive X axis
	...
	const arcMaterial		*globalMaterial;		// used to override everything draw
} renderView_t;

// exitPortal_t is returned by ARCRenderWorld::GetPortal()
typedef struct {
	int						areas[2];		// areas connected by this portal
	const arcWinding		*w;				// winding points have counter clockwise ordering seen from areas[0]
	int						blockingBits;	// PS_BLOCK_VIEW, PS_BLOCK_AIR, etc
	arcNetHandle_t			portalHandle;
} exitPortal_t;

// guiPoint_t is returned by ARCRenderWorld::GuiTrace()
typedef struct {
...
typedef struct modelTrace_s {
	...
	arcVec3						point;			// end point of trace in global space
	arcVec3						normal;			// hit triangle normal vector in global space
	arcVec2						st;					// texture coordinate
	const arcMaterial			*material;		// material of hit surface
	const srfTriangles_t	*	geometry;
	arcVec3						surfaceColor;
	...
	const arcMaterialClass		*surfaceClass; // for material types/classes
} modelTrace_t;
...
public:
	...
	virtual ARCRenderModel	*GetEntityHandleDynamicModel( qhandle_t entityHandle ) = 0;

	//-------------- Decals and Overlays  -----------------
	//virtual ARCRenderModel *CreateDecalModel() = 0;
	//virtual void 			AddToProjectedDecal( const idFixedWinding& winding, const arcVec3 &projectionOrigin, const bool parallel, const arcVec4& color, idRenderModel* model, int entityNum, const arcMaterial** onlyMaterials = NULL, const int numOnlyMaterials = 1 ) = 0;
	...
	virtual void			ProjectDecalOntoWorld( const arcFixedWinding &winding, const arcVec3 &projectionOrigin, const bool parallel, const float fadeDepth, const arcMaterial *material, const int startTime ) = 0;

	// Creates decals on static models.
	virtual void			ProjectDecal( arcNetHandle_t entityHandle, const arcFixedWinding &winding, const arcVec3 &projectionOrigin, const bool parallel, const float fadeDepth, const arcMaterial *material, const int startTime ) = 0;

	// Creates overlays on dynamic models.
	virtual void			ProjectOverlay( arcNetHandle_t entityHandle, const arcPlane localTextureAxis[2], const arcMaterial *material ) = 0;
	...
	//-------------- Scene Rendering -----------------
	//virtual void			AddEnvBounds( arcVec3 const &origin, arcVec3 const &scale, const char *cubemap ) = 0;
	...
	virtual	arcNetHandle_t	FindPortal( const arcBounds &b ) const = 0;
	...
	virtual int				PointInArea( const arcVec3 &point ) const = 0;

	// fills the *areas array with the numbers of the areas the bounds cover
	// returns the total number of areas the bounds cover
	virtual int				BoundsInAreas( const arcBounds &bounds, int *areas, int maxAreas ) const = 0;
	...
	virtual guiPoint_t		GuiTrace( arcNetHandle_t entityHandle, const arcVec3 start, const arcVec3 end ) const = 0;

	// Traces vs the render model, possibly instantiating a dynamic version, and returns true if something was hit
	virtual bool			ModelTrace( modelTrace_t &trace, arcNetHandle_t entityHandle, const arcVec3 &start, const arcVec3 &end, const float radius ) const = 0;

	// Traces vs the whole rendered world. FIXME: we need some kind of material flags.
	virtual bool			Trace( modelTrace_t &trace, const arcVec3 &start, const arcVec3 &end, const float radius, bool skipDynamic = true, bool skipPlayer = false ) const = 0;

	// Traces vs the world model bsp tree.
	virtual bool			FastWorldTrace( modelTrace_t &trace, const arcVec3 &start, const arcVec3 &end ) const = 0;
	...
	virtual void			DebugLine( const arcVec4 &color, const arcVec3 &start, const arcVec3 &end, const int lifeTime = 0, const bool depthTest = false ) = 0;
	virtual void			DebugArrow( const arcVec4 &color, const arcVec3 &start, const arcVec3 &end, int size, const int lifeTime = 0 ) = 0;
	virtual void			DebugWinding( const arcVec4 &color, const arcWinding &w, const arcVec3 &origin, const arcMat3 &axis, const int lifeTime = 0, const bool depthTest = false ) = 0;
	virtual void			DebugCircle( const arcVec4 &color, const arcVec3 &origin, const arcVec3 &dir, const float radius, const int numSteps, const int lifeTime = 0, const bool depthTest = false ) = 0;
	virtual void			DebugSphere( const arcVec4 &color, const ARCSphere &sphere, const int lifeTime = 0, bool depthTest = false ) = 0;
	virtual void			DebugBounds( const arcVec4 &color, const arcBounds &bounds, const arcVec3 &org = vec3_origin, const int lifeTime = 0 ) = 0;
	virtual void			DebugBoundsDepthTest( const arcVec4 &color, const arcBounds &bounds, const arcVec3 &org = vec3_origin, const int lifetime = 0, bool depthTest = false ) = 0;

	virtual void			DebugBox( const arcVec4 &color, const ARCBox &box, const int lifeTime = 0 ) = 0;
	virtual void			DebugFrustum( const arcVec4 &color, const ARCFrustum &frustum, const bool showFromOrigin = false, const int lifeTime = 0 ) = 0;
	virtual void			DebugCone( const arcVec4 &color, const arcVec3 &apex, const arcVec3 &dir, float radius1, float radius2, const int lifeTime = 0 ) = 0;
	virtual void			DebugAxis( const arcVec3 &origin, const arcMat3 &axis ) = 0;

	// Polygon drawing for debug visualization.
	virtual void			DebugClearPolygons( int time ) = 0;		// a time of 0 will clear all polygons
	virtual void			DebugPolygon( const arcVec4 &color, const arcWinding &winding, const int lifeTime = 0, const bool depthTest = false ) = 0;

 	virtual void			DebugFOV( const arcVec4 &color, const arcVec3 &origin, const arcVec3 &dir, float farDot, float farDist, float nearDot = 1.0f, float nearDist = 0.0f, float alpha = 0.3f, int lifetime = 0 ) = 0;

	virtual size_t			MemorySummary( const ARCCommandArgs &args ) = 0;
	...
	virtual void			DrawText( const char *text, const arcVec3 &origin, float scale, const arcVec4 &color, const arcMat3 &viewAxis, const int align = 1, const int lifeTime = 0, bool depthTest = false ) = 0;
