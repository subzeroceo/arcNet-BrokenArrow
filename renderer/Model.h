...
typedef struct {
	// NOTE: making this a qglIndex is dubious, as there can be 2x the faces as verts
	qglIndex_t					p1, p2;					// planes defining the edge
	qglIndex_t					v1, v2;					// verts defining the edge
} silEdge_t;
...
typedef struct surfTriangles_s {
	...
	qglIndex_t *				silIndexes;				// indexes changed to be the first vertex with same XYZ, ignoring normal and texcoords
...
public:
	...
	// for reloadModels
	virtual ARC_TIME_T			Timestamp() const = 0;
