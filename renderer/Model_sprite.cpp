...
===============
*/
ARCRenderModel *	idRenderModelSprite::InstantiateDynamicModel( const struct renderEntity_s *renderEntity, const struct viewDef_s *viewDef, ARCRenderModel *cachedModel ) {
	...
	if ( cachedModel != NULL ) {
		...
		assert( arcNetString::Icmp( cachedModel->Name(), sprite_SnapshotName ) == 0 );
	...
	arcVec3 right	= arcVec3( 0.0f, renderEntity->shaderParms[ SHADERPARM_SPRITE_WIDTH ] * 0.5f, 0.0f );
	arcVec3 up		= arcVec3( 0.0f, 0.0f, renderEntity->shaderParms[ SHADERPARM_SPRITE_HEIGHT ] * 0.5f );
...
===============
*/
arcBounds idRenderModelSprite::Bounds( const struct renderEntity_s *renderEntity ) const {
	arcBounds b;
