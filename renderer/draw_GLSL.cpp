...
static arcCVarSystem harm_r_lightModel("harm_r_lightModel", harm_r_lightModelArgs[0], CVAR_RENDERER|CVAR_ARCHIVE, "[Harmattan]: Light model when draw interactions(`phong` - Phong(default), `blinn_phong` - Blinn-Phong.)", harm_r_lightModelArgs, idCmdSystem::ArgCompletion_String<harm_r_lightModelArgs>);
static arcCVarSystem harm_r_specularExponent("harm_r_specularExponent", "4.0", CVAR_FLOAT|CVAR_RENDERER|CVAR_ARCHIVE, "[Harmattan]: Specular exponent in interaction light model(default is 4.0.)");
#if 0
#define _GLPROGS "gl2progs"
#else
#define _GLPROGS "glslprogs"
#endif
static arcCVarSystem	harm_r_shaderProgramDir( "harm_r_shaderProgramDir", "", CVAR_SYSTEM | CVAR_INIT | CVAR_SERVERINFO, "[Harmattan]: Special external GLSL shader program directory path(default is empty, means using `" _GLPROGS "`).");
...
void RB_GLSL_CreateDrawInteractions(const drawSurf_t *surf)
{
	...
	for (; surf ; surf=surf->nextOnLight) {
		...
		GL_VertexAttribPointer(offsetof(shaderProgram_t, attr_Normal), 3, GL_FLOAT, false, sizeof(arcDrawVert), ac->normal.ToFloatPtr());
		GL_VertexAttribPointer(offsetof(shaderProgram_t, attr_Bitangent), 3, GL_FLOAT, false, sizeof(arcDrawVert), ac->tangents[1].ToFloatPtr());
		GL_VertexAttribPointer(offsetof(shaderProgram_t, attr_Tangent), 3, GL_FLOAT, false, sizeof(arcDrawVert), ac->tangents[0].ToFloatPtr());
		GL_VertexAttribPointer(offsetof(shaderProgram_t, attr_TexCoord), 2, GL_FLOAT, false, sizeof(arcDrawVert), ac->st.ToFloatPtr());

		GL_VertexAttribPointer(offsetof(shaderProgram_t, attr_Vertex), 3, GL_FLOAT, false, sizeof(arcDrawVert), ac->xyz.ToFloatPtr());
		GL_VertexAttribPointer(offsetof(shaderProgram_t, attr_Color), 4, GL_UNSIGNED_BYTE, false, sizeof(arcDrawVert), ac->color);
...
{
#if 0
	arcNetString	fullPath = "gl2progs/";
	fullPath += name;
#else
	arcNetString	fullPath = cvarSystem->GetCVarString("harm_r_shaderProgramDir");
	if (fullPath.IsEmpty())
		fullPath = _GLPROGS;
...
static void RB_GLSL_GetUniformLocations(shaderProgram_t *shader)
{
	...
	for (i = 0; i < MAX_VERTEX_PARMS; i++) {
		arcNetString::snPrintf(buffer, sizeof(buffer), "u_vertexParm%d", i);
		shader->u_vertexParm[i] = glGetAttribLocation(shader->program, buffer);
	}

	for (i = 0; i < MAX_FRAGMENT_IMAGES; i++) {
		arcNetString::snPrintf(buffer, sizeof(buffer), "u_fragmentMap%d", i);
	...
	for ( i = 0; i < MAX_FRAGMENT_IMAGES; i++ ) {
		arcNetString::snPrintf(buffer, sizeof(buffer), "u_fragmentCubeMap%d", i);
...
static void R_InitGLSLCvars( void )
{
	...
	r_usePhong = !(lightModel && !arcNetString::Icmp(HARM_INTERACTION_SHADER_BLINNPHONG, lightModel));
...
void R_ReloadGLSLPrograms_f(const arcCommandArgs &args)
{
	int		i;
