...
bool ARCRenderWorldLocal::PortalIsFoggedOut( const portal_t *p ) {
	...
	lightShader->EvaluateRegisters( regs, ldef->parms.shaderParms, tr.viewDef, ldef->parms.sndRef );
...
void ARCRenderWorldLocal::AddAreaLightRefs( int areaNum, const portalStack_t *ps ) {
	...
	for ( lref = area->lightReferences.areaNext; lref != &area->lightReferences; lref = lref->areaNext ) {
		light = lref->light;
...
All the modelrefs and lightReferences that are in visible areas
