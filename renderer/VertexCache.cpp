...
arcCVarSystem ARCVertCache::r_showVertexCache( "r_showVertexCache", "0", CVAR_INTEGER|CVAR_RENDERER, "" );
arcCVarSystem ARCVertCache::r_vertexBufferMegs( "r_vertexBufferMegs", "32", CVAR_INTEGER|CVAR_RENDERER, "" );

ARCVertCache		vertexCache;
#pragma hdrstop
#include "../idlib/precompiled.h"
#include "tr_local.h"
...
========================
*/
void ARCGraphicsBuffer::InitVertexArrayObject() {
...
void ARCGraphicsBuffer::RenderView( viewDef_t *parms ) {
	...
	static_cast<ARCRenderWorldLocal *>( parms->renderWorld )->FindViewLightsAndEntities();
	...
	//static_cast<ARCRenderWorldLocal *>( parms->renderWorld )->WriteVisibleDefs( tr.viewDef );
...
==============
*/
static void ARCGraphicsBuffer::ListVertexCache_f( const arcCommandArgs &args ) {
	vertexCache.List();
}
...
/*
==============
ARCVertCache::ActuallyFree
==============
*/
void ARCVertCache::ActuallyFree( vertCache_t *block ) {
	if ( !block ) {
...
==============
*/
void *ARCVertCache::Position( vertCache_t *buffer ) {
	if ( !buffer || buffer->tag == TAG_FREE ) {
		common->FatalError( "ARCVertCache::Position: bad vertCache_t" );
	}
...
/*
===========
ARCVertCache::Shutdown
===========
*/
void ARCVertCache::Shutdown() {
...
===========
*/
void ARCVertCache::Alloc( void *data, int size, vertCache_t **buffer, bool indexBuffer ) {
	vertCache_t	*block;

	if ( size <= 0 ) {
		common->Error( "ARCVertCache::Alloc: size = %i\n", size );
	}
...
/*
===========
ARCVertCache::AllocStaticVertex
===========
*/
vertCacheHandle_t ARCVertCache::AllocStaticVertex( const void *data, int bytes ) {
	if ( staticData.vertexMemUsed.GetValue() + bytes > STATIC_VERTEX_MEMORY ) {
		idLib::FatalError( "ARCVertCache::Alloc: size = %i update or increase STATIC_VERTEX_MEMORY\n ", bytes );
	}
...
/*
===========
ARCVertCache::AllocStaticIndex
===========
*/
vertCacheHandle_t ARCVertCache::AllocStaticIndex( const void *data, int bytes ) {
...
===========
*/
void ARCVertCache::Touch( vertCache_t *block ) {
	if ( !block ) {
		common->Error( "ARCVertCache Touch: NULL pointer" );
	}

	if ( block->tag == TAG_FREE ) {
		common->FatalError( "ARCVertCache Touch: freed pointer" );
	}
	if ( block->tag == TAG_TEMP ) {
		common->FatalError( "ARCVertCache Touch: temporary pointer" );
	}
...
===========
*/
void ARCVertCache::Free( vertCache_t *block ) {
	if ( !block ) {
		return;
	}

	if ( block->tag == TAG_FREE ) {
		common->FatalError( "ARCVertCache Free: freed pointer" );
	}
	if ( block->tag == TAG_TEMP ) {
		common->FatalError( "ARCVertCache Free: temporary pointer" );
	}
...
===========
*/
vertCache_t	*ARCVertCache::AllocFrameTemp( void *data, int size ) {
	if ( size <= 0 ) {
		common->Error( "ARCVertCache::AllocFrameTemp: size = %i\n", size );
	}
...
/*
===========
ARCVertCache::CacheIsCurrent
===========
*/
bool ARCVertCache::CacheIsCurrent( const vertCacheHandle_t handle ) {
...
/*
===========
ARCVertCache::EndFrame
===========
*/
void ARCVertCache::EndFrame() {
...
vertCache_t *ARCVertCache::CreateTempVbo( int bytes, bool indexBuffer ) {
...
int ARCVertCache::GetNextListNum( void ) const {
