...
typedef	struct aasFeature_s {
	...
	void			DrawDebugInfo( int index=-1 );
  	int				GetLookPos( arcVec3& lookPos, const arcVec3& aimAtOrigin, const float leanDistance=16.0f );
} aasFeature_t;										//--------------------------------
													// 12 Bytes
// RAVEN END
...
// jsinger: changed to be Lexer instead of idLexer so that we have the ability to read binary files
	...
	bool						ParseVector( Lexer &src, arcVec3 &vec );
